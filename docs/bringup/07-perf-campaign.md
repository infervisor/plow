...
numerics-changing lever was re-verified. Nothing here recompiles or re-tunes;
Stage 7 only *measures* and *documents*. A number that moves a lever belongs in
Stage 5/6, not here.
...
* **Same-session.** Both engines (plow and any comparator) measured in **one
  ...
  `perf-data/plow-gfx942/README.md` records a vLLM number moving **33% (7.57 →
  10.03 ms)** on re-measure, which silently re-rated every ratio in the
  directory. Re-baselining both engines in one session is repeatedly called out
  as *the single most valuable missing measurement*.
...
* **Swept, not single-point.** plow's fixed-width decode makes its position
  strongly concurrency- and context-dependent — it can win at c4, tie at c8,
  lose at c16 on one model (`perf-data/b2-concurrency-family.md`). One point is
  not a capacity claim; a curve is.
...
| `vllm bench serve` (`--backend openai-chat`) | single-stream + low-concurrency latency, and the like-for-like comparator (same client both engines) | `perf-data/harness/bringup_bench.sh`, orchestrated by `bringup_showdown.sh` |

Supporting scripts in `perf-data/harness/`:
...
| `bringup_ceiling.py` | cuBLASLt/torch fp8+bf16 GEMM ceiling at your model's exact prefill shapes — the roofline reference (edit `SHAPES`) |
...
built `plowrt` (`nix develop`; `cargo build -p plowrt --release --features cuda`
or `--features hsa`) and the Stage 6 recipe.

### 0. Environment sanity (invalidates everything downstream)

Before any number, confirm the box (`perf-data/harness/BRINGUP.md` §0):

* GPU actually used — grep the serve log for `backend ready — GPU accelerated`.
  CPU fallback is a **warning, not an error**; it serves correct text slowly and
  an unwary bench measures the CPU path.
* No resident foreign compute (`nvidia-smi --query-compute-apps=...` /
  `rocm-smi --showuse` reads 0% before any A/B) — the persistent cooperative
  megakernel outlives its host process, so a bench started while a prior `serve`
  tears down reads a contended box.
* Toolchain via `nix develop`; profiler availability (`sudo ncu` vs `nsys`)
  checked once per box.
...
```bash
perf-data/harness/gpulease gate \
  perf-data/harness/bringup_gate.sh $ASSETS plow-fp8 8080
# reference arm, then:
...
On GH200/12B this measured fp8 1324–1468 / bf16 804–861 TF/s and is what
separates "the kernel is slow" from "the box is saturated"
(`perf-data/gemma12b-gh200-prefill-campaign.md`). Decode capacity is usually
**HBM-bandwidth-bound**: state the bytes moved and the achieved GB/s, not just
the ms (`perf-data/plow-gfx942/README.md` derives plow at ~970 GB/s = 18% of peak
vs vLLM at ~2.4 TB/s = 45%).
...
Mandatory cross-engine checks (`BRINGUP.md` §6):
...
python3 perf-data/harness/b2-ib/slo_capacity.py  # max-users under each SLO
```
...
32768 65536"`), reporting TTFT and TPOT at each. This is where plow's
sliding-window / flash-decode path is characterized: TPOT is often close to
context-flat while a comparator's grows (`perf-data/plow-gfx942/README.md`: plow
TPOT +7.5% over 1k→64k vs vLLM +87% on 12B). State the crossover if you compute
one, and state that it is a trend, not a served point.
...
> credit it when reading a TTFT ratio (`BRINGUP.md` §3,
...
> recompile at a larger `--max-ctx`, do not re-run.
...
Committed under `perf-data/`. The per-arch results home is
`perf-data/plow-gfx942/` (linked from the README's perf section); a campaign
targeting one GPU family lands there, cross-arch capacity reports at the
`perf-data/` root (e.g. `b2-concurrency-family.md`, `serving-capacity-report.md`).

Structure a campaign write-up to the established format:

* **Header line**: date, branch @ commit, box (GPU, arch, CU/SM count, driver,
  CUDA/ROCm), engine configs (both), harness (tool + pinned rev), profile (warm
  + measure durations, greedy, output tokens), and the TTFT convention.
* **Scope-actually-measured banner** (an *honesty banner*): state plainly what
  ...
  request labels the un-run rows and does not project them. `perf-data/plow-gfx942/`
  additionally requires a `Scope:` class line (CDNA3-SPECIFIC / AMD-GENERAL /
  PLOW-ARCHITECTURAL / METHOD / …) so a finding cannot silently inherit the
  directory's arch claim.
...
4. **Swept, not single-point.** A concurrency ladder and a context ladder, not
   one cell.
5. **Documented in `perf-data/`.** A committed write-up in the established format
...
* **A/B order / session drift bias.** Arms benched under different background load
  ...
  everything** (`perf-data/amd-bench-ab-order-bias.md`, `BRINGUP.md` rule 3).
...
* **Only concurrency-1 rows compare kernels on a single-sequence engine.** plow's
  AMD serve is `batch=1`, so its concurrency ladder measures requests *queueing*
  while a continuous-batching comparator climbs — the ratio columns there are not
  a kernel comparison (`perf-data/plow-gfx942/README.md` caveat 2).
...
* **Never launch a battery twice / kill with SIGTERM.** Two copies on one port →
  ...
  tear down.
* **Standalone probes overstate** (THE PROBE LAW): a +15% probe was −3 ms
  in-model; an isolated GEMV scored a lever 1.00× where the megakernel measured
  1.43×. A campaign quotes only in-model, in-serve numbers.
...
| path | role |
|---|---|
| `perf-data/harness/gpulease` | advisory lease + contention audit (rc=76 = contended) |
| `perf-data/harness/bringup_gate.sh` | token-identity correctness gate |
| `perf-data/harness/bringup_bench.sh` | one client pass (`vllm bench serve`), appends `cells.tsv` |
| `perf-data/harness/bringup_showdown.sh` | sequential-exclusive multi-arm showdown template |
| `perf-data/harness/bringup_ceiling.py` | cuBLASLt/torch GEMM ceiling at your exact shapes |
| `perf-data/harness/BRINGUP.md` | the full bring-up runbook — Stage 0 sanity, gate discipline, attribution ladder, benchmark protocol §6, probe law |
...
| `perf-data/plow-gfx942/` | the per-arch results home (README = index, LESSONS = method) |
