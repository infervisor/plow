...
| 41 | 2026-09-11 | branch `shared-expert-fold` | perf | devgen mla.rs `glm_shared_fold`, exec/amd_moe_aiter.rs, exec/amd.rs `bind_packed_experts`, runtime/amd/{op_moe.h,moe_aiter_adapter.hip} | New opt-in `PLOW_GLM_MOE_SHARED_FOLD` (default off; knob-off packet byte-identical): GLM shared expert folded into the native AITER MoE call as expert 256, 257/top-9 on the pinned objects (AITER gfx942 GLM-5 table is indexed 257/9). ABA on 8×MI300X: −25 ms per 8192 prefill chunk (controls within 2 ms), +2.2% C20 out tok/s (within the 1.8% control spread), retrieval 18/18 ×3. Shared expert moves bf16 → A8 (per-row 3.9–4.8% rel-L2 vs f64, the routed A8 floor). Needs packet-stamped objects incl. `interp_mla_{small,split}_fp8kv_gq` | open — default flip held: per-row/column numerics decomposition (job 1789166236) and a broad accuracy eval owed |
