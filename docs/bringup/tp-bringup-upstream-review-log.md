...
| 51 | 2026-09-12 | (pre-existing) | test flake | `crates/devgen` lib tests (`mla::kimi_tests`, `manifest::tests::the_lean_block_does_not_change_the_pairing_hash`) | **`cargo test -p devgen --lib` is flaky in parallel.** Same test binary (no crate change on branch `collective-bw`): 446/446 in one run, 1 failure in the next, 15 in a run that overlapped a heavy hipcc build; every failing test passes alone (`kimi_tests` 46/46). The failing Kimi tests see the FP8-KV emit path switched on (`HeadNormRopeFp8` / `FlashMlaDecodeFp8` where they expect `RmsNorm` / `FlashMlaDecode`) with no `PLOW_*` / `GLM_*` knob in the environment — a process-global emit-config override leaking between concurrent tests, more likely under CPU load. Rerun with `--test-threads=1` (or on a quiet box) before calling a devgen gate red. | open |
| 52 | 2026-09-12 | 2388560e (collective-bw), a6b39a0d (hsa-numa-kernarg) | perf (default flip, pair) | `PLOW_XR_SCHED=aiter` RS 8 / AG 24 prefill objects + `PLOW_AMD_NUMA_HOST_POOLS=1` | **Combined served A/B qualifies both as defaults, as a pair.** Job `0-1789190409z-combined-t4`: one binary (a6b39a0d, clean export) in every arm, `PLOW_AMD_TAIL_SPARSE_CTX=16384` in every arm (the current default), treat = the rs8ag24 object set + NUMA host pools; 20 x 70k/700 at C20, then a 20-conversation prefix workload, then retrieval `--suite all`. Out tok/s 61.35 / 63.24 / 60.66 (ctrl / treat / ctrl2): +3.66% vs the control mean, controls 1.14% apart. Prefill tick p50 888.8 / 857.0 / 890.3 ms (-31.8 ms vs a 1.5 ms control gap), decode-only tick p50 89.6 / 87.7 / 92.1 ms (-3.4%, just outside the 2.7% control spread), TTFT -4.3%, TPOT -4.7%. Retrieval 18/18 base + 21/21 tail in all three arms. In serve the NUMA placement held (treat ranks 4-7 kernarg pages on nodes 7/7/4/7, ctrl all on node 0), but its decode gain is far below the 7-layer amd-bench -23%: serve pins the engine thread to rank 0's socket, so its control never sits on amd-bench's unpinned slow level. The run credits the pair, not each knob; the prefill-tick gain fits the collective change. The prefix workload got 0 turn-2 hits in every arm: the 0.05 retention cap (~9.6 GiB/rank) holds ~2.8 x 64K prefixes of replicated KV, so later publishes evict them before turn 2 (under investigation). |
| 53 | 2026-09-12 | c30ca7bc, 160c6b2e (branch `glm-wuv-fold-r2`) | perf (opt-in) | `PLOW_GLM_FOLD_LT` (prefill), `PLOW_GLM_FOLD_LT_DECODE` (decode rungs 16/20), `exec/amd_mla_fold.rs`, `scripts/build_glm53_gfx942_serving_objects.sh` | **The W_uv fold (`MlaMergeFold` at nsplit 1, a `[T,512] x [512,256]` GEMM per head that the interpreter runs one row at a time on the VALU) as the native FP32 hipBLASLt route.** Prefill tier 3 (job `0-1789190410-wuv-fold-l4b`, L4 TP8 packet, one binary, one object set, ctrl / fold / ctrl2 / fold2 through the production mux): steady 8192 chunk 50.70 -> 49.67 ms (-1.03 ms, 5x either arm's spread), TTFT at 73,728 tokens -9.2 ms; -0.26 ms/layer, ~ -20 ms per full 78-layer chunk. That is below the -31 ms estimate: normalize + GEMM + convert plus one more segment boundary leave the fold at ~290 us/layer in flow against ~545 on the interpreter. Fold route on all 8 ranks in the fold arms only; decode ticks unchanged. The serving-object script now builds the fold objects when the packet was emitted with the knob (before, a fold packet could not be served from a regenerated set). Decode: new `PLOW_GLM_FOLD_LT_DECODE` (rungs 16/20, nsplit 8/4) makes the isolated fold a raw `MlaFold` decode segment; **retired** (branch `glm-wuv-decode-retire`): tier 2 (job `0-1789201138-wuv-fold-decode-t2b`, 1 GPU, 78 cold W_uv layers, interpreter arm = the production `d_mla_merge_fold<512,256>` body at the rung geometry) measured it a loss. At rung 20 / 4 splits the native fold costs 41.2-41.7 us/layer against 16.8 us for the interpreter (12.5 us body after its 4.3 us dispatch floor): +29 us/layer = +2.2-2.3 ms/step; rung 20 / 16 splits +2.5-2.7 ms/step; rung 16 / 8 splits +2.2 ms/step; rel-L2 vs FP64 identical on both paths (1.65e-3). Why: the decode trace's ~97 us/packet for this op is gate wait on the flash partials, not fold body, so the estimated -6 ms/step never existed; the native route instead pays three dispatches (>= 13 us of floor), a small-M FP32 GEMM, and in flow one more segment boundary per layer. Emitter arm, `MlaFold` decode route, tests and tier-2 harness removed; 160c6b2e is the resurrection point. Tier 1 for both knobs: knob off byte-identical to the control packet; knob on changes only the 78 folds of each affected program plus one segment per layer, pairing hash unchanged; 0.03% (decode 0.025%) of bf16 outputs move by 1 ulp on the real W_uv, rel-L2 vs FP64 unchanged. Not bit-identical, so a default needs a served A/B plus retrieval (tier 4, batched with a freshly built object set). | open (opt-in) |
| 54 | 2026-09-12 | 908efbdb, 9d2f9c3b, 671db1d8 (branch `glm-sp-seams-r2`) | perf (opt-in) | `PLOW_GLM_SEQ_PAR`, `PLOW_GLM_SEQ_PAR_PROJ`; `devgen/src/mla.rs` GLM prefill seams | **Sequence-parallel TP seams for the GLM 2048/8192 prefill buckets** (Kimi-K3's `K3Tp` pattern, the existing `XReduceScatter` / `XAllGather` ops and objects). `SEQ_PAR`: every o_proj / FFN two-shot becomes a reduce-scatter; the residual and the norm after it run on the rank's t/8 band; the normed rows are all-gathered from peer slot 3. `_PROJ` adds: q_a (+ norm), kv_a, k_rope, the indexer k/weights and the router score/top-k run on the band, and the entry gather carries 2624 of 6144 columns. Both knobs off: the production packet is byte-identical (b7566b8f, stamp 0xa91a3e9d10deeba9). Tier 3 (job `0-1789190411-sp-seams-t3-p3`, `--layers 8` TP8, one binary, one stamped object set per packet, sweep medians of 14 reps over two interleaved rounds): steady chunk 2 (8192 rows at prior 8192) 86.76 / 86.43 / 79.79 ms (ctrl / SEQ_PAR / +PROJ) = -0.04 ms/layer (inside noise) and **-0.87 ms/layer, about -68 ms per full 78-layer chunk**; chunk 1 -0.22 / -1.05 ms/layer. Trace: the split collectives cost +0.35 ms over the two-shot per 16 seams, and the band glue saves about 2 ms off the critical path, so SEQ_PAR alone does not pay. Band GEMMs at 1024 rows: q_a 131 us + kv_a 229 us per layer against 560 us for hipBLASLt q_a+kv_a at 8192 rows; k_rope, the indexer and the router barely gain (the narrow-N tiles are latency-bound). Next step: a kv_a band tile, about -12 ms/chunk. Numerics, judged against in-job two-process floors (see #50): every arm is inside its floor with the same argmax (dense-only act.xn mean abs diff 2.0e-3 vs a 2.1e-3 floor; past MoE, logits 6.7e-4 vs 2.8e-3). Bit identity is untestable: layers 0-2 already differ across processes. Ragged tails move full-bucket collectives (about +5 s per 100 prompts). Tier 4 for `_PROJ` is batched with the W_uv fold (#53) on one packet. | open (opt-in; tier 4 batched with #53) |
| 55 | 2026-09-12 | 908efbdb (branch `glm-sp-seams-r2`) | should-fix (latent) | `plowrt/src/exec/kvrow.rs` `rebase_chunk_rows` | **The ragged row shrink rescaled sequence-parallel band packets.** A band `Residual` (`<base>@band<T>` output) carries `(T/tp)*H` elements, which is a multiple of T, so under the default-on `PLOW_RAGGED_CHUNK` it was rescaled to `(clen/tp)*H` and dropped live rows of the rank's fixed band. That was latent for K3's default-on `PLOW_SEQ_PAR_SEAMS` and live for the GLM seams. The shrink now skips packets whose output is a band view; test `ragged_rows_leave_sequence_parallel_band_packets_alone`. | fixed 908efbdb |
| 56 | 2026-09-12 | branch `hsa-kernarg-vram-r2` | perf (opt-in) | `PLOW_AMD_KERNARG_VRAM`; `device/hsa.rs` (`vram_kernarg_ring`, `write_kernarg`, `publish_device_kernarg`) | **Kernarg ring in the GPU's own VRAM.** With the kernarg ring in host memory the GPU reads each dispatch's kernargs from lines the host writer thread holds dirty in its socket's cache, so the GPUs on the other socket pay a cross-socket snoop per dispatch (the +77 µs per-window rank 4–7 lag left under `PLOW_AMD_NUMA_HOST_POOLS` with the writer pinned to socket 0). The ring now lives in VRAM behind the large BAR (prefaulted every 64 KiB, host-kernarg fallback when the BAR cannot map it); each slot is staged on the stack, copied through the BAR and published with CLR's `DeviceKernelArgsReadback` sequence, the gfx94x default (ROCm/clr `rocclr/device/rocm/rocvirtual.cpp` `VirtualGPU::submitKernelInternal`, `rocsettings.cpp`): sfence, rewrite the last kernarg byte, mfence, volatile read of it, before the header's release store; a readback that disagrees re-copies the slot and poisons the backend if it still disagrees, so a stale slot is never published. No existing fence changed. Tier 3 (job `0-1789190413-kernarg-vram-L7`, 7-layer TP8, pinned to socket 0, 6 processes per arm interleaved): pinned control 10.910, NUMA pools 9.988, **NUMA + VRAM 7.062 ms/step** (spreads 0.31 / 0.32 / 0.020 ms); rank 4–7 minus 0–3 gap 105 / 77 / 0.6 µs; one-shot XReduce time ranks 0–3 189 → 164 → 29 µs; mean window between collectives 579 → 530 → 409 µs; all 8 ranks token-identical in every process, and cross-process greedy-token agreement vram-vs-base 0.678 against base-vs-base 0.655 (the #50 floor). Projected numa → vram: −121 µs per window × 156 ≈ −19 ms per 78-layer rung-20 tick (raw; the pinned NUMA share projected −7.7 ms and served −3.1 ms decode-only p50 in the combined tier 4, #52), plus ≈ −6..−13 ms per 8192 prefill chunk from the lone-GEMM interval (−15 / −33 µs on ranks 0 / 4, 781 launches). | default on (#57) |
| 57 | 2026-09-12 | branch `kernarg-vram-default-flip` | perf (default flip) | `PLOW_AMD_KERNARG_VRAM` (`RuntimeConfig::amd_kernarg_vram`) | **The served A/B qualifies VRAM kernargs as a default on their own.** Job `0-1789202824-kernarg-vram-t4`: one binary, runtime-only arm (`PLOW_AMD_KERNARG_VRAM=1`) on the 08c0ad44 checkpoint set, 20 × 70k/700 at C20, ctrl → treat → ctrl2. Out tok/s 63.08 / 70.18 / 63.37 (+11.0 % against the control mean, controls 0.46 % apart); decode-only tick p50 88.0 / 62.9 / 85.1 ms (−27.3 %, controls 3.3 % apart), p90 89.5 / 63.1 / 85.9 ms (−28.0 %); median ITL 88.5 / 63.1 / 85.5 ms; prefill tick p50 855.3 / 834.5 / 853.9 ms (−2.4 %, controls 0.2 % apart); full 8192 chunk mean 793.3 / 792.7 / 799.3 ms (−0.4 %, inside the 0.8 % control spread), so the prefill tick's gain is the decode dispatched in it (72.5 / 54.2 / 70.4 ms, −24.2 %); retrieval 18/18 + 21/21 in every arm, 0 server errors, all 99 tail chunks per arm in bucket 8192. Only the kernarg memory changes, so the output is bit-identical by construction. Default on (`Option<bool>` + `RuntimeConfig::amd_kernarg_vram`, unset → on, the #52 NUMA pattern); `=0` / `--amd-kernarg-vram=false` = the host kernarg ring (rollback); a refused VRAM allocation or BAR mapping still falls back to the host kernarg pool with a warning. | fixed (default on) |
| 58 | 2026-09-12 | branch `prefix-attach-par` | perf (default behaviour) | `memory/vmm.rs` `VmmKv::try_attach`; `exec/amd/shared_prefix.rs` `attach_ranks`; PFATTACH `kept=` | **A full-model TP shared-prefix hit spent ~2.4 s of engine-thread time, nearly all of it unmapping and re-mapping blocks that were already in place.** PFATTACH split (job `serving8k-t3a`, checkpoint set 08c0ad44, C2 multi-turn, 65,504-row hits, one binary per arm): the review head 8dbf1c40 spends cursor 2.42 s = `clear` 0.23 (`begin_slot`) + `stage` 2.14 + `commit` 0.06; the stage unmapped the slot's stale blocks (the previous occupant's, kept mapped by deferred reclaim) and re-mapped the prefix, for all 8 ranks in series (~268 ms each), and every decoding stream stalled for it (that tick: 2.8 s). With the fix, an attach keeps a prefix-column slot that already maps the block it would map there (turn 2 in turn 1's slot, whose stale columns ARE the published blocks: no unmap, no map, refs unchanged; atomic against the reclaimer because `reclaim_stale` unmaps only a slot it finds `Stale` under the pool lock) and leaves stale columns past the prefix to the reclaimer under `retire_window`'s invariant: `stage` 207 / 222 ms, `commit` 197 / 189 ms, cursor 0.64 / 0.63 s; turn-2 TTFT 2.79 / 2.76 s → 0.97 / 0.97 s; the stalled tick 2.8 → 0.97 s. The map-failure unwind now unmaps exactly what is still mapped, under the lock. Correctness: the serving guard on the same set with the fixed binary PASS (retrieval 39/39 base + tail, C2 hits 2/2 at 0.84 s turn-2 TTFT, shared_prefix on 8/8 ranks, tail-sparse active, 0 refusals) with 4,078 blocks kept in place over 14 hits (PFATTACH `kept=`); mock-VMM tests on a strict per-VA mock (keep + reclaimer pass; keep against a live reclaimer thread, 200 rounds; failed-map unwind; stale-count bound over 50 rounds). Staging the 8 ranks on 8 threads was measured and removed: each rank's stage stretched to 126-223 ms, wall 218-223 ms = the serial sum, because ROCr serializes every `hsa_amd_vmem_*` call (as `ENGINE_SECTIONS` in vmm.rs records). | open — merge; next target is the remaining ~0.4 s per 64K hit: `stage` ~210 ms plus `commit` ~190 ms. `commit` rose from 56 ms with the fix; a guess, not measured: it now contends with the reclaimer draining the deferred stale columns, since ROCr serializes both. `clear` (`begin_slot`) is another ~0.23 s. |
| 59 | 2026-09-12 | branch `seqpar-fold-default-flip` | perf (default flip, pair) | `PLOW_GLM_SEQ_PAR`, `PLOW_GLM_SEQ_PAR_PROJ`, `PLOW_GLM_FOLD_LT` (`EmitConfig` recipe knobs, `apply_production_defaults`) | **The seams (`_PROJ`) and the W_uv prefill fold are production defaults for GLM on gfx942 TP8.** Served tier 4 as a pair (job `0-1789204585-sp-proj-fold-t4`, one plowrt from 008dda69, ctrl / treat / ctrl2, 20 × 70k/700 at C20): out tok/s 63.71 / 68.13 / 63.96 (+6.73% vs the control mean, controls 0.39% apart), full 8192 chunk 792.3 / 703.4 / 792.7 ms (−11.2%), prefill tick −11.0%, decode unchanged, retrieval 18/18 + 21/21 in every arm. HBM: ~0.95 GiB/rank (fold ~0.5 + seams peer region ~0.3): the VRAM floor at C20 × 64K goes 4.50 → 3.55 GiB free on the tightest card. Mechanism as the nine recipe knobs (`Option<bool>`, recorded as `production_default`, rollback per knob `--glm-...=false`, flag / env / replay win). `SEQ_PAR` stands aside for `PLOW_GLM_XR_RES` / `_XR_BAND`; `_PROJ`'s default follows `SEQ_PAR`; `apply_production_defaults` now records each knob's resolved value. The frozen serving recipe replays `PLOW_GLM_FOLD_LT=false`, so a regeneration must drop that entry to take the fold default; the serving-object script then builds the fold ELFs (vendor dir must hold `glm_fold_lt_gfx942.elf`) and the seams arm (from `plow_config.h`). | fixed (default on) |
| 60 | 2026-09-12 | 1378af31 (checkpoint 2, PR #34) | measurement | the user's 100-prompt benchmark on `/workspace/plow-serving/glm53-tp8-1378af31` | **75.73 out tok/s on 100 x 70k/700/.14 at C20, every default as shipped** (job `1789209486-bench100-1378af31`): 939.5 s, 100/100 requests, median TTFT 13.0 s, median ITL 62.9 ms, P99 ITL 945 ms. That is +13.8% on the tail-sparse run (66.56, #36) and +17% on the re-baseline (64.3-65.1). Defaults in the set: tail-sparse, the capped prefill all-reduce, NUMA host pools, kernarg rings in VRAM, and the prefix-attach keep path. Not yet included: the seams _PROJ + W_uv prefill fold defaults (#59, +6.7% on the 20-prompt A/B), which go to checkpoint 3. |
| 61 | 2026-09-12 | 90a1b438 (checkpoint 3, PR #35) | measurement | the user's 100-prompt benchmark on `/workspace/plow-serving/glm53-tp8-90a1b438` | **82.08 out tok/s on 100 x 70k/700/.14 at C20** (job `1789211100-bench100-90a1b438`): 866.8 s, 100/100 requests, median TTFT 10.8 s, median ITL 62.9 ms. That is -72.7 s against checkpoint 2 (#60, 939.5 s), matching the #59 projection (807 full chunks x 89 ms). TICK split: prefill work 613.1 s (70.7%), decode pass inside prefill ticks 63.9 s, decode-only ticks 188.9 s (3,046 at ~62 ms), host other + idle 0.6 s. Scheduling is not where time goes; the remaining gap to 100 tok/s (~173 s) is inside prefill programs first. |
| 62 | 2026-09-12 | 90a1b438 (checkpoint 3) | measurement (attribution) | the 100-prompt run (#61) plus one all-rank PLOW_TRACE_RAW trace of a 65K-prior 8192 chunk, a 4096 chunk and a rung-20 decode step (job `0-1789212148-attrib-trace`) | **Plow loses time on GPU kernels, not scheduling.** Of 866.8 s: vendor native kernels ~430 s (49.6%: AITER sparse MLA prefill 172 s, AITER MoE 91 s, TP indexer 52 s, hipBLASLt), plow interpreter kernels ~255 s (29.4%), collectives ~125 s (14.4%), launch/drain ~44 s (5.1%), host ~12 s (1.4%; idle and gaps 0.8 s). An 8192 chunk (715 ms GPU span) has a fixed ~258 ms: full-bucket seam collectives 112 ms (compiler structure), latency-bound interpreter tiles ~78 ms, vendor floors ~58 ms, boundaries ~7-10 ms. Decode (58.9 ms per rung-20 step) is a per-layer op-latency chain: rungs 1-8 all ~53 ms; IndexSelect 3.7 ms on 20 workgroups; XReduce transfer is 21.8 us, no longer a decode cost. Every rung-20 decode GEMM is already hipBLASLt. Prefill interpreter GEMMs (o_proj, shared gate+up/down, router, q_rope, the seams band GEMMs at M=1024) are 18.7% of a chunk at 2-27% of peak; moving them to hipBLASLt nets about -54 ms/chunk (~-47 s/run). Top levers to 100 tok/s: prefill GEMMs to hipBLASLt (-42..47 s), link-rate collectives and row-aware seams on tails (-35..45 s), the decode latency chain (-25..40 s). Report: tmp/reports/attribution.md (scratch). |
| 63 | 2026-09-12 | 4c10b57e (tb-seams-body), reverted in 0269505e | correctness (opt-in route) + process | token-batch bodies with the production seams (`PLOW_TOKEN_BATCH_TP` + `PLOW_PACKED_SPARSE_PF` on the 0xa6e7db0b3b83e581 recipe) | **Seams-aware bodies corrupt long-context prefill; reverted.** Tier 4 (`1789218712-tb-t6-t4`): the treat arm fails retrieval, base 9/18 and tail 0/21, against 18/18 and 21/21 on the control. Every prompt whose middle chunks ride an 8192 body returns word salad from the first token (0/30), while single-chunk prompts pass (9/9). Throughput looked normal (75.62 vs 76.57 tok/s), and there were 0 rank disagreements. The body without seams (tb-t4) passed retrieval on the same route, so the regression is in 33f11171/36f04b75. Bodies are opt-in, so the production default and checkpoints #33-#35 never carried it. **Process gap:** tier 3 passed (8-layer timing resolved, chain_check2 clean) because its gates checked positions and rank agreement, not values. New standing gate for any body or packing change before tier 4: a value-level equivalence stage at tier 3, with the same prompt through the body route and the isolated route and the written KV rows and final logits compared against the cross-process floor (#50). Diagnosis (dependency-aware check, KV dumps) in progress. **Root cause (confirmed, kernel and packet):** in a body the runtime routes pure RmsNorm/HeadNormRope segments to the `_tb` norm twin (derive_packed_segment_families, exec/amd.rs:3134). The twin sizes packets with plow_tb_rows(i0), which returns the decode-span count ndec for ndec <= i0 < capacity, so each of the ~235 seams band RmsNorms per 8192 tick (i0 = T/8 = 1024) normalized only 1-20 rows. Riding chunks' projections, and the KV they write, were then garbage. The CPU dependency check was clean; the fix (band norms stay on the primary object, loader refuses family-5 norms with i0 neither T nor band) is kept on branch tb-seams-body, unmerged. **Decision: bodies parked.** tb-t4 (no seams) -6.6%, tb-t6 (seams, corrupt) -0.9% tok/s with middle tick 756.6 vs 742.3 ms; a correct seams body adds ~2 ms of band-norm work (~758 ms), about +16 ms per middle chunk. The deficit is structural: 1314 vs 625 segments plus the band chain. Truncated-depth pricing projected +21.8 ms at 78 layers and does not extrapolate for bodies. Tier-4 scripts for body or packing work now exit non-zero on a retrieval regression. **Guard merged (face5830, from e99c7b82):** devgen test production_programs_read_only_rows_every_rank_holds emits the production recipe without bodies (buckets with tails, every decode rung; seams, _PROJ, fold) and fails if any program reads a tensor only one rank's rows hold; IndexTpPf's peer slot is modelled write-first. The fix for the body norm twin stays on tb-seams-body @ add056bd. |
| 64 | 2026-09-12 | 7acc3c1f, 1720830e (branch `seams-ragged`) | perf (opt-in) | `PLOW_AMD_RAGGED_SEAMS`; `exec/kvrow.rs` (`rebase_band_rows`, `ragged_band_views`), `exec/amd.rs` `rebind_band_views` | **Ragged chunks size the sequence-parallel seams by live rows.** A chunk with clen < T live rows gets bands of b = ceil(clen/8) rows. The reduce-scatter / all-gather counts go from T to 8b rows, and band packets from T/8 rows to b. The @band views are rebound per chunk in the tensor-pointer table, so there is no packet or object change. Runtime knob, off by default. Tier 3 (job `0-1789221475-seams-ragged-t3`, --layers 8 TP8, seams + _PROJ + fold defaults, one packet / object set / plowrt, ctrl vs treat by env only). Timing at prior 57344, medians of 10: 2500-row tail 45.75 -> 37.74 ms (-17.5%), 4073-row tail 56.56 -> 50.76 ms (-10.2%), full 8192 chunk unchanged (+0.2%, noise). A 1024-row tail runs in the dense 2048 bucket: -0.8%. Both tail points give a full-bucket seam cost of ~1.44 ms/layer, i.e. about 112 ms per 78-layer chunk (matching the attribution). So a tail saves about 112 x (1 - clen/8192) ms: ~56 ms at the 4073-row mean, ~5.6 s over a 100-prompt run. Tail-chunk trace: reduce-scatter 549/454 -> 303/171 us, all-gather 373 -> 134 us, band residual 457 -> 181 us. Value equivalence (59844-token prompt, 2500-row tail): KV rows of layers 1/3 (pre-MoE) and 5/7 (post-MoE), the tail act.xn rows and the logits all sit inside the ctrl-vs-ctrl-2 floor (#50), including the tail rows alone; same argmax. PLOW_DUMP_ACT gains name:path:bytes (dump leading bytes: the KV rows a prompt wrote, without reading unmapped ring slots). | open (opt-in) |
| 65 | 2026-09-12 | 4ac4ff41, b293d5a3, 03e5de32, 5b412489, fbc2e299 (branch `prefill-gemm-lt`, merged 503633c5) | perf (opt-in) | `PLOW_GLM_GEMM_LT_PF_EXT` / `--glm-gemm-lt-pf-ext` (comma list of o_proj, shared, router, band; `1` = all four); `runtime/amd/glm_lt_pf_ext_gfx942.json`; loader GemmLt checks; hidden `PLOW_DUMP_KV` | **Prefill interpreter GEMMs to hipBLASLt by group.** Tier 3, own objects, arms interleaved (0-1789219022, 0-1789221845). Steady 8192 chunk at 8 layers against off (78.76 ms): all -3.29, o_proj+band -2.53, o_proj+band+shared -2.86 ms, about -29 ms per 78-layer chunk and -25 s/run. Strict value gate (each metric <= 1.25x its cross-process floor): for o_proj+band+shared, logits 1.04x, top-8 expert flips 17.05% against a 16.97% floor, last-layer KV 1.10x. Layer-0 KV differs only by rounding: its floor is 0, and the band route changes 4.9% of fp8 latent bytes (rel 3.6e-4). The router group is rejected: it put 8-layer logits at 5.4x the floor in both jobs. Segments per 8192 chunk go from 625 to 1357, about 15 us per added boundary. With the knob off, the 78-layer emit is byte-identical to production. Split-K for the narrow band shapes is deferred, projected -10.7 ms/chunk. | open (opt-in); combined tier 4 pending |
| 66 | 2026-09-12 | none | process | packet pairing hash, GPU queue runner, decode ops | **Three process findings.** (1) The pairing hash does not cover segmentation. A 4-layer emit without build3.sh's emit-time env (PLOW_MLA_PF_V2 and friends, which `--replay-knobs` does not carry) put FlashMlaPrefill into mixed segments yet kept the same hash, so objects built against another emit's plow_config.h still pass the stamp check (the #44 class). Every emit exports that env, and tier-4 preflight now checks that each FlashMlaPrefill segment is pure. (2) The queue runner reaps only its direct child. A job script killed by PID left plowrt on all 8 GPUs for about 25 s into the next job, because GNU `timeout` moves its child into a new process group. Stop a queue job by session (`pkill -TERM -s <sid>`). A runner-side session reap is written but not applied. (3) No decode op may spin-wait across workgroups. The split IndexSelect's in-op grid sync hung the rung-20 decode intermittently, under both the global queue and static streams; ordering across workgroups goes through interpreter gates only. | open: hash coverage follow-up; runner change pending the user |
| 67 | 2026-09-12 | branch `decode-gemm-group-r2` | perf (opt-in) | `PLOW_GLM_DECODE_GEMM_GROUP`; `devgen/src/mla.rs` (`glm_decode_lt_routes`, `emit_glm_mla`, `emit_glm_dsa_decode_select`, `emit_glm_moe_ffn_rows`) | **Same-input native decode GEMMs emitted adjacent.** devbuild starts a segment on every native/interpreter change in emission order, and at the native decode rungs the shared gate/up sat behind the router top-k, and the indexer's k/weights/q projections each sat between interpreter slivers. The knob emits gate/up straight after the router GEMM (top-k and Glu then share one interpreter segment), and on full-indexer layers idx k/w beside q_a/kv_a/k_rope and idx q_b beside q_absorb/q_rope. Same instructions and dependencies, reordered, so objects and pairing are unchanged and the knob-off packet is byte-identical. Rung-20 chain 1431 → 1293 packets: −3 segments per indexer layer, −1 per MoE layer. Tier 3 `0-1789217373-decode-grp-L7` (7-layer TP8 on the NUMA + VRAM-kernarg defaults, 6 processes per arm interleaved, socket-0 pinned): 7.073 → 6.832 ms/step (−3.4 %, spreads 0.033 / 0.045); class-weighted 78-layer projection −1.75 ms/step ≈ −6.9 s/run. The first attempt (branch `decode-latency`, 9be42075) was swamped by per-process levels that those defaults have since removed. | open (opt-in; combined tier 4) |
| 68 | 2026-09-12 | 46ecd49d (merge of `decode-chain`) | process | review branch consolidation | **Every campaign branch is now on the review head, or preserved on origin.** Merged since checkpoint 3: band-read-guard (face5830), seams-ragged (39612eb8), prefill-gemm-lt (503633c5), decode-gemm-group-r2 (8fb46736), seams-coll-bw (79ca4cc3), decode-chain (46ecd49d). The last brings the opt-in split DSA select, its gated three-phase rewrite (`PLOW_GLM_SELECT_SPLIT`, ascending output order, no cross-workgroup spin) and the decode glue LayerNorm sizing (`PLOW_GLM_DECODE_GLUE_CUS`); both are unproven on GPU and off by default, so the default emit is unchanged. Checks on 46ecd49d: workspace and plowrt `cuda,hsa` all-targets clean, devgen 455, plowrt lib 884 passed / 54 ignored. Deliberately NOT merged: `tb-seams-body` (bodies parked, #63) and the older per-campaign branches whose content already reached the head by earlier merges or cherry-picks; both are pushed to origin for the record. In flight at the time of writing: the combined tier 4 (job 1789228962-combined-t4) of ragged seams + prefill hipBLASLt + decode GEMM grouping + the seam RS cap, treat packet 0x940fe7d6cf30b317. | open |
| 69 | 2026-09-12 | this commit (defaults), tier 4 job `1789228962-combined-t4` | perf (new defaults) | `PLOW_GLM_GEMM_LT_PF_EXT` (now `o_proj,band,shared`), `PLOW_GLM_DECODE_GEMM_GROUP` (now on), `PLOW_AMD_RAGGED_SEAMS` (now on); `PLOW_XR_SCHED_NWG_SRS=8` was already the build default | **Combined tier 4 qualifies all four levers; three become production defaults.** One treat arm carried the seam reduce-scatter cap, ragged seams, the prefill hipBLASLt groups and the decode GEMM grouping against the checkpoint-3 set, bracketed by two controls, one plowrt from 79ca4cc3. 20 x 70k/700 at C20: out tok/s 75.63 / 81.65 / 76.26, i.e. **+7.5%** against a control mean 0.8% apart. Full 8192 chunk 703.9 / 654.2 / 698.9 ms (-6.8%, -47 ms). Tail chunks 529.8 / 444.3 / 529.0 ms (-16.0%, -85 ms), which is ragged seams. Decode-only tick 62.7 / 60.8 / 62.8 ms (-3.2%), which is the GEMM grouping, against its -1.75 ms projection. Prefill tick p50 -6.4%. Retrieval 18/18 base + 21/21 tail in all three arms and the script's retrieval gate passed; VRAM floor unchanged at 3.55 GiB. Scaled over the run (807 full chunks, 100 tails, 3946 decode passes): about -54 s, i.e. ~87-88 tok/s from 82.08. The treat packet was 0x940fe7d6cf30b317; the new default emit reproduces it, and each knob keeps a per-knob rollback. Preflight also gained a check that every FlashMlaPrefill segment is pure (see #66). | open: re-emit the production serving set and re-baseline the 100-prompt run |
| 70 | 2026-09-12 | a7596da2 (serving set `/workspace/plow-serving/glm53-tp8-a7596da2`, packet 0x940fe7d6cf30b317) | baseline | 100-prompt benchmark + serving guard on the new defaults | **Re-baseline after the tier-4 defaults: 86.67 out tok/s.** Job `1789232148-bench100-a7596da2`, 100 x 70k/700/.14 at C20, every default as shipped: 820.93 s, 100/100 requests, median TTFT 9.547 s, median TPOT 209.1 ms, median ITL 61.03 ms, P99 ITL 810.8 ms. Against checkpoint 3 (82.08 tok/s, 866.8 s, TTFT 10.8 s, ITL 62.9 ms): **+5.6% tok/s, -45.9 s, TTFT -11.6%, ITL -3.0%**, which matches the tier-4 projection of about -54 s within the run-to-run spread. The serving set was rebuilt from a7596da2 by the frozen recipe and its DEFAULT emit reproduces the tier-4 treat packet exactly (0x940fe7d6cf30b317, 113 objects, manifest verified). Guard PASS (job `0-1789232148-gate-a7596da2`): retrieval 39/39 suite all, C2 prefix hit rate 1.0 with turn-2 TTFT 0.829 s, tail-sparse active by default, no refused programs, every rank numa_host_pools and kernarg_vram on. | closed |
| 71 | 2026-09-12 | f2c510ed (branch `a2a-collective-pricing`, merged 9ec05387) | design validated (no route change yet) | `runtime/tests/tp_alltoall_bench.c` + `tp_alltoall_kernels.hip`; reports/rowsplit-attention-design.md | **Query-row-split sparse attention is viable: both halves measured.** (1) Tier 2, 1 GPU (job `0-1789230550-rowsplit-t2c`): at equal work (65,536 row-head pairs) the row split runs 1024 rows x 64 heads as four 16-head launches in **1429.7 us/layer** against **2691.9 us** for today's 8192 rows x 8 heads, i.e. **1.88x**, because each rank then gathers 2.42 GB instead of 19.33 GB of keys; rel-L2 against a double-precision reference is 1.12e-3 vs today's 1.08e-3. The 64-head (m64x1) and 32-head (m32x1) objects fail to launch at one query per workgroup, and the 4/8-query qh16 objects share one KV selection across their queries, which sparse attention cannot use; the usable route is four 16-head launches of `mla_dec_stage1_bf16_a16w16_subQ16_mqa16`. (2) The two all-to-alls the layout needs (66 MB and 59 MB per rank) cost **553.1 us per layer together at a 48-workgroup cap, 225.6 GB/s**, byte-exact on every rank (job `0-1789234094-a2a-collective-bench3`); the harness reproduces ~233 GB/s on an unmodified all-gather, which is what makes the figure trustworthy. Two bugs had to be fixed first: the launch's grid argument is total threads, not workgroups, and the first kernel used 2-byte scalar remote loads (57 GB/s); copying `d_xall_gather_mega`'s one-wave-per-peer 16-byte pattern gave 226 GB/s. **Net: -98 ms attention, +43.1 ms collectives, -8..-12 ms for the selection gather the split makes unnecessary = -63 to -67 ms per 8192 chunk.** Not bit-identical (different attention object), so it needs the #63 value gate plus retrieval at tier 4. | open: implement the all-to-all op and the emitter arm |
| 72 | 2026-09-12 | none (scratch measurement; see reports/rung-sizing-facts.md) | finding (ladder) | the rung width ceiling: 11 independent row gates, plus the hipBLASLt band route's 256/1024 band rows | **A prefill rung wider than 8192 is buildable and runnable; its benefit is NOT established (the 3-4% below is a measurement artifact, see the correction at the end of this row).** Measured at 8 layers, DSA on, band hipBLASLt route off in BOTH arms so only the width differs (jobs `0-1789237126-rung-wide-t3j` for the 8192 ladder, `0-1789237785-rung-wideonly` for a packet whose only bucket above 2048 is 16384, so it cannot fall back): 16,384-token prefill 148.9 -> **142.6 ms** (-4.0%), 32,768 291.4 -> **280.5** (-3.7%), 65,536 592.3 -> **573.0** (-3.3%). That is ~5.5 ms per launch removed at 8 layers, scaling with launches saved, which is the fixed-cost model's prediction. Extrapolated at the same ratio, a 71k-token prompt's prefill would drop 3-4%, about -19..-23 s of the 820.9 s run, and TTFT per prompt by the same fraction. **What it costs to ship:** (1) the 8192 ceiling is enforced in ELEVEN independent places (asserts in mla.rs for the hipBLASLt fold, indexer TP prefill and AITER MoE; four `(2048..=8192)` route ranges; load gates in amd_index_tp.rs, amd_gemm_blk.rs, amd_sparse_mla.rs, amd_mla_fold.rs, amd_moe_aiter.rs, amd.rs's eligible-rows check and gpu.rs) - a real change replaces those literals with one constant; (2) the band sub-route asserts band rows in {256, 1024}, i.e. T in {2048, 8192} exactly, so a 16384 rung needs pinned hipBLASLt shapes at 2048 band rows or loses that route; (3) activation memory grows with T. Also: emitting a wide rung needs `PLOW_GLM_DSA=1 PLOW_GLM_DSA_PF=1` (a 4-layer truncation has no full-indexer layer, and without the DSA pair there is no sparse route at all - three probe attempts were wasted on this). **Ranked below the row split (-55..-59 ms/chunk) and decode packing (-57 s/run); parked with this finding rather than built.** **CORRECTION (same day):** the 8192-ladder arm (`0-1789237126-rung-wide-t3j`) ran with `PLOW_PREFILL_SEG_TIMING=1`, which silently disables `PLOW_TP_PREFILL_SEGMENT_MAJOR` and adds per-segment barriers; the wide-only arm did not set it. So the 3-4% compares two scheduling modes, not two widths. Like-for-like at 65,536 tokens, both without segment timing (`0-1789236705-rung-wide-t3i` cap 8192: 572.5 / 572.9 ms; wide-only: 574.0 / 571.6 ms), there is **no measurable difference**. The wider rung's value is unproven and needs a clean rerun at 16,384 and 32,768 tokens before any work on it is justified. **Clean re-run (job `0-1789240789-rung-wide-clean`, 8 layers, one packet carrying both rungs, SEG_TIMING off, two interleaved rounds x 3 reps):** cap 8192 vs cap 16384 = 77.79/77.75 ms at 8,192 tokens, 143.22/143.46 at 16,384, 280.95/280.21 at 32,768, 572.24/572.38 at 65,536, i.e. no difference outside 0.3% noise. The run printed no per-chunk lines, so it does not prove the 16384 arm launched fewer chunks; either way no benefit is established, and the rung stays parked. | open (parked; benefit unproven) |
| 73 | 2026-09-12 | this commit (harness: runtime/tests/mq_probe_gemmlt_bench.c, hsa_backend getter) | finding (scheduling) | multi-queue AQL: a second, barrier-free queue beside the collective | **A second AQL queue hides real GEMM work under a collective for free, but the serving win is 2-4%, not the ~110 s first sized.** Measured on 8x MI300X with the production `GemmLtPf` kernel at the prefill shape (8192x4096x2048) against a real collective, bit-exact in every arm: alone 198-209 us; one GEMM on queue 2 beside the ~675 us collective -> pair wall 886.3 -> 668.0 us (-24.6%), GEMM stretched to ~262 us by contention. Fill sweep (jobs `mq-gemmlt-pf-matrix`, `mq-gemmlt-pf-fill`): N=1 and N=2 copies add 0 wall (79% of the window covered), N=3 adds +135 us, N=4 +315 us, so ~670-680 us of independent GEMM-shaped work hides per collective. Sizing from the production build.json: one 8192 chunk has 678 `GemmLtPf` and 385 collectives (153 `XReduceScatter` + 232 `XAllGather`; `XReduceTwoShot` is no longer dispatched under the default seams, and the free window for those smaller ops is unmeasured). After saturation and DAG-independence discounts: **~20-40 ms/chunk, ~17-34 s/run.** Decode shape (M=20, 18 us) was 2.6-3.5% per pair and is not worth it. Shipping needs (1) devbuild.rs to keep edge-free, CU-disjoint segment pairs (`segment_resource::co_resident`) on a second queue instead of deleting cross-segment edges (~100-150 lines), and (2) a real second queue, kernarg ring and dual drain in hsa.rs (`queue_count()==1` today) with proof the queue-0 path pays nothing. Detail: reports/multi-queue-aql.md sections 5-7. | open (qualified go, sequenced behind the prefix cache) |
| 74 | 2026-09-12 | none (production serving set a7596da2, job `1789242180-bench-isl8k`) | measurement (reference) | ROCm ATOM's GLM-5.2 recipe table (ISL 8192 / OSL 1024) reproduced on plow with the same client flags (`--random-range-ratio 0.8 --request-rate inf --ignore-eos`) | **plow on 8x MI300X (TP8, FP8 weights, FP8 KV) vs ATOM's published 4x MI355X (TP4, MXFP4, FP8 KV, no MTP/DPA):** C1 (8 prompts): median TTFT **727 ms** vs 363 (mean 767, P99 1362), median TPOT **51.6 ms** vs 10.0, output 19.2 vs 97 tok/s. C16 (64 prompts): median TTFT **1236 ms** vs 411 (mean 2445, P99 14058), median TPOT **63.2 ms** vs 21.8 (ITL 50.1), output 210.5 vs 710 tok/s, total 1890 vs 6395 tok/s. Not same hardware or precision, so a direction, not a verdict: the prefill gap is ~2x at C1 and ~3x at C16 (prefills queue one launch at a time; the 14 s P99 is requests waiting behind other prefills), and **the decode gap is larger (5x at C1)**, which no prefill lever touches. A same-hardware vLLM ROCm baseline on GLM-5.3-FP8 is being brought up to replace ATOM's numbers as the reference. C64 not run: the serving set's slot count was not raised for it. | open (reference) |
| 75 | 2026-09-12 | none (reference; scratch tmp/vllm-baseline, job `1789243657-vllm-bench100`) | measurement (same-hardware baseline) | vLLM 0.28 ROCm serving GLM-5.3-FP8 on the same 8x MI300X: `-tp 8 --kv-cache-dtype fp8 --no-enable-prefix-caching --max-model-len 81920 --gpu-memory-utilization 0.9`, AITER on, INT4 quick-reduce off, DSA on (chunked prefill, max_num_batched_tokens 16384) | **The first same-hardware vLLM baseline: vLLM is 15% ahead on throughput, all of it in decode.** Same benchmark and client as plow's 100 x 70k/700 at C20: vLLM **708.5 s, 100.42 output tok/s** (total 10006), TTFT mean 20.5 s / median 9.70 s / P99 118.7 s, median TPOT 179.9 ms, **median ITL 29.2 ms**, P99 ITL 1556 ms. plow a7596da2 (FP8 KV, same day): 816.1 s, 87.18 tok/s (total 8687), TTFT 20.0 / 8.52 / 118.4 s, TPOT 211.0 ms, **ITL 61.0 ms**, P99 ITL 808 ms. TTFT is a tie (plow's median 12% lower); plow's tail ITL is half vLLM's; vLLM's decode step is ~2.1x shorter, which is the whole throughput gap. The recorded blocker ("vLLM cannot serve GLM-5.3 here, AITER's JIT needs lld") was a library-path bug: the JIT runs `<ROCM_PATH>/llvm/bin/ld.lld` as a child process that inherited the wrapper's nix-glibc LD_LIBRARY_PATH and died (rc 127, undefined `__tunable_is_initialized`); a shim ROCM_PATH whose ld.lld wrapper clears LD_LIBRARY_PATH fixes it. This replaces ATOM's MI355X table (#74) as the reference; the ISL-8192 table on vLLM follows. | open (reference) |
| 76 | 2026-09-12 | this commit | perf (decode, default flip) | `config.rs` `amd_decode_min_rung` (`PLOW_AMD_DECODE_MIN_RUNG`, default 8), `exec/amd.rs` `decode_prog_for` (TP engines only) | **A lone TP decode row now runs the rung-8 program, not rung 1: C1 TPOT −20%.** Rung 1 keeps a whole layer in one interpreter segment (151 segments, 1,808 launches/token), but its one-row dense projections run as GEMV over all 304 workgroups (~106 µs each); the rung-8 program runs native GEMM groups (20-34 µs) in 1,293 segments. amd-bench C1 at 8192 ctx: 51.4 → 41.0 ms/token (host enqueue 3.5 → 21.8 ms, GPU drain 45.7 → 16.5 ms). Served (job `1789245417-decode-c1-serve-ab`, ISL 8192/OSL 1024, C1, one binary, ctrl/floor/ctrl): median TPOT 51.85 / **41.44** / 52.27 ms, ITL 51.51 / 41.00 / 51.46, output 18.94 / **23.72** / 18.83 tok/s, TTFT unchanged. Equality, teacher-forced 128 steps (jobs `decode-c1-tf-r1`, `-tf-r8`; bf16 rank-0 vocab shard, row 0): r1 vs r8 relL2 p50 0.209, top-64 0.069, argmax 127/128, inside the same-code cross-process floors 0.206 (r1) / 0.238 (r8) (tracker #50's nondeterminism). A first read of the dumps looked like a 2.8x logit-scale gap; it was two layout artifacts (rung 8 fills parked rows 1-7, and the dump is bfloat16, not fp16), not a numeric difference. At rung 8 the step is host-bound, so parallel per-rank enqueue and a cheaper launch now compound with this. vLLM 0.28 on the same box decodes C1 at ~17 ms (5 requests). Single-GPU engines keep the plain ladder; `=1` restores it. | landed |
| 77 | 2026-09-12 | none (scratch patches parked: tmp/par-enqueue/par-enqueue-pool-3e914d9a.patch, tmp/launch-cost/kernarg-cache.patch) | finding (decode host path, measured null) | `amd_tp.rs` `submit_decode_batched_at` (per-rank enqueue pool), `device/hsa.rs` `HsaBackend::dispatch` (kernarg image cache) | **C1 decode is GPU-bound: making the host side cheaper does not shorten the step.** At one live row and 8192 ctx the rung-8 program showed enqueue 21.8 ms + drain 16.5 ms, which read as host starvation. Two independent host cuts, each teacher-forced and equality-checked (bf16 shard-aware `tfcmp.py`, inside the same-code floor), falsified that: (1) a persistent pinned per-rank enqueue pool cut enqueue 21.96 → 2.48 ms but drain rose 16.39 → 36.93 ms, total 41.00 → 41.04 ms (r1: 51.48 → 51.45); (2) a VRAM kernarg image cache cut per-launch host cost 2.0 → 0.5 µs (enqueue 21.97 → 5.44 ms) with enqueue+drain flat to 13 µs and total 41.01 → 40.99 ms (r1 got 6 ms worse on one run, unresolved). The launches were already overlapping GPU work; the rung-8 program's real GPU time is ~38 ms/token, not 16.5. A `PLOW_TP_AGREE_EVERY=16` arm saved 4 µs (readback costs ≤77 µs). Both patches parked; they would matter only on a genuinely host-bound path. The per-token audit (the one host wait after the GPU finishes) is the exception: a collective-only audit table measured −0.8 ms (landing separately once the late-rank fault proof passes). | parked |
| 78 | 2026-09-12 | none (scratch patch parked: tmp/xrn-fuse/xrn-fuse.patch, knob-off byte-identical) | finding (decode GPU path, measured null) | `GLM_FUSE_XRN` extended to every decode rung and every seam (XReduce + residual add + RMSNorm in one pass per row) | **Fusing the decode all-reduce with its residual add and norm does not speed up the step at full depth.** Each decode step has 153 XReduce → AddNorm seams (~55-65 µs per pair in the trace). Tier 3 (8 layers, 3-4 cycles) showed −31 µs (r1), −95 µs (r8), −98 µs (r20) per step, which scaled to a predicted ~−1 ms at full depth. Full depth (jobs `xrn-full-r8`, `-r20`, 2 cycles per arm, teacher-forced): r8 ctrl 41.74 / 41.58 vs fused 41.79 / 42.90 ms/step; r20 ctrl 51.16 / 51.71 vs fused 57.60 / 51.84. The fused arm was slower in every pairing, and slow-level runs appeared only with it (4 of 19 fused runs vs 0 of 15 controls across all tiers). Logits stay inside the control-vs-control floor. Likely reason (not measured): the seam's rendezvous and arrival skew, not the packet boundary, is what sits on the critical path. Lesson: 8-layer decode deltas for per-seam changes do not predict full depth; confirm at full depth before trusting them. The older row-1-only `GLM_FUSE_XRN` stays default off. | parked |
| 79 | 2026-09-12 | this commit | perf (decode host, safety-neutral) | `exec/amd.rs` `xaudit_insts`, `enqueue_xaudit` | **The per-token TP safety audit scans only the program's collective instructions.** The audit kernel (`plow_xctr_audit`) derives each gate's expected count from collective opcodes and ignores every other op, but was handed the whole program (1,644 instructions at rung 1, 2,034 at rung 8) for each of 474 gates, and that GPU scan (~1.0 ms) was the one host wait after the GPU finishes a token. A load-time table of just the collectives (opcodes 24, 25, 26, 28, 29, 116; unit test over all 65,536 opcode values) cuts the audit from 862 / 1,051 µs to 132 / 131 µs and the per-token total by 0.76 ms (r1) / 0.86 ms (r8), same process, variants cycled every 8 tokens (job `ar-cyc`). Same kernel, same post-drain placement, same gates and status word, so the verdict cannot change. Fault injection (job `ar-fi2`, r8, audit modes cycled): stale last-segment gate, stale mid gate, and a rank's collective moved to an unused gate were caught identically by the full scan and the narrow table; a collective bail's `0xDEAD|rank` status word, written by the collective itself, is read through the same path. A 1.5 s late-rank injection never bailed because the measured collective deadline is ~10 s, not the 1 s the `amd_tp.rs`/`tp.rs` comments claim. Also measured, not landed: folding the audit behind the last segment and a pinned readback save no more than narrow at C1; decode counter rearm is off the critical path (a 2 ms sleep there costs ≤0.14 ms); host-direct xctr clears qualified on hardware (the device sees host BAR stores) but save only ~90 µs. | landed |
| 80 | 2026-09-12 | c8497246 | perf (prefill host, safety-positive) | `exec/amd_tp.rs` prefill audit, `check_xstate`; `config.rs` `PLOW_TP_PREFILL_AUDIT_PINNED` (default on) | **A prefill chunk's TP counter audit is one pinned copy per rank instead of ~474 BAR reads.** The prefill audit ran after the GPU drained and read every gate through the BAR, 4.33 ms per chunk on 8 ranks. Copying the gates plus the status word into a per-rank pinned buffer and comparing on the host against the same expectation table costs 0.12 ms. Prefill sweep on GLM-5.3 TP8, BAR vs pinned alternated per chunk in one process (job `ar-pfsweep2`, 2-4 chunks per cell, spread ±0.5 ms): chunk total 99.5 → **95.7 ms** at 128 rows, 126.7 → **122.5 ms** at 512 (−3.8 / −4.2 ms). It also checks the status word, so a deadline bail whose counts still completed is now caught; the BAR gate audit alone could not see it (unit test `xstate_readback_is_an_exact_gate_and_status_check` covers exact, under and over counts and a `0xdead0005` bail). An injected stale prefill gate (241 reads 16) was caught through this path. Double-buffered prefill rearm moved the chunk ~0.2 ms, inside noise, and was not landed. `=0` restores the BAR audit. The landing binary itself has not run on the GPU; the timings are from the scratch binary with the same check minus one ~10 µs status read. | landed |
| 81 | 2026-09-12 | none (reference; job `1789245519-vllm-isl8k-c1c16`, scratch tmp/vllm-baseline) | measurement (same-hardware baseline) | vLLM 0.28 ROCm on GLM-5.3-FP8, 8x MI300X, TP8, FP8 KV, `--logprobs-mode processed_logprobs` (AITER's top-p sampler faulted intermittently) | **Same-hardware ISL 8192 / OSL 1024 table, vLLM vs plow (tracker #74's settings).** vLLM C1 (8 prompts, 0 failed): TTFT median **449 ms** (mean 510, P99 925), TPOT **17.2 ms**, ITL P99 17.7, 56.8 output tok/s. vLLM C16 (64 prompts, 0 failed): TTFT median **889 ms** (mean 1837, P99 9539), TPOT 34.8, ITL 26.2 (P99 361), 395 tok/s. plow a7596da2: C1 TTFT 727 / TPOT 51.6 ms / 19.2 tok/s; C16 TTFT 1236 (P99 14058) / TPOT 63.2 / ITL 50.1 / 210.5 tok/s. So vLLM is 1.6x faster to first token at C1 and 1.4x at C16, 3.0x on C1 decode and ~1.9x on C16 ITL. Since then plow landed the C1 rung floor (#76, TPOT 51.9 → 41.4) and the narrow audit (#79, −0.8 ms), and a single-row decode program measured 33.65 ms/token at full depth (one run, pending repeats). The chat-template smoke answered 'Paris', clearing the earlier garbage-output caveat on vLLM's numbers (#75); the C64 row is still queued. | open (reference) |
| 82 | 2026-09-13 | this commit | perf (prefill TTFT, default flip) | `config.rs` `amd_tail_sparse_ctx` default 16384 → 8192 | **A dense final chunk at prior ≥ 8192 now runs on the sparse 8192 bucket (was ≥ 16384).** Prompts of 8,193-16,384 tokens ended in a dense 2048-program tail whose attention over 8,192 prior rows cost ~150 ms of fixed drain; the sparse bucket's fixed cost at that prior is ~10 ms. Prefill sweep, one serving binary, ctrl/floor/ctrl (job `rung2048-tail-ab`, 3 reps): tail at prior 8192 of 600/1200/1854/2048 rows 285.5/319.8/359.9/369.5 → **145.1/181.4/226.7/232.9 ms** (controls within 0.6 ms); fresh prompts identical; whole-request TTFT for 8,792-10,240-token prompts 1006-1094 → 866-958 ms. Quality: retrieval guard at floor 8192 with every tail placed at prior 8192 through the sparse path (`quality-tailprior.py`, job `rung2048-guard-tail8192`) passed 39/39, prefix C2 hit rate 1.0; the dense control (`rung2048-guard-tail16384`) ran alongside. The sparse attention at these priors selects top-2048 keys, which is the DSA model's own trained path, not an approximation plow invents. Below 8192 is not measured and not changed. `PLOW_AMD_TAIL_SPARSE_CTX=16384` restores the old floor. | landed |
| 83 | 2026-09-13 | this commit | correctness (shared prefix) + two opt-in knobs | `exec/amd/shared_prefix.rs` `publish_rows`, `memory/vmm.rs` `resolve_prefix_hazard`, `memory/prefix.rs` `evict_tail`; `PLOW_VMM_CACHE_MIN_FREE_MIB`, `PLOW_AMD_PREFIX_FINE_ROWS` | **A shared-prefix attach could pair old physical blocks with a newer snapshot's scales.** Only the first cache group resolved the hazard, so the indexer-key group (8192-row blocks) kept orphaned old blocks while a later prefill of the same content published fresh scales. Strict publish now resolves every group and `evict_tail` unlinks the orphaned suffix, stopping at nodes a live sequence still references; the publish is refused only while such a holder exists. Job `attach-verify4` (mixed two-system-prompt pass, all ranks byte-checked per attach): whole-rule / per-piece mismatches 4 / 0 -> **0 / 0** over 63 attaches; turn 2/3 hit rate 0.917 / 1.0; 288 hazard events, 48 resolved, 240 blocked by a live holder (1-4 blocks at rows 16384/18400). Opt-in, unset by default: pressure eviction on real free device memory (HSA agent info 0xA015) and sub-granule fine matching. Attach cut2 (reused-slot attach 511 -> 5 ms) held: its turn-1 TTFT p50 read 10.3 s vs 5.6 s in the same check and needs isolating without the byte-check hashing. | open (cut2 held) |
| 84 | 2026-09-13 | this commit | perf (first-chunk TTFT, opt-in) | `PLOW_MLA_PF_ROW_SPLIT`; `exec/amd_sparse_mla.rs` `split_patch`/`enqueue_split`, `exec/amd.rs` | **A fresh prompt's attention rows past 2047 run on the native sparse route.** A chunk starting below prior 2047 ran all attention on the interpreter gather arm (285 ms of an 8192-row first chunk vs ~171 ms native), because the native fixed-width selection needs 2048 causal keys per row. The chunk now splits at round_up_8(2047 - prior): later rows go through `SparseMla::enqueue_split` with the same indexer selection, earlier rows stay on the interpreter with union and flash row counts patched to the cut, so their per-8-query tiles are unchanged. Runtime only; packet and objects unchanged. Full depth, knob the only difference (job `p0split-full-depth`): 8192-row first chunk 725.9 -> **616.8 ms** (-15.0%, attention 285.3 -> 183.5), 4096-row 388.7 -> **357.7 ms** (-8.0%); retrieval 39/39; serving guard pass. 2-layer gate: layer-1 KV rows below the cut byte-identical, rows above and last-row logits inside the floor (rel-L2 1.0e-2 / 3.4e-3), same argmax. ISL8192 C1 TTFT estimate 727 -> ~618 ms (vLLM 449). Default flip waits on the served A/B (`p0split-serve-ctl`/`-split`). | open (opt-in) |
| 85 | 2026-09-13 | this commit | perf (prefill, opt-in) | `PLOW_AMD_UNION_SKIP`; `exec/amd_sparse_mla.rs` `unread_unions`/`union_skip_table`, `exec/amd.rs` `union_unread` | **The host skips the `IndexUnionPf` segments the native sparse route leaves unread.** At prior ≥ 2047 the AITER sparse flash gathers `iidx_pf` per row; the per-8-query union table (21 segments, 18.3 ms per 8192 chunk) is read only by the interpreter gather arm. The load-time proof: a union segment is skippable only when it holds that union alone, carries no successor or xctr obligations, and every instruction reading its table or mask before the next union is a sparse flash on a native route (under `PLOW_GLM_DSA_PF_SPAN=3` one union feeds up to four flashes). The skip also needs that route active, no packed binding, and segment specialization; the phase-chain packet count returns 0 for the segment. Packet and objects unchanged. T3 (ctrl / on / ctrl2 in one job, `glue2-t3-g1-rung`, floor = |Δctrl| + 2·MAD): P8192-S 667.2 / 648.3 / 666.8 ms, **−18.7** (floor 3.7); P4096-S −11.1 (floor 1.2); P8192-0 −0.8 (floor 7.5, unchanged as expected). Fired: `skippable=21` on program 3, every rank. An earlier run that required exactly one reader never fired. Firing trace (`glue2-t3-g1-trace`): no `IndexUnionPf` in the chunk window, interpreter→native boundaries 426 → 405. Values (`glue2-t3-g1-value`, 20480-id prompt, ctrl vs ctrl2 as the cross-process floor): last-row logits rel-L2 3.9e-2 vs floor 3.6e-2, argmax equal; layer-0 KV rows byte-identical in every arm; layer-77 KV at the floor. Default off; flip waits on T4. |
| 86 | 2026-09-13 | this commit | perf (prefill, opt-in) | `PLOW_GLM_MOE_SHARED_SEED`; `devgen/src/mla.rs` `emit_glm_moe_ffn_prefill`, `exec/amd_moe_aiter.rs` `Mode::SharedBf16`, `runtime/amd/moe_aiter_adapter.hip` `keep_out` | **The MoE combine pass is gone on the AITER route: the fused call accumulates onto the shared partial.** The shared expert's hipBLASLt down projection writes `dg_tp` (the MoE seam's reduce-scatter source) directly, and `MoeAiterFp8Pf` mode 3 names it as its output. Prepare leaves it (`keep_out`, was `pad`; ABI marker `plow_moe_aiter_keep_out_abi_1`) instead of zero-filling it, and fmoe's bf16 atomics add the routed partials onto it. `MoeCombinePf` (6.9 ms per 8192 chunk) and the 100 MB zero store per layer disappear. The host route requires a preceding full-row `GemmLtPf` writing the output and no combine reading it, and refuses an adapter without the marker. Knob off: `model.pkt` byte-identical; on: −75 `MoeCombinePf` and 75 `MoeAiterFp8Pf` at mode 3 in programs 2 and 3, nothing else; the adapter's disassembly differs only in `plow_moe_aiter_prepare`. Not bit-identical: the accumulation order moves (the control's is already cross-process nondeterministic, #50). T3 (`glue2-t3-g4-rung`, floor = |Δctrl| + 2·MAD): P8192-S 648.5 / 641.4 / 648.8 ms, **−7.3** (floor 2.3); P4096-S −3.3 (floor 2.1); P8192-0 −8.6 (floor 7.4). Values (`glue2-t3-g4-value`, off vs off2 as the floor): last-row logits rel-L2 4.3e-2 vs floor 3.4e-2 (bound 0.14), argmax equal; layer-0 KV byte-identical; layer-77 ckv 0.139 vs floor 0.143, krot 0.021 vs 0.023. PASS. Default off. |
| 87 | 2026-09-13 | none (reports/gemm-shootout.md) | finding (kernels, closed) | 132 (projection, M) GEMM/GEMV shapes: plow tiles × split-K × grid, pinned hipBLASLt sweep (456 kernels), hipBLASLt library heuristic; jobs `gs-tiles-*`, `gs-sweep-*`, `gs-ltlib3-*`, `gs-lt-current`, `gs-repick-pf`, `gs-t2-anchor` | **On one clock, hipBLASLt beats every plow tile on all prefill projections and decode rungs 8–20** (q_a D20 13.4 vs 17.9 µs, o_proj 8192 353.6 vs 500.7 µs); plow GEMV wins decode 1 (13/13 shapes) and 2 (9/13). The earlier plow split-K wins and `band-split-k.md` §2 compared device-event tiles against host-wall pinned kernels and are invalid. Routes unchanged; split-K levers dropped. The rung protocol now requires one timing method per comparison (523226fa). | closed |
| 88 | 2026-09-13 | none (parked) | finding (decode, measured regression) | `PLOW_AMD_GEMM_LT_REPICK` (runtime; 13 decode `GemmLtPf` routes, 5 descriptors) | **Isolated-dispatch kernel wins do not transfer to the decode chain.** T2: −7.7 ms/step predicted at D20 (host-wall sweep); device-clock anchor 12/17 routes faster (q_a D20 45.2 → 27.0 µs, sh_down 48.8 → 14.7). T3 on the full production set, ctrl/new/ctrl2 in one job (`gs-t3-d20`, `gs-t3-d8d16`): D20 +0.50 / +0.74 ms amd-bench 65k / 8k (floors 0.47 / 0.62), +0.62 ms served decode ticks at C20 (floor 0.37); D16 +0.44 (0.40); D8 +0.54 (0.31). Decode kernel choices change only on T3 evidence. | parked |
| 89 | 2026-09-13 | d25c8aff, 0fb49ad9 | verification (default on) | `lean-plow/Plow/Knobs/{Schema,Formula,Resolve,Consistency,Scope}.lean`; `crates/{devgen,plowrt}/src/knob_spec.rs`; `plowc knob-scope`, `plowrt knob-scope`; `scripts/knob_scope_ci.sh` | **Every knob is registered and every emit is checked.** Checkpoint K resolves each configuration against the registry (cli > env > production default > static) and checks every cross-knob constraint; fatal at `plowc` emit, re-checked at plowrt load (absent or skipped K warns, failed K refuses). Checkpoint S proves which programs, ops and fields a knob changes; §4.6 fixtures on real packets: the 6deb4025 cap rejected on op 29, the wide-collectives variant accepted, off→off identical, tail-sparse and decode-min-rung route moves exact, G1 11 route moves, G4 750 instruction diffs all in scope. Production `model.pkt` unchanged (8b15f4a2). Emits need `PLOW_VERIFY_BIN` to an S-capable `plow_verify`. | fixed 548bda7e (stage C, #90) |
| 90 | 2026-09-13 | 548bda7e, b2aa8809 | verification (default on) | `lean-plow/Plow/Knobs/Ledger.lean`; `crates/lean_verify/src/checkpoints/perf.rs`; `scripts/{ledger_append,perf_cert}.py`, `scripts/perf_gate_ci.sh`; `docs/schemas/perf-ledger-entry-v1.schema.json`; plowrt `knob_spec.rs` `check_assets` | **A knob default flip needs a measured performance certificate.** Checkpoint P reads a per-rung ledger (ctrl, treat, ctrl2 samples) and accepts a flip only when every touched rung improves beyond its noise floor \|ctrl − ctrl2\| + 2·MAD, computed in rationals; a rung without enough samples is `insufficient_evidence` and blocks. Eight theorems without sorry (`checkP_sound`, `insufficient_blocks`, `flip_non_regression`, `per_rung_argmin`, …). History replay accepts G1, G4 and the tail floor; #76, `XR_RES`, `XR_DEC_CUS`, FP8 block-scale and small-rung `auto` are insufficient (summaries without samples). `perf_cert.py stamp` writes `rungs[].perf_cert` into `build.json`; plowrt warns once per process without one (b2aa8809: it had warned once per rank and tripped smoke WARN gates); CI fails a default change without a committed certificate. Production `model.pkt` unchanged. Emits use `PLOW_VERIFY_BIN=/workspace/plow-verify/plow_verify-stageC`. | open — re-measure the insufficient flips with per-request samples |
| 91 | 2026-09-13 | none (jobs `p0split-t4-{ctl-r2,split,ctl2,split2}`) | perf (first-chunk TTFT, T4 verdict) | `PLOW_MLA_PF_ROW_SPLIT` (#84); served A/B `t4-ci.py`, exact ISL 8192/4096, C1 ×32, C16 ×160, OSL 128 | **Row split does not pass the served A/B; it stays opt-in.** Medians move the right way (8192 C1 TTFT 856.6/848.2 → 752.7/756.0 ms; 8192 C16 1697/1696 → 1488/1489 ms, TPOT 139.6 → 132.9; 4096 C16 856/857 → 795/795 ms), and the harness check ctl2 − ctl contains 0 in every cell, but no paired-mean TTFT CI clears 0 (pooled 8192 C1 −62.3 [−107.9, +0.0] ms; 8192 C16 −143.2 [−293.9, +4.1]; 4096 C16 −71.2 [−169.1, +27.2]). The first 4096 C1 treatment arm has TPOT +3.29 [+1.24, +5.80] ms while the second has −0.22. Suspected variance: `begin_slot` costs 100–134 ms on a slot whose previous occupant published an 8192-token prefix and 0.4 ms otherwise (trace `p0split-ttft-gap`: 78 shared column-0 unmaps plus remaps per rank, serial over 8 ranks). | open — re-run T4 after the begin_slot fix lands |
| 92 | 2026-09-13 | this commit | perf (prefill TTFT, default flip) | `PLOW_AMD_UNION_SKIP` default on (`config.rs`, `plowrt/src/knob_spec.rs` Qualified), `perf-certs/rt.union_skip.json` | **The host skips the unread `IndexUnionPf` segments by default (review log #85).** Served T4 (`glue2-t4-off/-on/-off2/-on2`, one server per arm, landing binary d25c8aff tree, stage-A packets, exact-length ISL 16384 and 12288 at C1 x 32 and C16 x 160, OSL 128, bundled with the G4 packet): retrieval 18/18 base + 21/21 tail in every arm. Checkpoint P floor (|off − off2| + 2·MAD, per-request, `t4/SCORE-pfloor.txt`): no arm worse beyond its floor; C16 TTFT −39.0 ms (floor 14.4) and −23.1 (13.9), C16 TPOT −2.66 (0.85) and −1.92 (1.54) at ISL 16384 / 12288; C1 TTFT −35.6 (36.0) and −16.8 (37.1) and C1 TPOT inside the floor. The strict paired-bootstrap scorer (`t4/SCORE-ci.txt`) fails only on the C1 TPOT ±0.1 ms spread and C16 TTFT CIs that cannot resolve. `plow_verify` stage C checkpoint P accepts: P8192-S 667.0 → 648.3 ms (floor 3.7), P4096-S 391.0 → 379.9 (1.2), P8192-0 neutral within 7.5 (route scope), packet digest unchanged, 16 serving entries not worse. Rollback `PLOW_AMD_UNION_SKIP=0`. |
| 93 | 2026-09-13 | this commit | perf (first-chunk TTFT, opt-in) | `PLOW_MLA_PF_ROW_SPLIT_NATIVE_LO`; `exec/amd_sparse_mla.rs` `enqueue_split_lo`/`lo_csr`/`Route::native_lo`, `exec/amd.rs` | **With row split on, a fresh prompt's rows below 2047 prior keys also run the native sparse kernel.** The interpreter half (58.0 ms of an 8192 first chunk, 46.8 ms at 4096) cost 744 µs/layer for 2048 rows. Each lower row now attends all its causal keys through a ragged identity CSR on the pinned AITER kernel, single split only (ns=2 returns wrong values on ragged rows: T2 `pfroute-t2-ragged` rel-L2 0.47-0.64 vs 1.4e-4-1.9e-4 at ns=1; no production route uses ns=2 on a ragged CSR). The cut is exactly 2047 - prior, the interpreter flash launch is gone and its unread unions are skipped. T2 device clock: 316.6 µs/layer for the lower half. Full depth, row split on in every arm (job `pfroute-lo-t3-full`): 8192-row first chunk 615.0 / **575.1** / 615.4 ms (ctl / treat / ctl2, floor 5.0, **-40.1**), 4096-row 356.0 / **327.0** / 355.9 (floor 0.7, **-28.9**); P8192-S unchanged (+0.0, floor 2.4); retrieval 39/39, serving guard pass. 2-layer gate (`pfroute-lo-t3-gate-r2-row2047-native-floor`): logits 3.3e-3 vs floor 3.4-3.5e-3, argmax equal, L0 KV byte-identical, 16/16 lower and upper launches. Row 2047 is interpreter in both controls (8-aligned cut 2048) and native in treat, so it is scored against the largest single-row native ctl2-vs-ctl drift in the same job (r1 per-row analysis: FP8 ckv noise, no step across rows 2040-2055). Runtime only; packet and objects unchanged. | open (opt-in) |
| 94 | 2026-09-13 | this commit | perf (resumed-tail TTFT, opt-in) | `PLOW_AMD_TAIL_SPARSE_MIN_PAIRS`; `serve/engine.rs` `retarget_dense_tail`, `sched/slo.rs` `Ladder::bucket_for`, `knob_scope.rs` route model, generated `plow-asset/src/knob_gen.rs` | **A dense final chunk at prior 2047-8191 moves to the sparse bucket only when prior x tail rows reaches a pair floor.** A final chunk at these priors exists only after a prefix-cache resume; the prior-only floor (`PLOW_AMD_TAIL_SPARSE_CTX`, default 8192) cannot place it well. Served T3 through exact prefix resumes (job `pfroute-t3-tail`, 7 arms, chunk ms median of 5, dense 8192-floor control -> sparse): S 128 loses at R 2112 / 4160 (+10.7 / +8.5, floors 0.3 / 0.9); S 512 +0.8 at R 2112 (floor 0.3), -8.4 at 4160, -17.8 at 6208; S 2048 -20.5 / -59.3 / -99.1 at R 2112 / 4160 / 6208; R 8256, cold 2048 and cold 16384 unchanged. Dense cost grows with prior, sparse does not, so the crossover follows R x S (~1.23M / 1.33M / 1.29M pairs), not a row count. Values: pooled per-request logits floor (moved-route median rel-L2 within 3 x ctl2 over the same requests) PASS in every arm; retrieval tail2560 21/21, tail6144 21/21, base 18/18. `PLOW_AMD_TAIL_SPARSE_CTX=2048` + `=1500000` keeps every measured win and avoids every measured loss; confirm T3 at the crossover points `pfroute-t3-tail-pairs`. Default unset (today's behaviour). | open (opt-in) |
| 95 | 2026-09-13 | this commit | perf (prefill chunk, opt-in) | `PLOW_GLM_FUSE_POST` (`emit_config.rs`, `mla.rs` q_rope prologue, `manifest.rs`/`object.rs` requires + `plow_glm_fuse_post_arm`, `op_gemm_common.h` `d_gemm_t` RoPE epilogue, `interp.hip` `exec_gemm_med`, `build_gfx942.sh` prefill define) | **The 8192-bucket q_rope `GemmMed` applies the interleaved q RoPE in its store; the q `HeadNormRope` packet is not emitted (78 per chunk).** Ranked against the other GEMM-adjacent glue (attribution-v2 §2a): idx HNR128/LayerNorm, router top-k, kv/k writes and Glu follow a native GEMM or an all-gather, so only this pair fuses. T1: knob off model.pkt/plow_config.h byte-identical, objects disassembly-identical; checkpoint S accepted (235 differences in scope, program 3 only). T2 (`fusepost-t2`, 1 GPU, prefill interpreter object, captured qlat + q_rope weights, device clock, anchored): fused vs [GemmMed ; HNR] −20.6 / −16.2 / −34.2 / −52.4 µs at M = 20 / 1024 / 4096 / 8192, 6/6 trials each, bit-exact at every M. T3 (`fusepost-t3`, full depth, ctrl/treat/ctrl2): P8192-S 668.9 / 666.0 / 668.9 ms (−2.9, floor 3.0: miss by 0.1), P4096-S 391.7 / 389.1 / 391.6 (−2.6, floor 0.7), P8192-0 −3.4 (floor 6.8); L0 ckv/kidx/krot/scale byte-identical, logits 3.616e-2 vs floor 3.603e-2, argmax 68/68; retrieval 18/18 + 21/21; a fused packet on unarmed objects is refused at load. Confirm (`fusepost-confirm-p8192s`, 6 rounds × 2 reps, same objects and packets): P8192-S 666.2 / 662.6 / 665.2 ms, round-paired median −3.4 ms (mean −3.2), treat faster in 6/6 rounds, inside the 3.0 ms floor; P8192-0 −2.6 (floor 3.9). No default flip: ~0.5% of the chunk does not justify a serving-set regeneration alone. Rollback `PLOW_GLM_FUSE_POST=0` (default). | opt-in; −3.4 ms P8192-S, 6/6 rounds, inside the floor |
| 96 | 2026-09-13 | this commit | perf (first-chunk TTFT, default flip) | `PLOW_MLA_PF_ROW_SPLIT` default on (`config.rs`, `plowrt/src/knob_spec.rs` Qualified), `perf-certs/rt.mla_pf_row_split.json` | **Row split is on by default (#84 opt-in, #91 served A/B not resolved).** Served T4 on the default config (`pfroute-t4d-ctl/-split/-ctl2/-split2`, tip 38d97bac binary, `PLOW_VMM_RELEASE_RETIRE` absent, union skip at its default, one server per arm, exact-length ISL 8192 / 4096, C1 x 32 and C16 x 160, OSL 128; scorer `t4d-ci.py`, paired-median bootstrap CIs floored at the same-job control drift, declared before the arms ran and null-checked on #91's controls): TTFT -98.5 [-99.6, -96.0] ms at 8192 C1, -211.8 at 8192 C16, -27.4 at 4096 C1, -61.6 at 4096 C16; TPOT +0.02 (8192 C1, inside 0.31) / -11.65 (8192 C16) / -0.04 / -3.29 ms; ITL[0] -0.03 / -0.32 ms and E2E -95.2 / -28.6 ms at C1; 0 failed requests. Checkpoint P (plow_verify stage C) accepts: P8192-0 730.6 -> 630.3 ms (floor 2.3, `pfroute-t3-tail` cold 8192), P4096-0 388.9 -> 357.6 (floor 1.0, `p0split-full-depth`), P8192-S neutral within 5.0 (route scope: no split at prior >= 2047), packet digest unchanged, 32 serving entries (TTFT / TPOT / ITL[0] / E2E per cell, split and split2) not worse. Correctness: retrieval 18/18 + 21/21 (`p0split-full-depth`) and prefix-resumed tails 21/21 at priors 2560 / 6144 with base 18/18 (`pfroute-t3-tail` split arms); 2-layer gate rows below the cut KV byte-identical, logits rel-L2 3.4e-3. P4096-S has no split-on/off measurement (route scope leaves it unchanged). `PLOW_MLA_PF_ROW_SPLIT=0` is the rollback. | landed |
| 98 | 2026-09-13 | this commit | perf (first-chunk TTFT, default flip) | `PLOW_MLA_PF_ROW_SPLIT_NATIVE_LO` default on (`config.rs`, `plowrt/src/knob_spec.rs` Qualified), `perf-certs/rt.mla_pf_row_split_native_lo.json` | **Row split's lower half runs natively by default (#93 opt-in).** Served T4 with row split on in every arm (`pfroute-t4lo-ctl/-split/-ctl2/-split2`, 055dd128 binary, one server per arm, exact-length ISL 8192 / 4096, C1 x 32 and C16 x 160, OSL 128; scorer `t4d-ci.py`, paired-median bootstrap CIs floored at the same-job control drift): TTFT -35.5 [-37.7, -34.2] ms at 8192 C1, -80.9 at 8192 C16, -23.1 at 4096 C1, -58.7 at 4096 C16; TPOT +0.01 / -4.45 / -0.03 / -2.84 ms; C1 ITL[0] -0.12 / +0.19 (inside 0.44) and E2E -34.2 / -27.4 ms; 0 failed requests. The 4096 C16 `split` arm alone holds 10 pure-decode ticks of 186-2071 ms (dec_rows 16, no prefill, ticks 9654-9967), which widen its TPOT CI (+0.86 [-0.69, +2.74]); `split2`, the same configuration, has none and TPOT p50 87.6 vs ctl 90.7 ms, so it is the device-drain decode stall already seen in every arm class, not the lower half. Checkpoint P (plow_verify stage C) accepts: P8192-0 615.2 -> 575.1 ms (floor 5.0), P4096-0 355.9 -> 327.0 (floor 0.7), P8192-S neutral within 2.1 (the lower half exists only below prior 2047), packet digest unchanged, 32 serving entries not worse. Correctness: retrieval 39/39 and serving guard with both knobs on (`pfroute-lo-t3-full`); 2-layer gate logits rel-L2 3.3e-3 vs floor 3.4-3.5e-3, argmax equal, L0 KV byte-identical, row 2047 within the same-job single-row native floor. `PLOW_MLA_PF_ROW_SPLIT_NATIVE_LO=0` is the rollback. | landed |
| 99 | 2026-09-14 | this commit | perf (admission TTFT, default flip) | `PLOW_VMM_RELEASE_RETIRE` default on (`config.rs`, `plowrt/src/knob_spec.rs` Qualified), lever in `memory/vmm.rs` (`copy_out`, spare pool, `refill_spares`, `drop_spare`), `exec/amd/shared_prefix.rs` `release`, `exec/amd_tp.rs` `flush_deferred_publish`, `perf-certs/rt.vmm_release_retire.json` | **A finished request's cache-shared row-0 blocks are handed to the prefix cache as copies on the pool thread, so the next `begin_slot` keeps row 0 in place.** Before: `begin_slot` cost 100-134 ms on every request after a published 8192-token occupant, 78 serialized row-0 unmaps plus maps per rank; ROCr serializes `hsa_amd_vmem_*` process-wide (8-device swap probe 0.93x), while D2D copies scale (78 blocks x 8 devices 11.15 ms serial, 2.13 ms parallel). Copies run with the pool lock released into spares mapped at load (2 per track, within `PLOW_KV_POOL_MIB`); evicted copies return as spares (one extra column per track, off the reuse pool); spare drops never run inside a TP deferred publish. Iterations: v1-v3 failed smokes / T4 on copy-VA unmaps, maps and copies behind the reclaim gate, and pool-cap charging (`slotclear-t4-arms3`); v4r on 3f901456 failed 4096 C1 on drops during the publish that evicted (1638 per rank). Served T4 on the default config (`slotclear-t4-arms-v5`, v5 binary 48541734 on 9f52fb73, one server per arm, exact-length ISL 8192 / 4096, C1 x 32 and C16 x 160, OSL 128, scorer `t3/summarize.py` P floor rule): 8192 C1 clear 100 -> 0.5 ms, TTFT 757 -> 655 ms (-102.5, floor 21.3), E2E -92 ms; 8192 C16 TTFT -178 ms, TPOT -9.6 ms (-7.4%), E2E -1.4 s; 4096 C1 all within floor (v4r +37 ms ITL[0] gone). treat2 alone at 8192 C1 ITL[0] +4.2 / TPOT +0.10 ms is one-arm host-loop noise (decode ticks behind the GPU all cell, no drops; treat on the same binary tracks ctrl). 8192 C16 tradeoff: decode-tick p99 51.15 / 50.98 vs 49.21 ms, from the deferred spare drops on the two decode ticks after a prefill (+2.7 / +2.3 ms), with TPOT p99 124.2 / 124.8 vs 134.3 / 135.4 ms; drop pacing is follow-up. Checkpoint P (plow_verify stage C) accepts: 8192 C1 TTFT 757.2 -> 654.2 (floor 21.3), 8192 C16 1490.7 -> 1312.7 (floor 7.3), 4096 C1 neutral within 3.5, packet digest unchanged, 24 serving entries not worse, numerics unchanged. Correctness: `slotclear-guard-retrieval-v5` retrieval 39/39 and serving guard PASS with the knob on; 99 vmm unit tests incl. generation / eviction / attach / refill / drop races and a C1 CPU model. `PLOW_VMM_RELEASE_RETIRE=0` is the rollback. | landed |
| 100 | 2026-09-14 | this commit | perf (short-context decode TPOT, opt-in) | `PLOW_GLM_DECODE_DENSE_EXACT` (emit: `mla.rs` dense-exact rungs 8/16/20, `GLM_DENSE_EXACT_MIN_RUNG`; `packet/devbuild.rs` `DENSE_EXACT_PROG` / `ProgramRole::DenseExactRung`; `manifest.rs` topology `dense_exact`) + `PLOW_AMD_DECODE_DENSE_EXACT` (runtime: `exec/amd_tp.rs` `pick_decode_prog`, `exec/amd.rs` `dense_exact_for`, `exec/kvrow.rs` `decode_dense_exact`); knob scopes rows 8..20, generated `knob_gen.rs` | **While every row a decode rung advances holds at most 2048 keys, the DSA selection keeps all of them, so the step runs a dense-exact twin of that rung instead of the indexer + sparse flash.** Production max_ctx 81920 is above the 65536 per-packet DSA gate, so every decode step paid the indexer (q/k/weights projections, IndexScore, IndexSelect on 21 layers) even at short context. The emit knob adds decode/8, /16, /20 dense-exact programs (dense `FlashMlaDecodeFp8` at the DSA rung's split count, no indexer query side or selection; the kidx cache write stays so later DSA steps read written keys); tensors, union, objects and pairing are unchanged and knob off is byte-identical (checkpoint S: every difference in `decode/{8,16,20}/dense_exact`). The runtime knob swaps per step from the host kvlen vector; rungs below 8 never swap. Values: one layer at kv <= 2048, D8/D20 logits byte-identical to DSA every step; across kv 2048 (96 steps) on-vs-off stays inside the DSA run-to-run floor (r8 7.9e-4 vs 7.7e-4, b20 2.06e-3 vs 2.01e-3), argmax 96/96. Full depth prompt 2000 (off/on/off2 ms/step): D8 38.69/36.64/38.60, D20 52.30/49.18/52.07. Served C1 ISL 1024/OSL 256, 24 paired requests: TPOT 37.72/35.93/37.70 ms (-1.78, -4.7%; checkpoint-P floor 0.065 ms), TTFT unchanged. Quality with the runtime knob on: GSM8K 8-shot n=200 192/200 vs 193/200 off; retrieval 39/39, serving guard PASS; no server errors (jobs `livectx-c1-{a-ctrl,b-treat,c-ctrl2}`, `livectx-gsm8k-{treat,ctrl}`, `livectx-retrieval-treat`). **Lands opt-in, both knobs off:** the gain is short-context decode only (no step of a long-prompt workload qualifies), while an emitted set pays +16 MB packet and about +50 MiB per rank, and a flip needs checkpoint-P D16/D20 per-step samples. Side finding (open): full-depth rung 1 showed a 981 ms single-workgroup stall on a layer-49 shared-expert GEMV, which is why dense-exact starts at rung 8. | landed |
| 101 | 2026-09-14 | this commit | perf (served C1 TTFT host path; bug fix + opt-in) | `text/tokenizer.rs` vocab size cached at load in both constructors (bug fix, no knob); `PLOW_IDLE_DISPATCH` (`sched/batching.rs`, `serve/mux.rs` ingress count, `serve/chat.rs`, `serve/completion.rs`); `PLOW_ENCODE_FAST` and `PLOW_ENCODE_THREADS` (`text/tokenizer.rs` split-safe BPE check + dedicated rayon pool); `config.rs`, `plowrt/src/knob_spec.rs` OptIn, `docs/flags-reference.md`, `tests/api.rs`; rayon direct optional dependency | **Served GLM-5.3-FP8 TP8 C1 8192 spent 34 ms of its 614 ms client TTFT on the host before prefill; three runtime-only fixes remove almost all of it.** The /v1/completions token-id bound check called tokenizers 0.20 `get_vocab_size(true)`, which rebuilds the 154,856-entry vocab map per request (10.5-12.9 ms CPU, now a field read; out-of-vocabulary refusal unchanged, now with API tests). Idle dispatch skips the formation hold when the slot table is empty and no request is queued or still tokenizing (CPU mux C1 first frame 9.2 -> 0.085 ms; an 8-request burst takes the same 2 holds). Split encode cuts prompts of 8 KiB or more at a single ASCII space between letters, only for BPE tokenizers whose pre-tokenizer makes that cut id-preserving; token ids identical on 1420 GLM-5.3 prompts / 7.44M tokens for every variant (CPU 8192-token prompt: encode 11.3, N=4 4.7, N=8 2.5 ms). Served on 8x MI300X with all three on (N=8), same set and cells as the bin-0ada split: handler 11.9 -> 0.04 ms, tokenize 12.9 -> 3.15, queue 9.2 -> 0.10, client TTFT median 614.1 -> 593.0 ms (ttft_client token ids 601.3 -> 580.8), 0 failed. Knobs stay off: a C16 check of the unpinned tokenizer threads is queued. Checks: ws all-targets, plowrt cuda,hsa all-targets; plowrt lib 1009, api 10, co_serving 6, metrics 1, tokenizer 6. |
