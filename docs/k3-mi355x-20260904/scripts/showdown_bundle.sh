...
#   PLOW_BUNDLE_SRC=<worktree>            source tree (default: the d1-moe-decode-rule worktree)
#   PLOW_BUNDLE_CMAKE_EXTRA="-DX=Y ..."   extra object-build cmake flags (e.g. -DPLOW_XR_SCHED=aiter)
set -euo pipefail
wt=${PLOW_BUNDLE_SRC:-/home/lava/plow/.claude/worktrees/d1-moe-decode-rule}; model=/home/shaswot/models/Kimi-K3
...
nix develop -c bash -c 'exec cmake "$@" -DPLOW_HSACO_HIPCC="$PLOW_HIPCC" -DPLOW_HSACO_BUNDLER="$PLOW_BUNDLER"' bash \
  ...
  -DPLOW_HSACO_MOE_DECODE_GROUPED=ON ${PLOW_BUNDLE_CMAKE_EXTRA:-} >"$B/config.log" 2>&1
