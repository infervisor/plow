...
| `PLOW_GLM_SELECT_SPLIT` | `--glm-select-split` | unset (off) | Give each batched GLM decode row's DSA top-k selection up to N workgroups instead of one (capped so rows × N fits the 304 CUs: N = 15 at rung 20). Each row runs the cooperative radix (`d_index_select_coop`) over its own histogram/control strip (`act.ighist` / `act.igctl` grow to one strip per decode row), as `IndexSelect` `i[4] = 2`, `i[5] = N`. Same packed key as the local form, so the selected set is identical (exact top-k, ties by lowest index); `runtime/bench/interp/dsa_select_split_test.c` gates it against the local form and a host top-k. Needs `PLOW_GLM_SELECT_LOCAL` and decode objects built with `PLOW_DSA_SELECT_SPLIT=1`, which `build_gfx942.sh` sets from the packet's `requires`; plowrt refuses the packet on an object without `plow_dsa_select_split_arm`. Decode rungs only (the token-batch band keeps the local form). Opt-in pending its tier 3. |
