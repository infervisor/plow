...
| `PLOW_EMIT_PREFILL_CUBLASLT` | `--emit-prefill-cublaslt` | false; **production default true for Gemma-4 BF16 on sm_90a TP1** | Route the eligible dense prefill projections (`plow-asset::segment_roles::cublaslt_prefill_bf16`: M∈{128,256,512} × {(3840,15360),(3840,8192),(15360,3840)} and M∈{1024..8192} × the eight Gemma-4 shapes) to cuBLASLt, each as its own segment. With `PLOW_NO_GLU_FUSE=1` the unfused gate/up GEMMs are covered too, which is the measured win over the fused GLU role. The runtime dlopens `libcublasLt.so.13`; put the toolkit's `lib64` on `LD_LIBRARY_PATH`. Exact-shape algorithms are pinned from `<assets>/cublaslt_algos.jsonl` when present (`PLOW_LT_ALGOS`; produced by `PLOW_LT_ALGOS_WRITE` or the campaign `probe`, packetized by plowc from `tuning/nvidia/<arch>/*/cublaslt_algos.jsonl`), else selected at load. `=0` is the rollback. |
