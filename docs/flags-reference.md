...
|--------|--------|------|-------|
| `plowc` | `EmitConfig` | `crates/devgen/src/emit_config.rs` | Emit-time knobs (211 fields, 14 hidden diagnostics) |
...
### The knob registries are the authoritative list

`crates/devgen/src/knob_spec.rs` (emit: `EMIT`, `RAW_ENV`, `OBJECT_DEFINES`) and
`crates/plowrt/src/knob_spec.rs` (runtime) declare **every** knob, its default,
its status and the constraints over it; checkpoint K verifies an emit against
them and `build.json` records the verdict. Their evidence strings cite *this*
file, so a default or rollback direction changed in one must be changed in both.
A new `PLOW_*` knob, env read or `#if PLOW_*` define has to be registered there
(`cargo test -p devgen --lib knob`, `cargo test -p plowrt --features cuda,hsa
--lib knob`).

**This file is not exhaustive against those registries.** Roughly 130 registered
env knobs — test fixtures (`PLOW_TEST_*`), dumps and snapshots
(`PLOW_SEG_DUMP`, `PLOW_TENS_SNAP`, `PLOW_DUMP_ACT`), and per-family experiment
arms — are deliberately not tabulated here. Grep the registries before concluding
a knob does not exist.
...
| `PLOW_MAX_REQUEST_CHUNK` | `--emit-max-request-chunk` | unset | Maximum real rows one request contributes to a packed prefill launch (must be a prefill rung; **not** necessarily a power of two, since the ring rounds up itself). Below `PLOW_MAX_CHUNK` it also sizes the sliding-window KV ring: `next_pow2(window + request_chunk - 1)` rows per slot instead of `next_pow2(window + max_chunk - 1)`, so a 4096-row launch packs 1024-row slices onto 2048-row rings. A rung wider than `PLOW_MAX_CHUNK` is admissible when the ring already holds it — window 1024 at request chunk 4224 rings `next_pow2(5247) = 8192`, the same per-slot KV a 4096 chunk costs. **The cap is applied to a request's slice BEFORE the bucket is picked** (`pf_request_max_rows()`, `exec/gpu.rs`), so the rung only has to cover the capped slice, never the prompt: on the 12B ladder packet at 4224 an 8192-token prompt splits `[4224, 3968]`, the second slice landing on the 4096 rung with 128 padded rows. |
...
| `PLOW_NO_GLU_FUSE` | `--no-glu-fuse` | false; **production default true for Gemma-4 BF16 on sm_90a TP1** | Opt OUT of the fused GLU GEMM on non-AMD backends (`=1` disables). On Gemma-4 BF16 sm_90a TP1 `apply_production_defaults` sets it with `PLOW_EMIT_PREFILL_CUBLASLT`: gate/up as two cuBLASLt GEMMs plus the GeGLU pass beat the fused GLU role at every bucket Lt covers (C1 TTFT@1024 89.9 → 56.4 ms, @4096 −4 %, in128 and TPOT neutral; `perf-data/campaign/gemma4-12b.h100.bf16.csv`, `perf-certs/emit.no_glu_fuse.json`). `=0` is the rollback. |
| `PLOW_TMA_GEMM` | `--tma-gemm` | false | Emit TMA descriptors for GEMM operands (sm_90a+). Pure-GEMM segments (`PLOW_SEG_PURE_GEMM=1`) admit only mapped GEMMs, so the lean GEMM object needs them. The Gemma-4 H100 recipes set it: the HD256/GQA2 and HD512 prefill role objects they ship (`[objects].role_files`) bind the map, and their role emit rejects an unmapped packet. |
...
| `PLOW_ATTENTION_DECODE_BALANCE_GF` | `--attention-decode-balance-gf` | unset; **production default 4 for BF16 Gemma-4 hybrid attention (`full_attn_hd512`) on sm_90a TP1** | Full-attention decode GQA fusion factor with batch-aware balanced split counts (grid-aligned: ns=33 at B=1 on 132 SMs). Values: 2, 4, 8, 16; must divide the full-attention GQA ratio. `=0` is the rollback. |
| `PLOW_FA_MMAQK` | `--fa-mmaqk` | unset (opt-in; the BF16 Gemma-4 H100 recipes set `3`) | Decode attention Q·Kᵀ on the tensor cores with K streamed from global; bit 0 = hd256 layers, bit 1 = hd512 layers. Object-paired (rides in the manifest `tuning` and the pairing hash). 12B step ctx 1024 −0.30/−0.51/−1.02 ms at B=1/4/16; 26B-A4B 5.61/8.51/12.44 → 5.54/8.34/11.75 ms; GSM8K unchanged. Its checkpoint P certificate was rejected — 26B 128/C1 TPOT 5.400 -> 5.418 ms, worse than its 0.002 floor — so the default is opt-in and the recipes set it. `=0` is the rollback. |
| `PLOW_SLIDING_NS_CAP` | `--sliding-ns-cap` | false; **production default true for BF16 Gemma-4 hybrid attention (`full_attn_hd512`) on sm_90a TP1** | Cap the sliding-window decode nsplit at `win / 64`. `=0` is the rollback. |
| `PLOW_SLIDING_NS_GRID` | `--sliding-ns-grid` | false; **production default true for BF16 Gemma-4 hybrid attention (`full_attn_hd512`) on sm_90a TP1** | Floor the sliding-window decode nsplit of a batched rung to one work item per block. h100-sxm5 step at ctx 1024, B=2/4/8: 12B 11.92/12.55/13.71 → 11.45/11.98/13.04 ms, 26B-A4B 7.90/9.94/13.13 → 7.57/9.57/12.72 ms; B=1/16 unchanged. `=0` is the rollback. |
| `PLOW_SEG_FA512` | `--emit-seg-fa512` | unset; **production default 1 for BF16 Gemma-4 hybrid attention (`full_attn_hd512`) on sm_90a TP1** | hd512 FlashPrefill in its own segment class for the `_pffa` object: `1` = hd512 only, `all` = every head dim. The serve side reads the class from the packet. `=0` is the rollback. |
| `PLOW_SEG_FA256_GQA2` | `--emit-seg-fa256-gqa2` | unset; **production default true for BF16 Gemma-4 hybrid attention (`full_attn_hd512`) on sm_90a TP1** | Exact HD256/GQA2 sliding FlashPrefill in segments of their own, for the paired `_pfpackedfa256_gqa2` object. `=0` is the rollback. |
...
| `PLOW_GLM_PF_SMALL_CUS` | `--glm-pf-small-cus` | unset (off) | Cap machine-wide packets of the plain GLM prefill buckets below 2048 rows at N workgroups (`auto` = max(64, t/4): 64 at 128 rows, 128 at 512). Kernels grid-stride by `nblk`, so only the work→workgroup partition moves. GLM-5.3 TP8 gfx942 standalone chunk: 128 rows 99.4 → 86.6 ms (cap 64), 512 rows 126.4 → 121.7 ms (cap 128; cap 64 regresses 512 to 139.7); last-row logits within the cross-process floor. Unset ⇒ byte-identical packet. |
...
| `PLOW_GEMV_PREFETCH` | `--gemv-prefetch` | false; **production default true for dense (no `moe`) BF16 Gemma-4 hybrid attention (`full_attn_hd512`) on sm_90a TP1** | Decode objects prefetch a claimed `Gemv` slice's weight rows to L2 (LDS-DMA, no VGPRs) before polling its gate (L8). Packet-inert; recorded in the manifest so the paired `plow_config.h` defaults the object's `PLOW_GEMV_PREFETCH`. 12B step_bench ctx 192 B=1/4/16 10.55/10.75/11.44 → 10.44/10.57/11.41 ms; the MoE 26B-A4B was not measured. `=0` is the rollback. |
...
| `PLOW_KDA_WU_LEAN` | `--kda-wu-lean` | **true** (promoted) | Mark exact qpre BT64/D128 chunk-KDA Wu segments for the lean four-wave gfx950 Wu object. The marked packet requires its paired object; `=0` is the rollback. |
| `PLOW_KDA_CARRY_KEYFEED` | `--kda-carry-keyfeed` | **true** (promoted) | Feed the lean Wu's scaled-key hi/lo pair into the register-state carry (implies the lean Wu; needs `PLOW_KDA_CARRY_REGSTATE`). `=0` is the rollback. |
...
| `PLOW_GLM_INDEXER_WQ_W8A8` | `--glm-indexer-wq-w8a8` | false | Experimental gfx950 TP8 indexer query projection using original block128 FP8 weights/scales and the shared MLA query quantization. Requires `PLOW_GLM_MLA_W8A8` and unpooled 32x128 indexer geometry. Removes the legacy BF16 WQ allocation; leaves WK, normalization, cache and selection unchanged. Projection-only precision work, not complete indexer or serving qualification. |
| `PLOW_GLM_INDEXER_FP8` | `--glm-indexer-fp8` | false | Experimental gfx950 TP8 native indexer query/key quantization, packed block-16 FP8 cache and scoring. Requires `PLOW_GLM_INDEXER_WQ_W8A8`, `PLOW_GLM_MLA_BF16_PS` and `PLOW_GLM_DSA_PF`. Capacity must be 131072; request lengths may be shorter. Decode rungs 1/8/16/32/64; ordinary single-slot prefill buckets 128/512/1024/2048/4096/8192. Short buckets append keys without scoring. Excludes dense-exact, TP indexer, packed and row-split prefill. Selection remains the existing selector. Native objects must be supplied; connected-model accuracy and serving performance remain unqualified. |
| `PLOW_GLM_MLA_BF16_PS` | `--glm-mla-bf16-ps` | false | Experimental gfx950 TP8 BF16 attention segment for decode rungs 1–32 and 64. Requires `PLOW_GLM_MLA_W8A8`, BF16 KV and hash-pinned adapter/metadata/stage/reducer objects. Rungs below 32 use five ordered HSA dispatches; 32/64 use four with compact selected KV and load-time split metadata. The latter additionally require `mla_dec_stage1_bf16_a16w16_subQ16_mqa16.co` and the pinned `attention-reduce-3ca36e1de81b9fe111c2e1681fb8339af1639d37383e3ba868b3f79877bb7c1d.elf`. Replaces a pure flash/merge pair; live rows must be nonempty. Arms decode DSA above top2048 instead of the legacy 64K crossover. Dense context <=2048, no DCP or fused RoPE. Other rungs and prefill retain interpreter attention. No serving/performance promotion. |
| `PLOW_GLM_ROPE_BF16` | `--glm-rope-bf16` | false | Experimental gfx950 plain GLM rotary64/theta8e6 RoPE tables, context <=131072. Requires `PLOW_GLM_MLA_W8A8`. Hash-pinned GPU generation rounds to BF16 at model load; full indexers copy those coefficients into head128 tables with identity tails. Storage remains F32 for the existing interpreter ABI. No per-token launch. Device-only recipes reject CPU/CUDA materialization and baking; other models/default tables are unchanged. |
...
| `PLOW_EMIT_REWRITE` | `--emit-rewrite` | true | Lower the egglog rewrite (`crates/rewrite`) in devgen. plowc extracts the checkpoint's whole text-generation graph (every output) and passes each fused node's kind and anchor weight names; at the seams a rule covers, the extracted graph decides the fusion instead of the hand knob: `AddNorm` for `FusedResidualNorm` / `FusedResidual3Norm` (dense Llama/Qwen decode, GLM/Kimi/DeepSeek B1 and layer seam — `PLOW_GLM_FUSE_B1` / `PLOW_GLM_FUSE_SEAM` are ignored there), `NormResidualNorm` for `FusedNormResidualNorm` / `FusedNormResidualScaleNorm` (Gemma decode). Emitter preconditions (TP, rows, LDS fit) still gate. DEFAULT ON; `=0` is the rollback. When the checkpoint has no graph builder or a config it cannot parse (gpt_oss, nemotron, glm5_next, deepseek_v4, Gemma-4 MoE, kimi_k25) the emit logs one line and keeps the hand fusions, byte-identical to `=0`; a rewrite failure on a supported architecture warns and does the same. Only an explicit `=1` (env or `--emit-rewrite`) refuses those emits. Byte-identical to the hand fusions on the qualified GLM-5.3 TP8 recipe (B1 and SEAM set), Gemma-4-12B and the dense goldens. Changes the packet — and is NOT bit-identical — on any MLA emit whose recipe does not set `PLOW_GLM_FUSE_B1` (and, at TP > 1, `PLOW_GLM_FUSE_SEAM`): those seams become `AddNorm`, which reduces over the un-rounded sum. Cost: the whole-graph rewrite runs on every plowc devblob emit, about +3 s CPU on GLM-5.3 (about +1 s on Gemma-4-12B). |
| `PLOW_TUNE_IGNORE_DIGEST` | `--tune-ignore-digest` | true | Use dense-GEMM tuner records whose build digests have since moved, for the op cases that have NO current-digest record (`devgen::gfx950_gemm_measurements`). Current-digest records are loaded first and are never displaced, and a parked case is taken whole rather than mixing kernel ids across builds, so this only replaces an ANALYTICAL-model fallback with a measured tile. Motivation: the tuning digest covers the preprocessed dense family, so any kernel edit invalidates the entire campaign at once and every tile silently degrades to tier `portable` — the state the B0 tile workstream exists to fix. The strict rule ("a stale record is more dangerous than none") holds when the choice is between two measurements; for a case with none it compares measured-but-older against never-measured. DEFAULT ON, but WITHOUT a hardware certificate: no MI300X was reachable when it was flipped, so the argument is structural, not measured. The staleness line names how many cases were filled this way. `=0` restores strict staleness. |
...
| `PLOW_GLM_MOE_NATIVE_ALIGN` | `--glm-moe-native-align` | false | GLM prefill on the AITER MoE route (256 experts, top-8, align_par): the four interpreter `MoeAlignPf` packets are not emitted; `MoeAiterFp8Pf` names the gathered routing table (`j0` = table + 1) and the align partition count (`j1` = 64), and the host launches `plow_moe_align` (`moe_align_adapter_gfx942.elf`, `d_moe_align_pf` phases 1-4 on grids 64/1/64/64) before prepare. Byte-identical row maps and meta. The route refuses without the adapter. T3 stacked with the SP-seam fusion (review log #104, #105). |
| `PLOW_GLM_FUSE_SEAM_RN` | `--glm-fuse-seam-rn` | false | GLM TP8 prefill, sequence-parallel attention seam: the band `Residual` (xmid = x_in + reduced o_proj) and the post-attention `RmsNorm` into peer slot 3 become one `AddNorm` with `i2 = 1` (`d_residual_rmsnorm`: the sum is rounded to bf16 first, then reduced exactly as `d_rmsnorm`, so both outputs are the separate ops' bytes). The packet requires `PLOW_GLM_FUSE_SEAM_RN=1`; `build_gfx942.sh` sets the define on the prefill objects and plowrt refuses an object without `plow_glm_fuse_seam_rn_arm`. The MoE seam (whose norm belongs to the next layer) is not fused. T3 stacked with `PLOW_GLM_MOE_NATIVE_ALIGN` (review log #105). |
...
| `PLOW_GLM_DECODE_DENSE_EXACT` | `--glm-decode-dense-exact` | false | Also emit a dense-exact decode rung per ladder width ≥ 8 on the sparse FP8-KV DSA GLM decode (dense attention plus the indexer key-cache write, no top-k selection), served by `PLOW_AMD_DECODE_DENSE_EXACT` only on steps where every row holds ≤ 2048 keys, so the selection would keep all of them; the ladder, tensor table, `union` and `objects` are unchanged. |
...
| `PLOW_GLM_MOE_SHARED_FOLD` | `--glm-moe-shared-fold` | false | Fold GLM's shared expert into the native AITER MoE prefill call as expert 256 with a constant gate of 1.0 (router appends a 9th slot, align and the fused call run 257/9, the shared GEMM pair and the combine's `shared` operand go away). This is the shape AITER's gfx942 GLM-5 tuning table is indexed by; same pinned objects. Needs `PLOW_GLM_MOE_AITER`/`_RESIDENT`, TP, and a checkpoint that still holds the shared expert's block-FP8 bytes (the lite prep's are shadowed, and are found). MXFP4 (GLM-5.3 TP8, not EP, not `PLOW_MOE_PF_DET`): the same fold on the grouped A4W4 prefill chain (align/85/86 or the lean stage-1 object at 257/9, combine k=9), shared expert W4A16 → A4W4 over its own MXFP4 bytes; unmeasured. Decode keeps its own shared GEMVs. Opt-in; default OFF. NOT bit-identical: the shared expert moves from bf16 to A8W8 block-FP8, so the FFN output's rel-L2 vs f64 goes from ~1.7% to ~4.8% (≈2.8×; the fold equals routed + shared to bf16 rounding, error flat across columns — a precision cost, not a defect). Measured on GLM-5.3 TP8 (ABA, one binary, per-packet objects, merged runtime): −12.9 ms per 8192-row prefill chunk (single-request TTFT at 59k 7.94 → 7.84 s; resolved against a 17.6 ms control gap), no resolvable C20 throughput change (−0.42% vs the control mean inside a +2.46% control drift), ≈ −11 s of the 1511 s 100-prompt reference; retrieval 18/18 on all five arms. About half of the earlier −25 ms/chunk went to the merged 64-row tile + XCD-swizzle fmoe, which makes the call the fold adds pairs to cheaper. A broader accuracy eval (GSM8K plus long retrieval at C20) is the gate for any default flip. Emits `…expert_{weight,scale}_table_sf`; needs packet-stamped objects, the `plow_moe_shared_fold_arm` prefill object and the `plow_moe_aiter_nexp_abi_1` adapter. |
| `PLOW_GLM_MOE_SHARED_SEED` | `--glm-moe-shared-seed` | false | GLM TP8 prefill on the AITER MoE route. The shared expert's down projection writes the MoE seam's reduce-scatter source (`dg_tp`), `MoeAiterFp8Pf` mode 3 keeps it (prepare skips the zero fill; the adapter needs `plow_moe_aiter_keep_out_abi_1`) and the fused call accumulates the routed partials onto it, so no `MoeCombinePf` runs. Floor-bounded numerics (bf16 accumulation order). T3 on P8192-S: 648.5 / 641.4 / 648.8 ms (off / on / off2), −7.3 against a 2.3 ms floor; P4096-S −3.3; P8192-0 −8.6 (`glue2-t3-g4-rung`). |
| `PLOW_GLM_ROUTER_OVERLAP` | `--glm-router-overlap` | unset | GLM TP8 sequence-parallel prefill (2048/8192 buckets, interpreter band router). The router score reads the post-attention norm's band in peer slot 3, the bytes the hidden `XAllGather` copies into `xn2@band`, and depends on that norm instead of on the gather, so it runs while the gather is in flight. `1`: score and top-k are emitted ahead of the shared gate/up `GemmLtPf`, inside the attention-seam segment. `2`: the route-table gather and `MoeAlignPf` move there too. Bit-identical by construction (same kernels, same input bytes); no object change. Unset/`0` = byte-identical blob. Opt-in pending tier 3. |
| `PLOW_GLM_ROWBAND_ATTN` | `--glm-rowband-attn` | false | GLM-5.3 TP8 gfx942, 8192 prefill rung: the row-split sibling's replicated-projections arm. Each rank runs its `T/8` row band with all 64 heads: q_absorb (1024×32768×2048) and q_rope (1024×4096×2048) at full width from the band's q latents, the q RoPE at the band's positions, four 16-head `mla_dec_stage1_bf16_a16w16_subQ16_mqa16` launches, one native grouped fold (`plow_glm_fold_normalize_grouped`, adapter marker `plow_glm_fold_grouped_abi_1`) and o_proj (1024×6144×16384) straight into the band; no head all-to-all and no attention reduce-scatter, and q latents are all-gathered only on indexer layers. The runtime selects the sibling for full 8192 chunks whose bands all have a key plan: prior 0, 1023–1024 or ≥ 2047 (ranks below row 2047 read every causal key through the lower-half identity CSR). Excludes `PLOW_GLM_ROWSPLIT_ATTN` (checkpoint K constraint `rowband_attn_excludes_rowsplit_attn`). **Memory:** the full-width q_absorb, q_rope, o_proj and W_uv cost **29.25 GiB per rank measured** on GLM-5.3 (the earlier +16.9 GiB estimate was low by ~1.7x) (column shards bind as views into their full twins; o_proj and the FP32 fold copy do not); load refuses unless `free_bytes()` after load ≥ the replicated bytes + 8 GiB and logs `device memory after load` per rank. That comes out of the long-context KV headroom, free after load 83.44 → 55.58 GiB per rank measured (lh-t3-stage2-rowband); a default flip must weigh that. Floor-bounded numerics (different attention object, fold and GEMM accumulation order). T2 kernel price (lever-hunt T2-A, 1 GPU, prior 0): slowest band 1226 µs vs today's 1855 µs per layer, −49.0 ms per P8192-0 chunk. T3 (lh-t3-stage2-rowband, full depth): P8192-0 574.79 -> 465.06 ms (-110.23, floor 6.33) and P8192-S 650.85 -> 515.68 (-135.13, floor 3.95), both PASS; P4096-0 +0.78 ms against a 0.61 floor, not attributable -- the 4096 program is byte-identical between the two packets (all 5689 shared program keys identical; the row-band packet only adds 1414 rowsplit siblings). Served T4 pending. |
| `PLOW_GLM_ROWBAND` | `--glm-rowband` | false | **Runtime, read once at engine build.** Binds and dispatches the row-band arm carried by the packet. Serve-start only, never per request: the cost is the full-width `q_absorb`/`q_rope`/`o_proj` twins RESIDENT on every rank, so the knob decides *binding*, not just routing. Off, the twins are neither carved nor uploaded (each column shard carves its own storage, the ordinary path), the load-time free-VRAM refusal does not apply, and the siblings are never selected -- a packet carrying them then costs only their program records. This is what lets ONE packet serve both a latency profile and a capacity profile. Off, the twins are bound as a 1-byte view at address 0 so a stray read faults instead of silently returning another tensor. |
...
| `PLOW_GLM_SELECT_SPLIT` | `--glm-select-split` | unset (off) | Select each batched GLM decode row's DSA top-k in three gated packets (`IndexSelect` `i[4] = 2`, `i[6]` = phase): (1) up to N workgroups per row histogram the top 12 key bits of their chunk into the row's strip; (2) the same N find the boundary bin, write their chunk of a per-row position bitmap (keys above it) and append boundary-bin keys to a per-row candidate list; (3) one workgroup per row picks the remaining top candidates by key, compacts the bitmap into ascending positions and leaves the strips zeroed. N is capped so rows × N fits the 304 CUs (15 at rung 20). No workgroup waits on another inside a packet (the rule: under the global queue a decode op must not spin-wait across workgroups; a first form with an in-packet grid barrier hung intermittently at rung 20). Same packed key as the local form, so the selected set is identical; the list is in ascending position order on every run. `act.ighist` / `act.igctl` grow to per-row strips and `act.ibits` / `act.icand` are added. `runtime/bench/interp/dsa_select_split_test.c` gates the list against a host top-k. Needs `PLOW_GLM_SELECT_LOCAL` and decode objects built with `PLOW_DSA_SELECT_SPLIT=1`, which `build_gfx942.sh` sets from the packet's `requires`; plowrt refuses the packet on an object without `plow_dsa_select_split_arm`. Decode rungs only. Opt-in pending its tier 3. |
...
| `PLOW_GLM_GEMM_LT_PF_EXT` | `--glm-gemm-lt-pf-ext` | `o_proj,band,shared` on GLM gfx942 TP8 (production default), unset (off) elsewhere | Widens `PLOW_GLM_GEMM_LT` to prefill GEMMs it leaves on interpreter tiles: `1` = all, or a comma list of `o_proj`, `shared` (shared expert gate and up as two GEMMs plus a `Glu`, then down), `router` (the band router score) and `band` (the sequence-parallel band's q_a, kv_a, k_rope and indexer k/weights at `t/8` rows, `GemmLtPf` `i3=2`). Same pinned object; one kernel per shape and row tier from a sweep of all 456 256-thread kernels (`runtime/bench/amd/glm_projection/sweep.c`, job 0-1789215357). Standalone vs the traced in-flow tiles: o_proj 594 → 361 µs, shared gate+up 356 → 2×98, down 148 → 94; band at 1024 rows q_a 120 → 94, kv_a 114 → 48, k_rope 78 → 27, router 126 → 36, indexer k 94 → 31, weights 69 → 22. Every moved GEMM is its own segment: 625 → 1432 segments per 78-layer 8192 chunk (all groups). Tier 3 (job 0-1789219022, `--layers 4/8`, own objects, two interleaved rounds): steady 8192 chunk −1.24 ms (L4) and −3.08 ms (L8), −0.46 ms per MoE layer, ≈ −35 ms per 78-layer chunk; per group at L4: o_proj −0.72, band −0.46, shared and router within noise (one MoE layer; the shared GLU used to overlap the router → align chain inside one interpreter segment). Follow-up at L8 in one interleaved job (0-1789221845): all groups −3.29 ms, `o_proj,band` −2.53, `o_proj,band,shared` −2.86, so shared ≈ −5 ms and router ≈ −6.5 ms per 78-layer chunk. Value level against the cross-process floor (rank-0 act.xn, last-token logits, route table, band router logits, slot-0 KV rows via `PLOW_DUMP_KV`): `o_proj,band,shared` sits at the floor (logits 1.04×, act.xn 0.99×, top-8 set flips at MoE layer 7 17.05% vs 16.97%, layer-7 KV 1.10×). Layer 0 is bit-identical across processes and the band route moves its KV at the rounding level (95.1% of fp8 latent bytes identical, rel-L2 3.6e-4). With `router` the L8 logits sit 5-6× above the floor in two jobs, so it stays out of the recommended set: **`o_proj,band,shared`**, ≈ −29 ms per 78-layer 8192 chunk. Inert without `PLOW_GLM_GEMM_LT`; off is byte-identical (production packet 0xa6e7db0b3b83e581). The qualified default leaves `router` out: it was the only group that moved the final logits past the cross-process floor (5.4x at 8 layers, twice). Rollback: `--glm-gemm-lt-pf-ext ''` (or name fewer groups). **Tier 4 (job `1789228962-combined-t4`, batched with the other three levers, one plowrt from 79ca4cc3, ctrl / treat / ctrl2, 20 x 70k/700 at C20):** out tok/s 75.63 / 81.65 / 76.26 (+7.5% vs the control mean, controls 0.8% apart); full 8192 chunk 703.9 / 654.2 / 698.9 ms (-6.8%); tail chunks 529.8 / 444.3 / 529.0 ms (-16.0%); decode-only tick 62.7 / 60.8 / 62.8 ms (-3.2%); retrieval 18/18 base + 21/21 tail in every arm; VRAM floor unchanged at 3.55 GiB.|
| `PLOW_EMIT_PREFILL_CUBLASLT` | `--emit-prefill-cublaslt` | false; **production default true for Gemma-4 BF16 on sm_90a TP1** | Route the eligible dense prefill projections (`plow-asset::segment_roles::cublaslt_prefill_bf16`: every Gemma-4 projection shape at every rung 128–8192 (2026-09-17: at 128 rows the native object still cost ~50 µs per q/k/v/o launch); put the toolkit's `lib64` on `LD_LIBRARY_PATH`. Exact-shape algorithms are pinned from `<assets>/cublaslt_algos.jsonl` when present (`PLOW_LT_ALGOS`; produced by `PLOW_LT_ALGOS_WRITE` or the campaign `probe`, packetized by plowc from `tuning/nvidia/<arch>/*/cublaslt_algos.jsonl`), else selected at load. `=0` is the rollback. |
| `PLOW_EMIT_MOE_PF_LT` | `--emit-moe-pf-lt` | false; **production default true for BF16 Gemma-4 hybrid attention with `moe` on sm_90a TP1** | Isolate each layer's grouped MoE prefill pair (`MoeGroupGluGemmaPf` + `MoeGroupDownGemmaPf`) as a `MOE_PREFILL_CUBLASLT` segment (role 16; `dense_cublaslt::apply_moe_prefill`). Align, router and combine stay in their interpreter segment, so a runtime with `PLOW_MOE_PF_LT=0` runs the packet unchanged; the segment build script adds the glue object `interp_sm90a_moe_lt.cubin` for such packets. sm_90a only. 26B-A4B: segment 0.744 → 0.569 ms at 1024 rows, TTFT 1024/C1 43.0 → 38.7 ms. `=0` is the rollback. |
| `PLOW_EMIT_MOE_DEC_LT` | `--emit-moe-dec-lt` | false; **production default true for BF16 Gemma-4 hybrid attention with `moe` on sm_90a TP1** | Isolate each layer's fused expert pair (`MoeExpertGluNormGemma` + `MoeExpertDownGemma`) on every decode rung that runs the grouped arm (`PLOW_GEMMA_MOE_DEC_GROUP`, required) as a `MOE_DECODE_CUBLASLT` segment (role 17; `dense_cublaslt::apply_moe_decode`). Align, router and combine stay in their interpreter segment; a runtime with `PLOW_MOE_DEC_LT=0` runs every rung as one launch over the merged window, so the packet is unchanged for it. Needs the ABI 2 glue object `interp_sm90a_moe_lt.cubin`. sm_90a only. `=0` is the rollback. |
| `PLOW_GEMMA_MOE_DEC_GROUP` | `--gemma-moe-dec-group` | unset; **production default 4 for BF16 Gemma-4 hybrid attention with `moe` on sm_90a TP1** | Decode rungs with at least this many rows run the routed experts through the grouped tensor-core GEMMs instead of the per-slot GEMV walk. 26B-A4B step_bench ctx 1024 B=4/8/16: 9.61/12.75/15.28 ms (8, in-tree) → 8.73/10.15/12.57 (4 + `PLOW_EMIT_MOE_DEC_LT`). `=0` is the rollback (drops the `PLOW_EMIT_MOE_DEC_LT` default with it). |
| `PLOW_GEMMA_GEMM_LT` | `--emit-gemma-gemm-lt` | false | Qualified gfx942 TP1 Gemma-4-31B BF16 serial `o_proj`/`down_proj` kernels at the 2048/4096/8192 prefill rungs. Exact shape routing keeps slower Q/K/V, fused GLU, 2048 global `o_proj`, and unsupported rows on native Plow kernels. Requires the two objects produced by `scripts/build_gemma_lt.sh`. |
...
| `PLOW_GLM_FOLD_LT` | `--glm-fold-lt` | on for GLM on gfx942 TP8 (production default), off elsewhere | Native gfx942 FP32 MLA fold during prefill (buckets >= 2048). **Tier 3 (L4 TP8 packet, ctrl/fold/ctrl2/fold2 through the production mux): steady 8192 chunk 50.70 -> 49.67 ms (-1.03 ms per 4 layers, 5x the arms' 0.16/0.23 ms spread; TTFT at 73,728 tokens -9.2 ms) = -0.26 ms/layer, ~ -20 ms per full 78-layer chunk; decode ticks unchanged.** Each layer's `MlaMergeFold` (the W_uv fold, ~545 us/layer on the interpreter's VALU map) runs isolated as normalize -> pinned hipBLASLt FP32 GEMM (`glm_fold_lt_gfx942.elf`) -> bf16 convert. Packet: only the 78 folds of the 2048/8192 programs change (one block, `i[5]=1`), one extra segment per layer; pairing hash unchanged. Numerics: not bit-identical, 0.03% of bf16 outputs move by 1 ulp, rel-L2 vs FP64 unchanged (1.66e-3). Needs `glm_fold_lt_gfx942.elf` + `glm_fold_adapter.elf`, which `scripts/build_glm53_gfx942_serving_objects.sh` builds when the packet was emitted with the knob (VENDOR_DIR must hold the pinned image); +~504 MiB/rank HBM. Served on an older base: +0.49% with P99 TPOT +3.6% (one pair). **Tier 4 (job `0-1789204585-sp-proj-fold-t4`, batched with `PLOW_GLM_SEQ_PAR_PROJ`, one plowrt from 008dda69, ctrl / treat / ctrl2, 20 × 70k/700 at C20):** out tok/s 63.71 / 68.13 / 63.96 (+6.73% vs the control mean, controls 0.39% apart); full 8192 chunk 792.3 / 703.4 / 792.7 ms (−11.2%); prefill tick p50 −11.0%; decode-only tick unchanged (−0.4%, spread 0.8%); mean TTFT −9.3%; retrieval 18/18 base + 21/21 tail in every arm; VRAM floor 4.50 → 3.55 GiB free on the tightest card (the pair costs ~0.95 GiB/rank of HBM against the C20 × 64K headroom). Rollback: `--glm-fold-lt=false` (recorded as `cli`, and a `--replay-knobs` recipe keeps its own value — the frozen serving recipe records `false`, so drop it from a replayed recipe to take the default). |
| `PLOW_GLM_FOLD_LT_DECODE` | — | removed | Removed: measured +2.2 ms/step at rung 20 (tier 2, job 0-1789201138); see 160c6b2e to resurrect. |
...
| `PLOW_GLM_PLACE_PF` | `--glm-place-pf` | true | Per-XCD CU placement for the GLM prefill chain (`=0` rolls back). |
...
| `PLOW_GLM_SEQ_PAR` | `--glm-seq-par` | on for GLM on gfx942 TP8 (production default) unless `PLOW_GLM_XR_RES` / `PLOW_GLM_XR_BAND` is set, off elsewhere | Sequence-parallel TP seams for the GLM prefill buckets of 2048 and 8192 rows. Each o_proj and FFN all-reduce (`XReduceTwoShot`) becomes a reduce-scatter (`XReduceScatter`, op 25). The residual add and the following RMSNorm (the next layer's input norm, the post-attention norm, the final norm) run on this rank's `t/8` row band, and the normed rows are all-gathered (`XAllGather`, op 26) out of peer slot 3 (`act.h2_tp`). Between seams the residual stream lives only on the band. Same kernels and reduction order as the two-shot, so the gathered activations are bit-identical. The 128/512 rungs, decode and GLM-5.3-Next's hyperconnection wrap are unchanged. Peer region grows from 3 to 6 slots (+302 MB/rank at 8192 rows). Packets require `PLOW_SEQ_PAR_SEAMS=1` (packet-stamped objects compile the arm from `plow_config.h`). Refuses `PLOW_GLM_XR_RES` / `PLOW_GLM_XR_BAND`. Under `PLOW_RAGGED_CHUNK` the band packets keep their full band and the split collectives move the whole bucket, unless `PLOW_AMD_RAGGED_SEAMS` (below). Alone it is ≈0 at steady state (tier 3: −0.04 ms/layer); it is the prerequisite of `PLOW_GLM_SEQ_PAR_PROJ`, which carries the gain. Rollback: `--glm-seq-par=false` (also rolls back `_PROJ`'s default; recorded as `cli`, and a `--replay-knobs` recipe keeps its own value). |
| `PLOW_GLM_SEQ_PAR_PROJ` | `--glm-seq-par-proj` | on for GLM on gfx942 TP8 where `PLOW_GLM_SEQ_PAR` resolves on (production default), off elsewhere | Extends `PLOW_GLM_SEQ_PAR` (set both) to the layer-input seam: the input norm, q_a and its norm, kv_a and k_rope run on the band, and one all-gather carries q_a-normed / kv_a / k_rope (2624 of 6144 columns) out of peer slots 3/4/5 instead of the normed hidden. On a full-indexer layer the indexer's k and weights projections also run on the band, and a second gather carries them out of slots 2 (`act.ug_tp`) and 0. At the FFN input the router score and top-k also run on the band and the route table is gathered out of slot 5 (not under `PLOW_MOE_PF_DET` without the native MoE). The band GEMMs are interpreter tiles at `t/8` rows (hipBLASLt is pinned to `M = t`), so not bit-identical. Falls back to the `PLOW_GLM_SEQ_PAR` layer entry under `PLOW_GLM_GEMM_BLK`, a pooled indexer, or a sparse layer without `PLOW_GLM_INDEX_TP`. Tier 3 (`--layers 8` TP8): −0.87 ms/layer on the steady 8192 chunk, ≈ −68 ms per 78-layer chunk; numerics inside the two-process floor, same argmax. **Tier 4 (job `0-1789204585-sp-proj-fold-t4`, batched with `PLOW_GLM_SEQ_PAR_PROJ`, one plowrt from 008dda69, ctrl / treat / ctrl2, 20 × 70k/700 at C20):** out tok/s 63.71 / 68.13 / 63.96 (+6.73% vs the control mean, controls 0.39% apart); full 8192 chunk 792.3 / 703.4 / 792.7 ms (−11.2%); prefill tick p50 −11.0%; decode-only tick unchanged (−0.4%, spread 0.8%); mean TTFT −9.3%; retrieval 18/18 base + 21/21 tail in every arm; VRAM floor 4.50 → 3.55 GiB free on the tightest card (the pair costs ~0.95 GiB/rank of HBM against the C20 × 64K headroom). Rollback: `--glm-seq-par-proj=false` (recorded as `cli`, and a `--replay-knobs` recipe keeps its own value). |
| `PLOW_AMD_RAGGED_SEAMS` | `--amd-ragged-seams` | on (production default) | Runtime: size a ragged chunk's sequence-parallel seams by its live rows instead of the bucket. With `clen` live rows each rank's band becomes `b = ceil(clen / tp)` rows (at least 1). The reduce-scatter / all-gather counts go from `T` to `tp·b` rows, so each rank's slice stays its band's whole rows, and every band packet goes from `T/tp` rows to `b`. The `@band` views are rebound per chunk in the tensor-pointer table to `base + rank·b·row`, and a full chunk restores the load-time binding. The packet and objects do not change, and it acts only under `PLOW_RAGGED_CHUNK` on a packet that carries the seams. Opt-in; off leaves every patch and binding as before. Rollback: `--amd-ragged-seams=0`, which moves the whole bucket whatever the live rows. **Tier 4 (job `1789228962-combined-t4`, batched with the other three levers, one plowrt from 79ca4cc3, ctrl / treat / ctrl2, 20 x 70k/700 at C20):** out tok/s 75.63 / 81.65 / 76.26 (+7.5% vs the control mean, controls 0.8% apart); full 8192 chunk 703.9 / 654.2 / 698.9 ms (-6.8%); tail chunks 529.8 / 444.3 / 529.0 ms (-16.0%); decode-only tick 62.7 / 60.8 / 62.8 ms (-3.2%); retrieval 18/18 base + 21/21 tail in every arm; VRAM floor unchanged at 3.55 GiB.|
| `PLOW_GLM_DECODE_GLUE_CUS` | `--glm-decode-glue-cus` | false | Size the batched-decode glue packets to their work: the FP8 latent KV writer at one wave per row (was one workgroup for the whole rung), the router top-k at one workgroup per token, the MoE combine at one thread per element (were all 304 workgroups), and the DSA indexer's key LayerNorm at one workgroup per row on its own slice (was one workgroup for the whole rung). Pure width changes, bit-identical; opt-in pending the C20 A/B. |
| `PLOW_GLM_DECODE_GEMM_GROUP` | `--glm-decode-gemm-group` | on for GLM gfx942 TP8 (production default), off elsewhere | Batched decode (rungs with native `GemmLtPf`): emit the GEMMs that share an input next to each other — the shared gate/up straight after the router GEMM (top-k and Glu then share one interpreter segment), and on full-indexer layers the indexer k/weights projections beside q_a/kv_a/k_rope and its q projection beside q_absorb/q_rope. Same instructions and dependencies, reordered; pairing hash and objects unchanged; knob-off packet byte-identical. Rung-20 chain 1431 → 1293 packets. Needs no objects or vendor images of its own (same pairing as knob-off). **Tier 3** on the NUMA + VRAM-kernarg defaults (job `0-1789217373-decode-grp-L7`: 7-layer TP8, 6 processes per arm interleaved, socket-0 pinned, rung 20 at 65K): 7.073 → 6.832 ms/step (−0.241 ms, −3.4 %, spreads 0.033 / 0.045 ms); rank-0 interpreter segments per step 47 → 31; class-weighted 78-layer projection (3 dense-full + 57 MoE-plain + 18 MoE-full) −1.75 ms/step ≈ −6.9 s per 3946-pass run. The earlier unresolved result was per-process bimodality, which those defaults removed. Opt-in pending the combined tier 4. Rollback: `--glm-decode-gemm-group=false`. **Tier 4 (job `1789228962-combined-t4`, batched with the other three levers, one plowrt from 79ca4cc3, ctrl / treat / ctrl2, 20 x 70k/700 at C20):** out tok/s 75.63 / 81.65 / 76.26 (+7.5% vs the control mean, controls 0.8% apart); full 8192 chunk 703.9 / 654.2 / 698.9 ms (-6.8%); tail chunks 529.8 / 444.3 / 529.0 ms (-16.0%); decode-only tick 62.7 / 60.8 / 62.8 ms (-3.2%); retrieval 18/18 base + 21/21 tail in every arm; VRAM floor unchanged at 3.55 GiB.|
...
Fourteen `glm_*` knobs are ON by default when the model is GLM (`glm_moe_dsa` / `glm5_next`), the
...
`PLOW_GLM_GEMM_LT_DECODE_EXT` (the ninth, added once served: +1.9%, retrieval 18/18), and the pair
qualified together in one tier 4: `PLOW_GLM_SEQ_PAR` + `PLOW_GLM_SEQ_PAR_PROJ` (sequence-parallel
seams with the band projections) and `PLOW_GLM_FOLD_LT` (native W_uv prefill fold) — +6.73% out
tok/s against a 0.39% control spread, −11.2% per full 8192 chunk, retrieval 18/18 + 21/21, at
~0.95 GiB/rank more HBM (VRAM floor 4.50 → 3.55 GiB free on the tightest card at C20 × 64K).
`PLOW_GLM_SEQ_PAR`'s default stands aside when `PLOW_GLM_XR_RES` / `PLOW_GLM_XR_BAND` is set, and
`_PROJ`'s follows `PLOW_GLM_SEQ_PAR`. Two more joined through one combined tier 4 (review log #69:
+7.5% out tok/s, full 8192 chunk −6.8%, tails −16%, decode-only tick −3.2%, retrieval 18/18 + 21/21
in every arm): `PLOW_GLM_GEMM_LT_PF_EXT` (`o_proj,band,shared`; `router` stays out because it moved
the logits past the cross-process floor) and `PLOW_GLM_DECODE_GEMM_GROUP`. Off on every other
target, and off under `--mxfp4` (the native MoE arms are block-fp8 only).

That is the configuration serving GLM-5.3 on 8x MI300X — 86.67 out tok/s on the 100 × 70k/700 C20
benchmark, median TTFT 9.55 s, retrieval 39/39 (review log #70) — and each knob's own evidence is
in its row above. It was previously
...
recipe reproduces it. (The frozen serving recipe records `PLOW_GLM_FOLD_LT=false`: a
regeneration that should take the fold default must drop that entry from the replayed recipe.) `build.json` names the winner per knob (`cli` / `env` /
...
`PLOW_GLM_MLA_DEC_AITER`, `PLOW_GLM_PLACE_PF`, `PLOW_GLM_FUSE_ROPE`,
...
| `PLOW_XR_SCHED` / `PLOW_XR_SCHED_NWG` / `PLOW_XR_SCHED_NWG_RS` / `PLOW_XR_SCHED_AG_U` | **aiter (gfx942 prefill rows)** / 24 / 8 / 1 | **objects** (`scripts/build_gfx942.sh`, prefill rows) — `aiter` builds the strict-rank-order 16-byte prefill collective schedule (`-DPLOW_XR_SCHED_AITER=1`) on the first `NWG` workgroups of each `XReduceTwoShot` / `XReduceScatter` / `XAllGather` packet, the two-shot's reduce-scatter on the first `NWG_RS`; the other workgroups take only their `gate_ag` arrival and leave, so the packet (304 workgroups) is unchanged. `AG_U` = all-gather packs in flight per lane. The objects export `plow_xr_sched_aiter_1`, `plow_xr_sched_nwg` and `plow_xr_sched_nwg_rs` (u32 values = the caps). Bit-identical (host strict-order oracle, every bucket). 8x MI300X two-shot, 304 workgroups launched: 8192x6144 bf16 1002 → 728 µs (-27%, 181 → 242 GB/s effective; one shared cap of 24 gives 795), 2048 rows 289 → 186, 512 rows 119 → 76, 128 rows 68 → 42 µs. In model (4-layer TP8 packet, last 8192 chunk at prior 65536, bracketed by two controls): 1.06 → 0.92 ms per collective (rank 0; rank 4 0.97 → 0.79) with the defaults, 1.08 → 0.99 ms with one cap of 24; projected −23 to −31 ms per 78-layer 8192 chunk. Decode is not affected (decode rungs emit only the one-shot `XReduce`). **Default on** in `build_gfx942.sh` after the served A/B (credited as a pair with `PLOW_AMD_NUMA_HOST_POOLS`: full 8192 chunks 823.4 / 793.9 / 824.4 ms ctrl / treat / ctrl2, −29.9 ms per chunk against a 1.0 ms control gap; prefill tick p50 888.8 / 857.0 / 890.3 ms; +3.66 % out tok/s against a 1.14 % control spread; retrieval 18/18 + 21/21 in every arm); rollback `PLOW_XR_SCHED=twoshot` (`off` is an alias), which compiles none of it. The CMake option (the gfx950 path) keeps `off`. `PLOW_XR_TRACE_PHASES=1` (diagnostic) also reaches the decode objects. |
| `PLOW_XR_SCHED_NWG_SRS` / `PLOW_XR_SCHED_NWG_SAG` | **8** / unset (= `PLOW_XR_SCHED_NWG`, 24) | **objects** (`scripts/build_gfx942.sh`, prefill rows, with `PLOW_XR_SCHED=aiter`) — separate data-workgroup caps for the sequence-parallel seam halves, op 25 `XReduceScatter` (`_SRS`) and op 26 `XAllGather` (`_SAG`); the objects export `plow_xr_sched_nwg_srs` / `_sag`. The seam reduce-scatter runs on 8 workgroups by default (micro5, 8x MI300X, 304 launched: 8192x6144 394.5 → ~347 µs, 2048x6144 108.2 → 93.9 µs; in model, 4-layer TP8 seams packet, ctl / treat / ctl2: 73,728-row prefill 393.5 / 388.0 / 393.8 ms, per-reduce-scatter −63 µs (mean over ranks) to −92 µs (min), ≈ −10 to −14 ms per 78-layer 8192 chunk; logits within the control-vs-control spread); rollback `PLOW_XR_SCHED_NWG_SRS=24`, the cap it used before. No all-gather cap below 24 pays (16: +134 µs, 8: +512 µs at 8192x6144), so `_SAG` stays unset. Bit-identical (strict-order oracle). |
| `PLOW_DSA_TP_SELECT_THR` | 0 | **objects** (`scripts/build_dsa_tp.sh OUT -DPLOW_DSA_TP_SELECT_THR=1`) — also export `plow_dsa_tp_select_thr` from `dsa_tp_adapter_gfx942.elf`: the TP indexer's selection as `d_index_select_pf_thr` (12-bit digits until the boundary bin holds ≤ 2048 keys, one collect scan, a byte radix over those keys in LDS; two row scans instead of five, the same `dsa_pack_key_a` set). Launched only under `PLOW_DSA_SELECT_THRESHOLD`; off, the adapter disassembly is unchanged. |
| `PLOW_DSA_SELECT_SPLIT` | 0 | **objects** (`scripts/build_gfx942.sh`, decode and flash rows) — compile the split-row decode selection arm (`-DPLOW_DSA_SELECT_SPLIT=1`, op 59 `i[4] = 2`) and export `plow_dsa_select_split_arm`. Set automatically from the `requires` of a packet emitted with `PLOW_GLM_SELECT_SPLIT`; off, the objects are byte-unchanged. |
...
`scripts/build_sm90a_gemma4_segments.sh` (the Gemma-4 H100 campaign objects) defaults a BF16
packet (no `PLOW_HAS_QUANT_FP8` in its `plow_config.h`) to the recipe object set:
`PLOW_BUILD_FATLITE`, `PLOW_BUILD_MASKED_PADDING` (packed objects + the WGMMA BQ64 hd512 role
object, `PLOW_BUILD_PFATTN_KV64=0`, `PLOW_BUILD_PFATTN_QK_UNROLL=4`), `PLOW_BUILD_FA_GQA2_PAIR`,
`PLOW_BUILD_PFATTN_HD256_BKV32` and `PLOW_BUILD_PFATTN_HD256_GQA2_BKV32` all on;
...
| `PLOW_PF_INTERLEAVE_ADAPTIVE=1` | off | **CUDA packed prefill: size each prefill launch from the queue.** The oldest waiting prompt runs whole; prompt `j + 1` (r rows) joins its launch only while `j * r < (n - j) * PLOW_PF_CHUNK_COST`, i.e. while the delay to the `j` prompts already packed is under the fixed launch costs the `n - j` others save. Short prompts share a launch, long prompts run alone oldest-first, a deeper queue packs more. A prompt no launch can hold whole still fills the launch, as the static budget does; a shorter one that does not fit waits and is never split. |
...
| `PLOW_PF_CHUNK_COST=R` | 512 | cost of ONE prefill launch in padded-row equivalents (`rows + R × launches`). A launch re-streams every layer's weights: measured `ttft_ms = 0.112·rows + 60.1·chunks`, i.e. **60 ms ≈ 537 rows**. **Re-fit it per part** — on the 12B H100 ladder packet the launch fixed cost measures **~5.2 ms against ~0.0398 ms per bucket row, i.e. ≈131 rows**, so the 512 default overcharges a launch there by ~4× and biases the pick toward wide padded covers. `0` = pure-minimum-padding. |
| `PLOW_PF_COVER=1` | off | restore the covering-bucket prefill policy (exact-parity A/B vs the cost-aware default). Unset is the cost-aware DP cover (`pick_prefill_bucket`, `exec/gpu.rs`), which is the shipped default; `=1` is the rollback. |
...
| `PLOW_QUEUE_TTL_MS=<ms>` / `--queue-ttl-ms` | unset (`--slo-ms` × 40, at least 30 s) | **How long a request may wait for a slot before it is answered 429.** `0` never sheds a waiting request (vLLM behaviour: it queues). A request that can never fit the KV budget is still refused at once. At 70k context with admission seating 16 sequences, every request past the 16th waits for a whole generation to retire (≈150 s), so the default sheds it; a C32 70k cell lost 16/32 this way. Disconnected clients are swept either way. |
| `PLOW_TTFT_SLO_MS=<ms>` / `--ttft-slo-ms` | unset (arrival order) | **AMD prefill candidate order by TTFT deadline slack (EDF).** Slack = `ms - age - remaining rows x per-row cost` (per-row cost from the widest chunk at the request's prior). Order: prompts whose next chunk completes them with slack left, then the rest with slack left by slack, then requests that can no longer make the deadline by arrival. On its own it does not bind the chunk size (planned chunks run whole); with `PLOW_TBT_SLO_MS` both apply. A request meets TBT when its mean inter-token time (TPOT) is at or under `PLOW_TBT_SLO_MS`; goodput = requests meeting every set target. Not the admission `--slo-ms`, which sheds on predicted queue wait. |
...
| `PLOW_VMM_PREFIX=0/1` | auto | VMM-backed KV prefix cache. Automatic on eligible Hopper hybrid BF16-KV packets (HD256 sliding, HD512 full, window1024), including Gemma 4 31B BF16 and FP8 weights. Full blocks are shared; partial full-KV and sliding snapshots allow reuse at 32-token boundaries. `0` disables; `1` explicitly requests other supported layouts. Automatic selection excludes TP, recurrent state, mixed/prepared decode and explicit packed-prefill/live-KV modes. It takes precedence over packed-prefill metadata alone. Automatic selection is a geometry allowlist (the layout the sliding-ring snapshot path was qualified on), not a capability probe; the load log records `vmm prefix auto-selection` with the geometry and `VMM prefix cache selection` with `mode=auto|explicit|off`. `PLOW_PREFIX_CACHE=0` disables it regardless of this knob (no pool, no publishes, no radix tree, plain slot recycling). |
| `PLOW_VMM_PUBLISH_SHARED=0/1` / `--vmm-publish-shared` | 1 | CUDA VMM prefix cache: publish a boundary inside a block — a prompt's end or a generation's end, which only its own tail can reuse — only once the sequence's leading 32 tokens were seen on another sequence (an attach, or a request waiting on it in flight). Whole-block checkpoints (every 2048 rows at hd512 BF16) publish on the first sighting: they carry a shared system prompt to its second request, and a prompt shorter than one block has none. A unique-prompt workload then pays at most one window copy per 2048 prefilled rows (none below 2048); a replayed prompt or a multi-turn continuation publishes its tailed ends from its second sighting on. `publishes_skipped` in the pool stats counts the skips. `0` publishes every boundary of every sequence (~2 × 320 MiB of extra copies per request on Gemma 4 12B). |
| `PLOW_PREFIX_CACHE_OUTPUT=0/1` / `--prefix-cache-output` | 1 | CUDA VMM prefix cache: publish a finished sequence's end-of-generation boundary (prompt + generated rows: one sliding-window snapshot buffer, ~320 MiB on Gemma-4-12B, and its D2D copy on the engine thread at slot retirement), so a follow-up turn that embeds this turn's output attaches to it (measured: a 7230-token repeat 404 -> 32 ms). `0` skips that publish for single-turn traffic; the prompt-end boundary and the whole-block checkpoints still publish, so a shared system prompt and an exact prompt replay hit as before. |
| `PLOW_PREFIX_INFLIGHT_WAIT=0/1` / `--prefix-inflight-wait` | 1 | CUDA packed prefill: a request whose prompt shares whole KV blocks (2048 rows at hd512 BF16) with a prompt another slot is still prefilling is held at admission until that slot's prompt-end publish lands (or 5 s), then attaches to the checkpoint and prefills only its tail, instead of recomputing the shared rows in the same launch. The wait also counts as a sighting for `PLOW_VMM_PUBLISH_SHARED`, so the owner publishes. Log: `gpu: packed admission waiting on in-flight prefix`. `0` admits at once. |
| `PLOW_PREFIX_CHUNK_PUBLISH=0/1` / `--prefix-chunk-publish` | 1 | CUDA token-batch prefill, with the VMM prefix cache: publish the whole-block checkpoints (2048 rows at hd512 BF16) a prompt chunk completed as soon as that chunk lands, not only at the prompt's end, so a request held by `PLOW_PREFIX_INFLIGHT_WAIT` on a long prompt's shared leading blocks attaches after the first chunk that covers them instead of after the whole prompt. Each checkpoint is one sliding-window snapshot copy, the same one the prompt-end publish would make. Measured on Gemma-4-12B, 2048 shared + 6144 unique rows at C16: TTFT median 609 -> 405 ms, mean 1124 -> 1098, TPOT and tok/s within 1.5%; neutral within session spread on 2048 + 256 prompts (12B and 26B, C16, ABAB). `0` publishes them at the prompt's end only. |
...
| `PLOW_VMM_CACHE_MIN_FREE_MIB=M` | 4% of the device (CUDA: 10%, at most half of free-after-load; an eighth of it when the rings leave no room for 10% plus the static cache budget) | Pressure eviction: keep cached prefixes (shared blocks and boundary snapshots) until free device memory would drop below M MiB, instead of trimming at the static `PLOW_VMM_CACHE_MIB`/`_MEMORY_UTILIZATION` budget. Needs a backend that reports free device memory (HSA agent info `0xA015`; CUDA `cuMemGetInfo`, armed at load on the VMM prefix pool, log `vmm prefix cache: pressure eviction armed`); others keep the static budget. Unset derives 4% of the device (~7.7 GiB on a 192 GiB MI300X) rather than a fixed MiB figure that would be a third of a small card; on CUDA it derives 10% of the device — vLLM's headroom at its default `--gpu-memory-utilization 0.9`, so a cache-on server peaks where vLLM does — at most half of the memory free after the weights and KV rings loaded. When that free memory cannot hold the 10% plus the static cache budget (`PLOW_VMM_CACHE_MEMORY_UTILIZATION`), the packet is already at vLLM's peak without a cache, and the floor is an eighth of it instead. Gemma-4-12B leaves 14.2 GiB: floor 7.1 GiB, peak 73.7 GiB vs vLLM 75.3. Gemma-4-26B leaves 4.8 GiB: floor 602 MiB; its prefix-repetition C16 hits were 57/67 at 602 MiB, 52 at 764 and 8 at 2409, because a larger floor evicts the shared checkpoints. `=0` is the rollback to the static budget alone. |
| `PLOW_KV_ADMIT_HEADROOM=F` | 0.9 | **AMD** — fraction of the free-after-load device memory the mux may commit to admitted sequences' KV. Admission charges every live slot's reserved rows (prompt + `max_tokens`) at the packet's bytes-per-token and holds a request in the queue when it would not fit, instead of dispatching it into a device that cannot back it. Measured need: 20 x 70,000 rows wants 72.1 GiB where the load left 55.59 GiB free, which faulted the queue with `HSA_STATUS_ERROR_OUT_OF_RESOURCES`. A request larger than the whole budget is answered with a context-length error rather than queued forever. `0` is the rollback to slot-count-only admission. |
| `PLOW_AMD_PREFIX_FINE_ROWS=N` | unset | Share AMD prefix content below the 2 MiB VMM block floor in N-row chunks instead of leaving it a private copy. N must divide every cache group's block size. **CUDA** VMM prefix cache: the step of the intermediate checkpoints a publish records below the boundary; unset publishes one at every whole-block boundary (2048 rows at hd512 BF16 — a block-aligned boundary has no tail, so any prompt sharing those blocks attaches to it). Each checkpoint is a sliding-window snapshot (window x `snap_row_bytes`, 320 MiB on Gemma-4-12B), so a finer step multiplies that. |
| `PLOW_VMM_KV=0/1` | on | **AMD** — VMM-backed KV on ROCr (`hsa_amd_vmem_*`): reserve VA for `max_ctx`, map physical at each sequence's frontier; warns and falls back to the flat carve if the platform can't support it. Reached only when the shared-prefix pool is absent (`vmm_bringup` runs iff `shared_prefix.is_none()`), so with the prefix cache on — the default — this changes nothing; it makes the fallback path grow with context the same way. `=0` is the rollback. |
...
| `PLOW_IDLE_DISPATCH` | **on**; `=0` is the rollback | Mux: an arrival into an empty slot table skips the `--max-hold-ms` formation hold when no other request is queued or still inside a handler (tokenizing), so a lone C1 request dispatches at once. A burst's first arrival still sees its peers and holds as before; a request that reaches the handler after the check waits for the running prefill, as one arriving after the hold does today. Runtime only. |
| `PLOW_MUX_INLINE_TICK=0/1` | on for a CUDA engine, off otherwise | Mux: a GPU model's dispatcher runs on its own OS thread (a current-thread tokio runtime) and executes each tick inline, instead of sending the tick to the engine thread and awaiting it from a tokio worker. Removes two cross-thread wakes from every tick; nothing else in the loop changes, since it already waits for each tick before reading the queue. 26B-A4B H100, 128/128, `MULTISTEP=0`, ctl/on/ctl2/on2 (`PLOW_HOST_TIMING` handoff 50–60 → 0.3 µs/tick at C1, 83–87 → 0.3 at C16): TPOT C1 5.661/5.606/5.666/5.614 ms, C16 15.53/15.42/15.57/15.37 ms, C16 output 985/992/983/995 tok/s, TTFT unchanged. Runtime only. Default on for CUDA engines; AMD and CPU engines keep the engine thread. `=0` is the rollback. |
| `PLOW_ENCODE_FAST` | **on**; `=0` restores `encode` | Tokenize prompts with `encode_fast` (no offsets). Ids are identical to the default `encode`; GLM-5.3 CPU 8192-token prompt 12.3 → 11.1 ms. Runtime only. |
| `PLOW_ENCODE_THREADS=N` | 16 for a metaspace one-word BPE (Gemma), else unset | Tokenize a prompt of ≥ 8 KiB in up to N pieces on a dedicated N-thread pool, cut at a single ASCII space between two ASCII letters. Enabled only for a BPE tokenizer with no normalizer, truncation, padding, post-processor tokens, stripped added tokens, or dropout, and a `Split(Isolated)` + `ByteLevel(use_regex=false, add_prefix_space=false)` pre-tokenizer on the GLM or Qwen2 pattern, where such a space always starts a pre-token; any other tokenizer warns and stays serial. Ids identical to `encode` on 1420 GLM-5.3 prompts (vLLM random 1k–16k, chat template, adversarial). GLM-5.3 CPU 8192-token prompt 12.3 → 4.8 ms (N=4), 2.6 ms (N=8). A metaspace one-word BPE (space → `▁` normalizer, Split-on-space, no vocabulary token with an ASCII letter before `▁`: Gemma) splits by default with N=16 and `PLOW_ENCODE_SPLIT_MIN=512` (its default for that tokenizer; 4096 otherwise): Gemma-4 served TTFT 50.1 → 47.3 ms at 1024 in, 186.3 → 176.7 at 4096. `PLOW_ENCODE_THREADS=0` is the rollback. Runtime only. |
...
| `PLOW_MLA_PF_ROW_SPLIT=0` | on | AMD, with `PLOW_MLA_PF_AITER=1`: a sparse-rung prefill chunk starting below 2047 prior keys (a request's first chunk) no longer runs its whole attention on the interpreter gather arm. Rows from the first 8-aligned row holding all 2048 causal keys run the AITER sparse route (same indexer selection, `SparseMla::enqueue_split`); the earlier rows stay on the interpreter with the union and flash row count patched to the cut, so their per-8-query tiles and outputs are the unsplit ones. Runtime only: packet and objects unchanged. Default on since the served A/B on the default config (`pfroute-t4d`, review log #96) and checkpoint P (`perf-certs/rt.mla_pf_row_split.json`): 8192-row first chunk 730.6 -> 630.3 ms, 4096-row 388.9 -> 357.6 ms; served paired-median TTFT -98.5 ms at ISL 8192 C1 and -211.8 ms at C16. `=0` is the rollback. |
| `PLOW_MLA_PF_ROW_SPLIT_NATIVE_LO=0` | on | AMD, with `PLOW_MLA_PF_ROW_SPLIT=1` and the single-pass FP8 pack: the rows a row-split chunk leaves below the cut (fewer than 2048 causal keys) also run the AITER sparse kernel, each over all its causal keys through a ragged identity CSR (row `t` at prior `p` attends `[0, p + t]`), in its own launch before the native upper half. The cut becomes exactly the first 2048-key row (no 8-row alignment), the interpreter flash segment does not launch, and the union segments those flashes read are skipped. Only one split per row (ns=1) is admitted: the pinned kernel returns wrong values for ragged rows at two splits (T2 `pfroute-t2-ragged`: rel-L2 0.47-0.64 at ns=2 vs 1.4e-4-1.9e-4 at ns=1). The CSR (~8.4 MiB per rank) is rebuilt only when the chunk prior changes. Runtime only: packet and objects unchanged. T2 device clock: 2048 lower rows at ctx 8192 cost 316.6 us/layer (pack 16.0 + attention 300.6) vs 743.5 us interpreter in flow. Default on since the served A/B with row split on in every arm (`pfroute-t4lo`, review log #98) and checkpoint P (`perf-certs/rt.mla_pf_row_split_native_lo.json`): 8192-row first chunk 615.2 -> 575.1 ms, 4096-row 355.9 -> 327.0 ms; served paired-median TTFT -35.5 ms at ISL 8192 C1 and -80.9 ms at C16. `=0` is the rollback. |
...
| `PLOW_AMD_TAIL_SPARSE_CTX=<rows>` / `--amd-tail-sparse-ctx` | 8192 (`0` = off) | AMD: a request's DENSE final chunk whose prior context (prefix-cache resume + preceding chunks) is at least this many rows is planned into the sparse (DSA) prefill bucket instead of the smallest bucket that holds it; the chunk keeps its real row count, and its rows attend the top-2048 DSA selection instead of every prior key. GLM-5.3 TP8 at 65,536 prior (tier 3, d9fe7690): 271/698/1464-row tails 439/1429/1489 ms dense → 185/211/275 ms sparse, full chunks unchanged. Served, 100 × 70k/700/.14 at C20 (19 such tails), ABA on one binary: 64.95 / **66.56** / 65.17 out tok/s (+2.3 % vs the control mean; controls 0.34 % apart), the 19 tails 25.7 s → 4.3 s in the tick log. Retrieval: 18/18 base plus 21/21 short-tail cases (`quality.py --suite all`). No effect on packets without a sparse bucket; `0` restores smallest-bucket placement. **Lowered 16384 → 8192:** a final chunk at prior 8192 (prompts of 8,193-16,384 tokens) was still dense; on the sparse bucket it is 133-141 ms faster at 600-2048 rows (285.5/319.8/359.9/369.5 → 145.1/181.4/226.7/232.9 ms, controls within 0.6 ms), fresh prompts unchanged, whole-request TTFT for 8.8-10.2k-token prompts −132..−140 ms (job `rung2048-tail-ab`). Retrieval at floor 8192 with tails built at prior 8192: 39/39 (`rung2048-guard-tail8192`). |
...
| `PLOW_AMD_DECODE_MIN_RUNG=<slots>` / `--amd-decode-min-rung` | 8 (`1` = off) | AMD TP engines: never select a decode rung narrower than this, so a lone request decodes on the rung-8 program instead of rung 1. The wider rung still advances every occupied slot; the extra slots stay parked. GLM-5.3 TP8, ISL 8192 / OSL 1024 at C1, one binary: median TPOT 51.85 / **41.44** / 52.27 ms (ctrl / floor 8 / ctrl), output 18.9 → 23.7 tok/s, TTFT unchanged. Rung 1's one-row GEMVs run over all 304 workgroups (~106 µs each) while rung 8 uses native GEMM groups (20-34 µs); rung 8 then spends 21.8 ms of its 41 ms step enqueueing 10,944 launches. Teacher-forced per-step logits (128 steps, rank 0's bf16 vocab shard, row 0): r1 vs r8 relL2 p50 0.209, top-64 0.069, argmax 127/128, against same-code cross-process floors 0.206 / 0.238 (top-64 0.069 / 0.075). Single-GPU engines keep the plain ladder. |
...
| `PLOW_MULTISTEP_ADAPTIVE` / `--multistep-adaptive` | false | CUDA: a mux tick runs single-step while any admitted request still has prefill chunks left, a request waits for admission, or the previous tick freed a slot, and the full `PLOW_MULTISTEP` quantum once every live request is in pure decode — prefill chunks no longer wait behind K-step quanta and closed-loop completions do not batch into arrival waves, while decode-only stretches keep the quantum. |
...
| `PLOW_DECODE_PIPELINE` / `--decode-pipeline` | false | CUDA lookahead-1 decode: each tick enqueues the next step (inputs stay on the device: `ARGMAX_FIN` writes `in.ids`, `plow_advance` moves `pos`/`kvlen`) before waiting on the step in flight, so streaming, stop checks and scheduling overlap the device and every token streams as its step completes. Needs what the device multi-step needs (dynamic KV row, sampler object, greedy rows) and replaces the K-step quantum; a tick with prefill work, a changed row set or a host-sampled row completes the in-flight step first. A stop the host cannot foresee costs one discarded step. |
| `PLOW_PIPE_PREFILL` / `--pipe-prefill` | 0 | **Experimental, does not work yet — leave at 0.** With `PLOW_DECODE_PIPELINE` on, the mixed prefill/decode launch is enqueued behind the step in flight instead of reading that step out first: decode rows whose newest token the pipe still owes take it from the device (`d_last`, written by each launch's samples) rather than from the host, and `=2` additionally lets a prompt decode from its device-resident first token. Correct on greedy probes and on 128/C1, 128/C4 and 15000/C1 of the ladder, but **`=1` faults when rows retire while a mixed launch is parked**: `decode pipeline: step failed ... fed=2 ... CUDA_ERROR_ILLEGAL_ADDRESS` at `cuEventSynchronize`, reproduced with 4 x 14k-token prompts immediately after `decode_rows` fell 4 -> 2, and at 15000/C4 of the ladder (there the poisoned context left the client waiting with the GPU pinned). `=2` faults too, in the batched prefill. The open lead is the retired-row frontier rollback in `pipe_complete` (`self.pos[b] -= 1`), the one piece of `pos` bookkeeping that fires exactly on retirement. Its measured ceiling is small either way: the host gap it closes is 1.0 % of a prefill tick and 0.5 % of a decode tick (`PLOW_PF_PACKLOG`, 659 ticks). |
| `PLOW_SERVE_POLICY` / `--serve-policy` | pinned | `auto` chooses the campaign's serving profile per tick from live decode width and queue depth instead of fixing one for the run: high-concurrency at >=8 occupied rows or a non-empty queue, realtime at <=4, with the band between them as hysteresis and a 200-tick dwell (~2-3 s) before any switch; one log line per switch. It moves the knobs read at a per-tick decision point — prefill packing (`PLOW_PF_INTERLEAVE_ADAPTIVE`), the rung fast probe, and the queue TTL. `PLOW_DECODE_MAX_RUNG` is not one of them: it filters the decode rung ladder and caps slot capacity at startup, and the KV admission check sizes its reservation from it, so under `auto` the ladder is left whole and the rung controller widens on measured service times — which reserves KV for the full slot count. Unset (pinned) is what certs and campaign cells run. |
...
| `PLOW_UPLOAD_SLOTS=N` | 1 | AMD upload-ring pipeline depth; `1` = the pre-pipeline one-slab shape. Values above one are **experimental on ROCm** — concurrent copies into one large allocation fault on current gfx950 drivers. |
...
| `PLOW_TP_PREFILL_SEGMENT_MAJOR=0/1` (`--amd-tp-prefill-segment-major`) | on | **AMD TP prefill** — enqueue a chunk's segments in segment-major rank order and drain once per chunk, rather than after every segment. `=0` is the rollback. **`PLOW_PREFILL_SEG_TIMING=1` silently turns this off** (it adds an all-rank barrier per segment for timing), so a prefill measurement taken with segment timing on is not comparable to one taken without: on an 8-layer GLM packet a 65,536-token prefill measured ~593 ms with it and ~572 ms without. Undocumented until the flags audit. |
| `PLOW_CTR_DBUF=0/1` (`--amd-ctr-dbuf`) | on | **AMD** — counter double-buffering. `--amd-ctr-dbuf=false` disables. Undocumented until the flags audit. |
| `PLOW_L2_PLACE_PREFILL=0/1` (`--l2-place-prefill`) | on | **Emit** — apply the requested L2 placement to prefill programs as well as decode; the prefill counterpart of `PLOW_L2_PLACE`. Undocumented until the flags audit. |
| `PLOW_AMD_DECODE_GEMM_OVERLAP=0/1` (`--amd-decode-gemm-overlap`) | **not in this tree** | **Not implemented on this branch: nothing here reads this flag, so setting it does nothing.** The code lives on the unmerged branch `decode-latency`; the row is kept for its measured result. **AMD decode** — dispatch a native decode GEMM without the AQL barrier bit when it directly follows native GEMMs it is independent of (byte ranges planned at load, `amd_gemm_lt::overlap_barriers`). Bit-identical by construction. **Measured loss, keep off:** 7-layer TP8, 14 runs, median +1.5 ms/step (+14 %); `PLOW_TRACE_RAW` shows the native GEMM runs do not shorten and, on the grouped packet, every native interval grows 40–50 µs. Code on branch `decode-latency`. |
| `PLOW_AMD_NUMA_HOST_POOLS=0/1` (`--amd-numa-host-pools`) | on | **AMD** — take each rank's kernarg ring and fine-grained host staging (pinned upload/readback buffers, fill and zero stages) from the CPU agent nearest its GPU (`HSA_AMD_AGENT_INFO_NEAREST_CPU`) instead of the first CPU agent, so a GPU on the far socket reads every dispatch's kernargs from local memory; the host writer's cross-socket stores are posted. No fine-grained allocation is shared across ranks (peer buffers are VRAM). Every load logs one `HSA host placement` INFO line per rank: chosen/first/nearest CPU node and the page nodes (`get_mempolicy`) of the kernarg ring, AQL ring and completion signal. `=0` / `--amd-numa-host-pools=false` = the single-agent placement (rollback). **Tier 3** (7-layer TP8, 6 processes per arm interleaved): 12.576 → 9.665 ms/step median (−23 %), spread 1.8 → 0.23 ms (the per-process slow level is gone), ranks 0–3 wait inside the one-shot XReduce 196 → 39 µs; projected ≈ −25 ms per 78-layer rung-20 tick. **Tier 4** (combined with the collective RS8/AG24 schedule, tail-sparse on in every arm; 20 × 70k/700, C20): +3.66 % out tok/s against the control mean (controls 1.14 % apart), decode-only tick p50 89.6 / 87.7 / 92.1 ms (−3.4 %, just outside the 2.7 % control spread), retrieval 18/18 + 21/21 in every arm; under `serve` ranks 4–7 get kernarg pages on nodes 7/7/4/7. The gain is credited to the pair; NUMA's own full-model decode share is unresolved (serve pins its engine thread to rank 0's socket, so its control never had the tier-3 slow level). |
| `PLOW_AMD_KERNARG_VRAM=0/1` (`--amd-kernarg-vram`) | on | **AMD** — put each rank's kernarg ring in its own GPU's VRAM through the large-BAR host mapping (prefaulted every 64 KiB), so the GPU reads kernargs locally with no host-cache snoop whichever socket the writer thread runs on. Each slot is staged on the stack, copied through the BAR and made visible with CLR's `DeviceKernelArgsReadback` sequence — sfence, rewrite the last kernarg byte, mfence, volatile read of it back (ROCm/clr `rocclr/device/rocm/rocvirtual.cpp`, `VirtualGPU::submitKernelInternal`; the gfx94x default in `rocsettings.cpp`) — before the packet header is published; no existing fence changes. Falls back to the host kernarg pool (logged) when the VRAM pool or BAR mapping refuses. Pinned staging stays on the `PLOW_AMD_NUMA_HOST_POOLS` pools. The `HSA host placement` line carries `kernarg_vram`. **Tier 3** (7-layer TP8, every process pinned to socket 0 as serve pins its engine thread, 6 processes per arm interleaved): 10.910 (pinned control) / 9.988 (NUMA pools) / **7.062 ms/step** (NUMA pools + VRAM kernargs, spread 0.020 ms), −29 % against NUMA pools alone and −35 % against the pinned control; the rank 4–7 minus rank 0–3 per-window gap 105 / 77 / **0.6 µs**, one-shot XReduce time ranks 0–3 / 4–7 189/48 → 29/26 µs, lone-GEMM interval 20 µs on every rank; all 8 ranks token-identical in every process. **Tier 4** (job `0-1789202824-kernarg-vram-t4`, runtime-only arm on the 08c0ad44 checkpoint set, one binary, 20 × 70k/700, C20, ctrl / treat / ctrl2): out tok/s 63.08 / 70.18 / 63.37 (+11.0 % against the control mean, controls 0.46 % apart); decode-only tick p50 88.0 / 62.9 / 85.1 ms (−27.3 %, controls 3.3 % apart; p90 −28.0 %); prefill tick p50 855.3 / 834.5 / 853.9 ms (−2.4 %, controls 0.2 % apart); full 8192 chunk mean 793.3 / 792.7 / 799.3 ms (−0.4 %, inside the 0.8 % control spread: the prefill tick's gain is the decode dispatched in it, 72.5 / 54.2 / 70.4 ms); retrieval 18/18 + 21/21 in every arm. **Default on**; `=0` / `--amd-kernarg-vram=false` = the host kernarg ring (rollback). |
...
| `PLOW_TP_PREFILL_AUDIT_PINNED=0/1` (`--amd-tp-prefill-audit-pinned`) | on | **AMD TP** — read each prefill chunk's exact cross-GPU counter audit (same gates, same expectations) plus the collectives' status word with one pinned D2H copy per rank into a buffer allocated at load, instead of `n_xctr` large-BAR loads per rank. Takes precedence over `PLOW_TP_PREFILL_AUDIT_DIRECT`. The status word adds a check the gate audit alone cannot make: a collective that bailed at its deadline while a late peer still completed every count (`0xDEAD0000 \| rank`). Measured (GLM-5.3-FP8 TP8, 8x MI300X, `bench --prefill-sweep` 128/512, BAR and pinned alternated per chunk in one process): audit 4.33 → 0.12 ms per chunk, chunk total 99.5 → 95.7 ms (128) and 126.7 → 122.5 ms (512), chunk-to-chunk spread about ±0.5 ms; an injected stale counter is caught. `=0` restores the BAR audit. |
...
| `PLOW_VMM_DEFERRED_RECLAIM=0/1` / `--vmm-deferred-reclaim` | 1 | **AMD** VMM KV pools (shared-prefix cache groups and `PLOW_VMM_KV`): recycling a slot keeps the previous occupant's row-0 block in place when it is private, unmaps a cache-shared row-0 block inline, and leaves every other block mapped-but-stale for the pool thread to unmap (`hsa_amd_vmem_unmap` is a serialized ~0.3 ms KFD call; a 66k-token occupant at TP8 cost ~1.9 s of engine-thread unmaps per recycle). Zero-ref handle releases move to that thread too. Budget unchanged (stale blocks reach the pool/driver as soon as the thread runs; pool and cache caps apply as before). `0` = synchronous unmap in `begin_slot`. **CUDA** VMM prefix pool: honoured too — `begin_slot` keeps a private row-0 column in place (no `cuMemUnmap` + `cuMemMap`/`cuMemSetAccess` round trip for the next occupant's first block column, ~69 µs per granule) and the pool thread unmaps the stale columns; unmeasured on H100 at the time of writing, `0` restores the synchronous CUDA path. |
| `PLOW_VMM_RELEASE_RETIRE=0` / `--vmm-release-retire` | on | **AMD** shared-prefix cache groups, needs `PLOW_VMM_DEFERRED_RECLAIM`: when a request finishes, the pool thread gives the prefix cache a copy of the slot cache-shared row-0 blocks (batched D2D copy, generation-checked before the swap), so the slot own blocks become private and the next `begin_slot` keeps row 0 in place instead of unmapping the shared blocks inline. Copies run with the pool lock released (`begin_slot` waits out only the chunk in flight), so publish and map-ahead never wait on them. The copies land in spare blocks mapped ahead of time: two per track at load, sized within `PLOW_KV_POOL_MIB` (GLM-5.3-FP8 TP8: 354 MiB per rank). A copy stays mapped for its whole life; when the cache evicts it, it becomes a spare for the next copy-out, up to one extra column per track outside the reuse pool, so a full cache costs no unmap. Only spares past that cap are unmapped, by the pool thread, never during a TP deferred prefix publish (its evictions queue those drops, and the unmaps share ROCr's process-wide lock with the publish's allocations and copies: slotclear-t4-arms4-v4r, 4096 C1 dropped 1638 copies per rank and paid ~36 ms per prefill tick). `begin_slot` waits up to 5 ms for a copy-out still in flight before falling back to the inline swap. Why copies and not unmaps: ROCr serializes `hsa_amd_vmem_*` process-wide (rank-parallel swap 0.93x) while D2D copies scale across devices (78 blocks x 8 devices: 11.15 ms serial, 2.13 ms parallel). Measured before the change: `begin_slot` 100-134 ms after a published 8192-token occupant. Default on since review log #99: served T4 on the default config (`slotclear-t4-arms-v5`, ctrl/treat/ctrl2/treat2) clears `begin_slot` 100 -> 0.5 ms at 8192 C1, TTFT -102.5 ms at 8192 C1 and -178 ms at 8192 C16, TPOT -9.6 ms and E2E -1.4 s at 8192 C16, 4096 C1 within the checkpoint P floor; at 8192 C16 the two decode ticks after each prefill run the deferred spare drops (decode-tick p99 +1.9 ms, TPOT p99 still -10 ms); retrieval 39/39; certificate `perf-certs/rt.vmm_release_retire.json`. `=0` is the rollback. |
...
| `PLOW_HOST_TIMING=1` | off | Serving host path. Per request: `HOSTT submitted` (handler entry to mux submit: template + tokenize), `HOSTT first token emitted` (submit to the engine thread's first send) and `HOSTT first frame` (handler entry to the first SSE frame); the three join by order at C1. Per 256 decode-only ticks on the CUDA engine, one `HOSTT decode` line: tick period, engine call (launch + device + D2H), token emit loop per tick and per token (detokenize + channel send), the rest of the tick body, the dispatcher-engine handoff, the dispatcher's own loop, and the host share of the period. Diagnostics. |
| `PLOW_TICK_LOG=1` | off | AMD serve: one `TICK` line per mux tick (prefill launches/rows/ms, decode rows/ms, host remainder, idle before), one `PFCHUNK` line per prefill chunk (cursor / rebase / `prefill_chunk` / restore / snapshot / prefix publish ms) and one `PFSEG` line per TP `prefill_chunk` (prepare / rearm / xctr / enqueue / map_ahead / drain / audit ms, `prepare_maps` / `map_ahead_maps` VMM block mappings, cumulative per-rank drain); the `TICK` line also carries `dec_vmm` (ms the decode spent mapping KV blocks), `dec_maps`, `dec_segs` / `dec_inflight_enq` / `dec_inflight_sub` (rank 0's dispatches still in flight after the decode's enqueue and after its re-arm: near `dec_segs` = the GPU was behind the host, so both overlapped it) and `pub_deferred`; `PFSEG` also carries `prepare_vmm` / `prepare_patch` (the rest of `prepare` is the id/pos/kvlen uploads) and `publish_deferred`; `PFCHUNK` carries `publish_fill` (the snapshot copy inside a publish), `resume` (rows the request attached from the prefix cache; a request's first chunk after a hit has `c0 == resume`) and `sparse` (1 = the chunk ran a sparse DSA program, e.g. a `PLOW_AMD_TAIL_SPARSE_CTX` tail in the 8192 bucket); the first chunk of a request also splits `cursor` into `clear` (`begin_slot`) and `attach` (the prefix-cache lookup and attach). With the TP shared prefix cache, one `PFATTACH` line per attach attempt: `rows` attached (0 = miss), the all-rank `flush` (a deferred publish run first) / `stage` / `commit` ms (sums over ranks) and `kept` (prefix blocks the attach found already mapped in place and kept, all ranks). Diagnostics; stderr. |
...
| `PLOW_AMD_UNION_SKIP=0` | on | GLM sparse prefill (AMD): the host skips the `IndexUnionPf` segments while the native sparse attention route is active (prior ≥ 2047), because that route gathers `iidx_pf` and nothing reads the union table. Load logs `index union segments unread … skippable=N`. Runtime only: the packet and objects are unchanged. T3 on P8192-S: 667.2 / 648.3 / 666.8 ms (ctrl / on / ctrl2), −18.7 against a 3.7 ms noise floor; P4096-S −11.1; P8192-0 unchanged (`glue2-t3-g1-rung`). Default on since review log #92: served T4 (`glue2-t4`, off/on/off2/on2, bundled with `PLOW_GLM_MOE_SHARED_SEED`) not worse beyond the checkpoint P floor in any cell, C16 TTFT −39.0 / −23.1 ms and TPOT −2.66 / −1.92 ms beyond it; retrieval 18/18 + 21/21; certificate `perf-certs/rt.union_skip.json`. `=0` is the rollback. |
| `PLOW_DSA_SELECT_THRESHOLD=1` | off | GLM sparse prefill (AMD): the TP indexer launches `plow_dsa_tp_select_thr` instead of `plow_dsa_tp_select` — the same top-2048 set per row (same key, row-set identical in `runtime/tests/dsa_select_thr_gfx942_test.hip`) in two row scans instead of five. Needs the adapter built with `-DPLOW_DSA_TP_SELECT_THR=1`; the load fails by name otherwise. Runtime only: the packet is unchanged. T2 (`idxtier-t2-thr`, one rank band, device events): slowest-rank select 566 → 372 µs at 8192 rows / prior 65536, 318 → 197 at 4096 rows. Opt-in; T3 pending. |
...
| `PLOW_STATE_CLEAR_DEVICE` | **on**; `=0` restores the host-staged fills | AMD: clear slot-major recurrent state with one device kernel per rank instead of host-staged SDMA fills. Requires rebuilt decode objects carrying `plow_state_clear`. |
...
| `PLOW_TOKEN_BATCH_TP_OBJECTS=1` | off (`scripts/build_gfx942.sh`) | Also build `interp_packed_mla_{norm,flash}_tb{,_fp8kv}`: the packed MLA family objects with the slot-band resolver compiled in (`PLOW_PACKED_PREFILL_BAND=1`, marker `plow_packed_prefill_band_1`), which a token-batch body's MLA family segments are routed to. Requires `PLOW_PACKED_PREFILL_CONSUMERS=1`. Shipped objects untouched. Under `PLOW_HSACO_CONFIG` both are derived from the packet's `plow_config.h`: `PLOW_PACKET_HAS_TOKEN_BATCH_BODIES 1` turns on this and `PLOW_PACKED_PREFILL_CONSUMERS`, `PLOW_PACKET_HAS_PACKED_PREFILL_TOPOLOGY 1` the latter, and an explicit `=0` the packet contradicts is refused by name (the set would fail to load on the first body or sibling). |
...
| `--pf-attn-gemm` (`PLOW_PF_ATTN_GEMM`) | auto | vendor-GEMM attention for the full-attention prefill segments (Gemma-4 global hd512, one or several KV heads): cuBLASLt `Q.K^T` and `P.V` around the causal softmax of `attn_softmax_sm90a.cubin` (looked up in `--pf-seg-dir`, then the asset dir). No packet change; routed launches run the segment graph pieces around those segments. Unset = on when that object exists and the KV admission budget left after the route's scratch still admits every live request (`PLOW_DECODE_MAX_RUNG`, else the batch) at full context — the scratch otherwise costs admitted requests (Gemma-4-26B: on at rung 4, off at rung 16); `=1` forces it on (a missing object fails the load), `=0` is the rollback. `--pf-attn-gemm-tile` (`PLOW_PF_ATTN_GEMM_TILE`, 2048, capped at the widest routed bucket) = query rows per score tile, scratch = rows x heads x max_ctx x 2 B; `--pf-attn-gemm-grid` (`PLOW_PF_ATTN_GEMM_GRID`, 8) = softmax blocks per SM (one score row per warp); `--pf-attn-gemm-s32` (`PLOW_PF_ATTN_GEMM_S32`) keeps the scores in f32 between the GEMM and the softmax (2x scratch). |
| `--pf-attn-gemm-min-rows` (`PLOW_PF_ATTN_GEMM_MIN_ROWS`) | 1024 | `--pf-attn-gemm` routes a prefill bucket only when it has at least this many rows, and a launch only when some request in its pack has at least this many query rows; a pack of short slices keeps its segment graph, which beats three library launches per request at short lengths (Gemma-4-12B 128-token cells +0.8-4 ms at C1/C4 and +8% at C16 when routed; 1024-token cells neutral). |
...
| `--moe-pf-lt` (`PLOW_MOE_PF_LT`) | from the packet | `N` = in every prefill bucket of at least `N` rows, serve the packet's `MOE_PREFILL_CUBLASLT` segments (emit `PLOW_EMIT_MOE_PF_LT`) with two cuBLASLt grouped matmuls (`CUBLASLT_BATCH_MODE_GROUPED`, cuBLAS 13.4+ on Hopper: per-expert row counts and matrix pointers are device arrays, no host sync, graph-capturable) plus gather / GeGLU / gate-scale-scatter glue kernels from `interp_sm90a_moe_lt.cubin` (`PLOW_PF_SEG_DIR` or the assets dir). Unset = every bucket that carries the segments; `0` = the segments run in the interpreter (the rollback). |
| `--moe-dec-lt` (`PLOW_MOE_DEC_LT`) | from the packet | `N` = on every decode rung of at least `N` rows, serve the packet's `MOE_DECODE_CUBLASLT` segments (emit `PLOW_EMIT_MOE_DEC_LT`) with the same grouped-matmul chain as `--moe-pf-lt`, preceded by a norm glue kernel that stages `xn2 = rmsnorm(x) * gamma` (the fused norm of `MoeExpertGluNormGemma`). A routed rung runs as a captured graph of per-segment cooperative launches with the narrow arena plus the library calls; multistep (`PLOW_MULTISTEP`) enqueues that graph K times per quantum like any other rung. Unset = from the narrowest rung that carries the segments (the emit's `PLOW_GEMMA_MOE_DEC_GROUP`); `0` = every rung runs in the interpreter (the rollback). |
