...
| `PLOW_XR_SCHED_NWG_SRS` / `PLOW_XR_SCHED_NWG_SAG` | unset (= `PLOW_XR_SCHED_NWG`) | **objects** (`scripts/build_gfx942.sh`, prefill rows, with `PLOW_XR_SCHED=aiter`) — separate data-workgroup caps for the sequence-parallel seam halves, op 25 `XReduceScatter` (`_SRS`) and op 26 `XAllGather` (`_SAG`); unset they keep the all-gather cap `PLOW_XR_SCHED_NWG` they have always used, and the objects are byte-identical. Set, the objects export `plow_xr_sched_nwg_srs` / `_sag`. Bit-identical (strict-order oracle). Opt-in pending the seam microbench and tier-3/4 evidence. |
