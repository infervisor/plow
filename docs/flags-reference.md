...
| `PLOW_VMM_CACHE_MIN_FREE_MIB=M` | 4% of the device | Pressure eviction: keep cached prefixes (shared blocks and boundary snapshots) until free device memory would drop below M MiB, instead of trimming at the static `PLOW_VMM_CACHE_MIB`/`_MEMORY_UTILIZATION` budget. Needs a backend that reports free device memory (HSA agent info `0xA015`); others keep the static budget. Unset derives 4% of the device (~7.7 GiB on a 192 GiB MI300X) rather than a fixed MiB figure that would be a third of a small card; `=0` is the rollback to the static budget alone. |
...
| `PLOW_AMD_PREFIX_FINE_ROWS=N` | unset | Share AMD prefix content below the 2 MiB VMM block floor in N-row chunks instead of leaving it a private copy. N must divide every cache group's block size. |
