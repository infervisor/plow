...
| `PLOW_XR_SCHED` / `PLOW_XR_SCHED_NWG` / `PLOW_XR_SCHED_AG_U` | off / 24 / 1 | **objects** (`scripts/build_gfx942.sh`, prefill rows) — `aiter` builds the strict-rank-order 16-byte prefill collective schedule (`-DPLOW_XR_SCHED_AITER=1`) on the first `NWG` workgroups of each `XReduceTwoShot` / `XReduceScatter` / `XAllGather` packet; the other workgroups take only their `gate_ag` arrival and leave, so the packet (304 workgroups) is unchanged. `AG_U` = all-gather packs in flight per lane. Bit-identical (host strict-order oracle, every bucket). 8x MI300X two-shot, 304 workgroups launched: 8192x6144 bf16 970 → 794 µs (-18%), 2048 rows 288 → 204, 512 rows 120 → 81, 128 rows 68 → 46 µs. In model (4-layer TP8 packet, last 8192 chunk at prior 65536, ctl / K24 / ctl2): 1.08 → 0.99 ms per collective, projected −12 to −14 ms per 78-layer 8192 chunk. Decode is not affected (decode rungs emit only the one-shot `XReduce`). Opt-in until a served A/B. `PLOW_XR_TRACE_PHASES=1` (diagnostic) also reaches the decode objects. |
