...
| `PLOW_VMM_PUBLISH_SHARED=0/1` / `--vmm-publish-shared` | 1 | CUDA VMM prefix cache: publish a sequence's boundary (radix insert, snapshot buffer allocation, the sliding-ring + partial-block D2D copies and their stream sync) only once its leading 32 tokens were seen on another sequence. A unique-prompt workload then pays nothing per request (`publishes_skipped` in the pool stats counts them); a shared prefix — system prompt, replayed prompt, multi-turn continuation — publishes from its second sighting on, so the first reuse of a fresh lead misses once. `0` publishes every prefill completion and sequence end as before (~2 × up to 0.5 GiB of copies plus a cudaMalloc/cudaFree churn per request on Gemma 4 12B). Unmeasured on a GPU at the time of writing; `0` is the rollback. |
