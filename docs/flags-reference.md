...
| `PLOW_PIPE_PREFILL` / `--pipe-prefill` | false | With `PLOW_DECODE_PIPELINE` on, the mixed prefill/decode launch is enqueued behind the step in flight instead of reading that step out first. Decode rows whose newest token the pipe still owes take it from the device (`d_last`, written by each launch's samples) rather than from the host, and this launch's own samples — a prompt's first token included — are read back a tick later, so the host never waits between launches. Needs the unified token batch (`PLOW_TOKEN_BATCH`) and greedy rows; otherwise the tick falls back to completing the in-flight step first. A stop the host cannot foresee costs one discarded step. |
