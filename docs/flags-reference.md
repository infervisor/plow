...
| `PLOW_AMD_UNION_SKIP=1` | off | GLM sparse prefill (AMD): the host skips the `IndexUnionPf` segments while the native sparse attention route is active (prior ≥ 2047), because that route gathers `iidx_pf` and nothing reads the union table. Load logs `index union segments unread … skippable=N`. Runtime only: the packet and objects are unchanged. T3 on P8192-S: 667.2 / 648.3 / 666.8 ms (ctrl / on / ctrl2), −18.7 against a 3.7 ms noise floor; P4096-S −11.1; P8192-0 unchanged (`glue2-t3-g1-rung`). Default flip waits on T4. |
