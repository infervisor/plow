...
| `PLOW_TMA_GEMM` | `--tma-gemm` | false; **production default true for BF16 Gemma-4 hybrid attention (`full_attn_hd512`) on sm_90a TP1** | Emit TMA descriptors for GEMM operands (sm_90a+). Pure-GEMM segments (`PLOW_SEG_PURE_GEMM=1`) admit only mapped GEMMs, so the lean GEMM object needs them. `=0` is the rollback only for a packet without sm90a prefill attention role objects: the HD256/GQA2 and HD512 objects the Gemma-4 H100 recipes ship (`[objects].role_files`) bind the map, and their role emit rejects an unmapped packet. |
