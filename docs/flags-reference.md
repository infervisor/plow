...
| `PLOW_GLM_MOE_SHARED_FOLD` | `--glm-moe-shared-fold` | false | Fold GLM's shared expert into the native AITER MoE prefill call as expert 256 with a constant gate of 1.0 (router appends a 9th slot, align and the fused call run 257/9, the shared GEMM pair and the combine's `shared` operand go away). This is the shape AITER's gfx942 GLM-5 tuning table is indexed by; same pinned objects. Needs `PLOW_GLM_MOE_AITER`/`_RESIDENT`, TP, and a checkpoint that still holds the shared expert's block-FP8 bytes (the lite prep's are shadowed, and are found). Decode keeps its own shared GEMVs. NOT bit-identical: the shared expert moves from bf16 to A8W8 block-FP8 (per-row FFN rel-L2 vs f64 ~4–5%, the A8 floor the routed call already has). Measured on GLM-5.3 TP8 (ABA, same binary): −25 ms per 8192-row prefill chunk (single-request TTFT 9.74 → 9.54 s at 59k; controls within 2 ms), +2.2% C20 output tok/s (within one control-drift band), retrieval 18/18. Emits `…expert_{weight,scale}_table_sf`; needs packet-stamped objects, the `plow_moe_shared_fold_arm` prefill object and the `plow_moe_aiter_nexp_abi_1` adapter. |
