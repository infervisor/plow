...
| `PLOW_AMD_RAGGED_SEAMS` | `--amd-ragged-seams` | false | Runtime: size a ragged chunk's sequence-parallel seams by its live rows instead of the bucket. With `clen` live rows each rank's band becomes `b = ceil(clen / tp)` rows (at least 1). The reduce-scatter / all-gather counts go from `T` to `tp·b` rows, so each rank's slice stays its band's whole rows, and every band packet goes from `T/tp` rows to `b`. The `@band` views are rebound per chunk in the tensor-pointer table to `base + rank·b·row`, and a full chunk restores the load-time binding. The packet and objects do not change, and it acts only under `PLOW_RAGGED_CHUNK` on a packet that carries the seams. Opt-in; off leaves every patch and binding as before. |
