...
| `PLOW_VMM_CACHE_MIN_FREE_MIB=M` | unset | Pressure eviction: keep cached prefixes (shared blocks and boundary snapshots) until free device memory would drop below M MiB, instead of trimming at the static `PLOW_VMM_CACHE_MIB`/`_MEMORY_UTILIZATION` budget. Needs a backend that reports free device memory (HSA agent info `0xA015`); others keep the static budget. |
