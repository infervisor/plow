...
| `PLOW_TMA_GEMM` | `--tma-gemm` | false; **production default true for BF16 Gemma-4 hybrid attention (`full_attn_hd512`) on sm_90a TP1** | Emit TMA descriptors for GEMM operands (sm_90a+). Pure-GEMM segments (`PLOW_SEG_PURE_GEMM=1`) admit only mapped GEMMs, so the lean GEMM object needs them. `=0` is the rollback. |
