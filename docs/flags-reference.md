...
| `PLOW_EMIT_MOE_PF_LT` | `--emit-moe-pf-lt` | false | Isolate each layer's grouped MoE prefill pair (`MoeGroupGluGemmaPf` + `MoeGroupDownGemmaPf`) as a `MOE_PREFILL_CUBLASLT` segment (role 16; `dense_cublaslt::apply_moe_prefill`). Align, router and combine stay in their interpreter segment, so the packet runs unchanged until the runtime opts in with `PLOW_MOE_PF_LT`; the segment build script adds the glue object `interp_sm90a_moe_lt.cubin` for such packets. sm_90a only. |
| `PLOW_EMIT_MOE_DEC_LT` | `--emit-moe-dec-lt` | false | Isolate each layer's fused expert pair (`MoeExpertGluNormGemma` + `MoeExpertDownGemma`) on every decode rung that runs the grouped arm (`PLOW_GEMMA_MOE_DEC_GROUP`, required) as a `MOE_DECODE_CUBLASLT` segment (role 17; `dense_cublaslt::apply_moe_decode`). Align, router and combine stay in their interpreter segment; a runtime without `PLOW_MOE_DEC_LT` runs every rung as one launch over the merged window, so the packet is unchanged for it. Needs the ABI 2 glue object `interp_sm90a_moe_lt.cubin`. sm_90a only. |
