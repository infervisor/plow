...
| `PLOW_FA_MMAQK` | `--fa-mmaqk` | unset; **production default 3 for BF16 Gemma-4 hybrid attention (`full_attn_hd512`) on sm_90a TP1** | Decode attention Q·Kᵀ on the tensor cores with K streamed from global; bit 0 = hd256 layers, bit 1 = hd512 layers. Object-paired (rides in the manifest `tuning` and the pairing hash). 12B step ctx 1024 −0.30/−0.51/−1.02 ms at B=1/4/16; 26B-A4B 5.61/8.51/12.44 → 5.54/8.34/11.75 ms; GSM8K unchanged. `=0` is the rollback. |
