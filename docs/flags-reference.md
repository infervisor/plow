...
Twelve `glm_*` knobs are ON by default when the model is GLM (`glm_moe_dsa` / `glm5_next`), the
...
`_PROJ`'s follows `PLOW_GLM_SEQ_PAR`. Off on every other target, and off under `--mxfp4` (the
native MoE arms are block-fp8 only).
