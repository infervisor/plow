...
| `PLOW_ENCODE_THREADS=N` | unset | Tokenize a prompt of ≥ 8 KiB in up to N pieces on a dedicated N-thread pool, cut at a single ASCII space between two ASCII letters. Enabled only for a BPE tokenizer with no normalizer, truncation, padding, post-processor tokens, stripped added tokens, or dropout, and a `Split(Isolated)` + `ByteLevel(use_regex=false, add_prefix_space=false)` pre-tokenizer on the GLM or Qwen2 pattern, where such a space always starts a pre-token; any other tokenizer warns and stays serial. Ids identical to `encode` on 1420 GLM-5.3 prompts (vLLM random 1k–16k, chat template, adversarial). GLM-5.3 CPU 8192-token prompt 12.3 → 4.8 ms (N=4), 2.6 ms (N=8). Runtime only. |
