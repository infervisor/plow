...
| `PLOW_GLM_FOLD_LT` | `--glm-fold-lt` | false | Native gfx942 FP32 MLA fold during prefill (buckets >= 2048). **Tier 3 (L4 TP8 packet, ctrl/fold/ctrl2/fold2 through the production mux): steady 8192 chunk 50.70 -> 49.67 ms (-1.03 ms per 4 layers, 5x the arms' 0.16/0.23 ms spread; TTFT at 73,728 tokens -9.2 ms) = -0.26 ms/layer, ~ -20 ms per full 78-layer chunk; decode ticks unchanged.** Each layer's `MlaMergeFold` (the W_uv fold, ~545 us/layer on the interpreter's VALU map) runs isolated as normalize -> pinned hipBLASLt FP32 GEMM (`glm_fold_lt_gfx942.elf`) -> bf16 convert. Packet: only the 78 folds of the 2048/8192 programs change (one block, `i[5]=1`), one extra segment per layer; pairing hash unchanged. Numerics: not bit-identical, 0.03% of bf16 outputs move by 1 ulp, rel-L2 vs FP64 unchanged (1.66e-3). Needs `glm_fold_lt_gfx942.elf` + `glm_fold_adapter.elf`, which `scripts/build_glm53_gfx942_serving_objects.sh` builds when the packet was emitted with the knob (VENDOR_DIR must hold the pinned image); +~504 MiB/rank HBM. Served on an older base: +0.49% with P99 TPOT +3.6% (one pair). Opt-in pending a tier-4 A/B. |
| `PLOW_GLM_FOLD_LT_DECODE` | `--glm-fold-lt-decode` | false | The same native FP32 fold at decode rungs 16 and 20 (nsplit 8 / 4 in the production packet): isolated `MlaMergeFold`, a raw `MlaFold` decode segment, 3 launches. Separate from `PLOW_GLM_FOLD_LT` so prefill and decode qualify independently. Packet: only the 78 folds of the rung-16/20 programs change, one extra segment per layer, pairing hash unchanged; knob off is byte-identical. Numerics as the prefill fold (0.025% of outputs, 1 ulp; merge exact to f32). Composes with `PLOW_GLM_DECODE_GEMM_GROUP` (the fold boundary is flash / fold / o_proj in both layouts). Same objects as `PLOW_GLM_FOLD_LT`. Opt-in, tier 2. |
...
| `PLOW_GLM_SEQ_PAR` | `--glm-seq-par` | false | Sequence-parallel TP seams for the GLM prefill buckets of 2048 and 8192 rows. Each o_proj and FFN all-reduce (`XReduceTwoShot`) becomes a reduce-scatter (`XReduceScatter`, op 25). The residual add and the following RMSNorm (the next layer's input norm, the post-attention norm, the final norm) run on this rank's `t/8` row band, and the normed rows are all-gathered (`XAllGather`, op 26) out of peer slot 3 (`act.h2_tp`). Between seams the residual stream lives only on the band. Same kernels and reduction order as the two-shot, so the gathered activations are bit-identical. The 128/512 rungs, decode and GLM-5.3-Next's hyperconnection wrap are unchanged. Peer region grows from 3 to 6 slots (+302 MB/rank at 8192 rows). Packets require `PLOW_SEQ_PAR_SEAMS=1` (packet-stamped objects compile the arm from `plow_config.h`). Refuses `PLOW_GLM_XR_RES` / `PLOW_GLM_XR_BAND`. Under `PLOW_RAGGED_CHUNK` the band packets keep their full band and the split collectives move the whole bucket. Opt-in; off is byte-identical. |
| `PLOW_GLM_SEQ_PAR_PROJ` | `--glm-seq-par-proj` | false | Extends `PLOW_GLM_SEQ_PAR` (set both) to the layer-input seam: the input norm, q_a and its norm, kv_a and k_rope run on the band, and one all-gather carries q_a-normed / kv_a / k_rope (2624 of 6144 columns) out of peer slots 3/4/5 instead of the normed hidden. On a full-indexer layer the indexer's k and weights projections also run on the band, and a second gather carries them out of slots 2 (`act.ug_tp`) and 0. At the FFN input the router score and top-k also run on the band and the route table is gathered out of slot 5 (not under `PLOW_MOE_PF_DET` without the native MoE). The band GEMMs are interpreter tiles at `t/8` rows (hipBLASLt is pinned to `M = t`), so not bit-identical. Falls back to the `PLOW_GLM_SEQ_PAR` layer entry under `PLOW_GLM_GEMM_BLK`, a pooled indexer, or a sparse layer without `PLOW_GLM_INDEX_TP`. Opt-in; off is byte-identical. |
...
| `PLOW_AMD_ATTACH_PARALLEL=0/1` / `--amd-attach-parallel` | off | **AMD TP** shared-prefix cache: stage an attach's ranks on one thread each instead of one after another. A measurement arm: ROCr serializes every `hsa_amd_vmem_*` call behind one lock, so the ranks' driver work may not overlap; `PLOW_TICK_LOG=1` prints the per-rank `timeline` and `stage_wall` on `PFATTACH`. Same result either way; the parallel form also stages (then rolls back) ranks past a miss. |
