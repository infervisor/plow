...
| `PLOW_GLM_SEQ_PAR_PROJ` | `--glm-seq-par-proj` | false | Extends `PLOW_GLM_SEQ_PAR` (set both) to the layer-input seam: the input norm, q_a and its norm, kv_a and k_rope run on the band, and one all-gather carries q_a-normed / kv_a / k_rope (2624 of 6144 columns) out of peer slots 3/4/5 instead of the normed hidden. On a full-indexer layer the indexer's k and weights projections also run on the band, and a second gather carries them out of slots 2 (`act.ug_tp`) and 0. The band GEMMs are interpreter tiles at `t/8` rows (hipBLASLt is pinned to `M = t`), so not bit-identical. Falls back to the `PLOW_GLM_SEQ_PAR` layer entry under `PLOW_GLM_GEMM_BLK`, a pooled indexer, or a sparse layer without `PLOW_GLM_INDEX_TP`. Opt-in; off is byte-identical. |
