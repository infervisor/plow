...
## Conversion hoisting and interpreter attribution

The subsequent opt-in `PLOW_NV_GEMV_KPANEL_F32=1` caches each2048-element activation panel after BF16-to-FP32 conversion. It preserves row ownership and FMA/reduction order. Matched full cubins both use241 registers, zero stack/spills/local memory, and the unchanged12352-byte arena. SASS contains272 fewer static instructions with unchanged FFMA/global-load counts. Both13-shape GPU harnesses pass the existing cuBLASLt numerical gate, but the targeted down projection measures66.0µs in both arms. This instruction-count reduction has not produced a measured kernel improvement and is not promoted. Artifacts: `/tmp/plow-model-support-checks/qwen-kpanel-f32/`, including `control-gpu.log`, `candidate-gpu.log` and `full-sass-summary.json`.

The existing `step_bench` diagnostic, using the ordinary XREG+KPANEL cubin and native128-token prefill, measures128 decode steps after16 warmups: mean31.334ms, median31.335ms. CUDA-event interpreter time is31.245ms (host debug build, no HTTP/mux). This closely reproduces the serving TPOT; serving overhead does not explain the vLLM gap. A separate two-step build changes only `PLOW_NV_TRACE=1`: block0 observes1611 packets, with36.0% gate,60.6% body and3.4% signal cycle shares. GEMV accounts for994 observations and most gate/body cycles. Gate time includes waiting for predecessor work and stragglers; it is not a measurement of removable synchronization overhead. Other blocks can own different packet subsets. Traced totals are diagnostic only. Raw: `/tmp/plow-model-support-checks/qwen-kpanel-step-{control,trace}.log`; exact trace build provenance: `/tmp/plow-qwen-kpanel/trace-provenance.json`.
