...
r=pathlib.Path("plans/gemma4-12b-roofline");out=r/"cubin-bf16-fp8kv-request";out.mkdir(exist_ok=True)
...
common += ["-D"+s for s in ["PLOW_FP8_KV=1","PLOW_NV_GEMMA=1","PLOW_NV_FA_GF=2","PLOW_NV_EMBED_SMEM=1","PLOW_NV_MLA=0","PLOW_NV_MAMBA=0","PLOW_NV_DSA=0","PLOW_NV_GEMV_RB=1","PLOW_MOE_DOWN_LANESPLIT=1","PLOW_NV_FA_WPR=1","PLOW_NV_FP8_RB=4"]]
...
for packed in [0,1]:
 ...
 for role in ["seg","gemm","fa"]:
  name="interp_sm90a_"+suffix+role+("_fp8kv" if packed else "")
...
for name,flags in configs:
 ...
 with (r/("fp8-request-build-"+name+".log")).open("w") as f:
