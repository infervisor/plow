...
for label, asset in [("w8a8", "w8a8-native-b16")]:
    env = dict(os.environ, GEMMA_NATIVE_ASSETS=str(root / asset),
               GEMMA_NATIVE_OBJECTS=str(root / "cubin-w8a8-b16"),
               GEMMA_PF_CHUNK="1024", GEMMA_PF_BUDGET="2048")
    with open(root / f"w8a8-profile-{label}-server.log", "w") as log:
        server = subprocess.Popen(["bash", str(root / "serve_native.sh"), "bf16", "--pf-seg-time"],
        ...
        try:
            ...
            with open(root / f"w8a8-profile-{label}.log", "w") as bench:
                subprocess.run(["python3", "scripts/bench_packed_serve.py", "--url",
                                "http://127.0.0.1:8013", "--out", str(root / f"w8a8-profile-{label}.json"),
                                "--label", f"w8a8-profile-{label}", "--inputs", "16384",
                                "--outputs", "1", "--concurrency", "1", "--repeats", "1",
