...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as pool:
        for _ in range(2):
            ...
            assert actual == expected, ("concurrent outputs differ from isolated control", expected, actual)
        ...
        for during_decode in [False, True]:
            ...
            actual = list(pool.map(lambda case: generate(
                args.candidate, args.model, *case), cases))
            assert actual == expected, ("cancellation changed surviving requests", during_decode, expected, actual)
