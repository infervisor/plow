...
for label, asset in [("w8a16-staging-0", "w8a16-packed-b16"), ("w8a16-staging-1", "w8a16-packed-b16")]:
    env = dict(os.environ, GEMMA_NATIVE_ASSETS=str(root / asset),
               GEMMA_NATIVE_OBJECTS=str(root / ("cubin-" + label)),
               GEMMA_PF_CHUNK="1024", GEMMA_PF_BUDGET="2048")
    with open(root / f"w8a8-b16-serving-{label}-server.log", "w") as log:
        server = subprocess.Popen(["bash", str(root / "serve_native.sh"), "fp8"],
