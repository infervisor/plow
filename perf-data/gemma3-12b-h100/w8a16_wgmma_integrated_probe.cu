...
__global__ void candidate(bf16* y,const bf16* x,const unsigned char* w,const float* scale,unsigned M,unsigned N,unsigned K) {
  extern __shared__ __nv_bfloat16 arena[];
  d_gemm_sm90_impl<false>(y,x,w,M,N,K,0,blockIdx.x,gridDim.x,arena,nullptr,scale);
}
...
int main(int argc, char** argv) {
    ...
    run(8, 64, 256, true, flush); run(17, 71, 256, true, flush); run(17, 71, 264, true, flush); run(17, 71, 8, true, flush);
