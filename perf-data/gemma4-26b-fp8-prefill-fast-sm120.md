# fp8-KV FAST (PIPE=1) prefill flash — px4 hd512 — **NO-GO** (26B view, beat-fp8-prefill, Exp1)
...
The px4 hd512 FULL-attn arm and its compute-bound character are shared by 12B and 26B (the
microbench uses the 26B full-attn geometry n_head=16/n_kv_head=2 as the representative worst
case). The NO-GO is a property of the kernel, so it holds for both models; a separate 26B run
does not change the verdict. vLLM fp8kv 26B@128k target = 5133 ms.
