...
1. **The campaign tip does not compile.** `crates/plowrt/src/exec/gpu.rs:2506` carries committed
   ...
   (`origin/worktree-agent-acfa45450cb637150` into `worktree-plowrt-max-completion-tokens`).
...
| GPU exclusive | **ENFORCED** — every run under `perf-data/harness/gpulease`; a sibling agent was active on the card and the lease serialised it |
