...
| Global-queue ASAP order (09-04) | Emit-time stable sort of each (segment, XCD) window by ASAP rank | TPOT 27.700 → 27.490 ms (−0.210, −0.76%), TTFT −6.1 ms, exact | promoted | `PLOW_GQ_ORDER=asap` default, `=stream` rollback |
...
| Tagged one-shot XReduce (09-04) | 8-byte tagged words, no counter, strict rank-order accumulate | in-network body 15.63 µs; tagged hot 3.716 vs one-shot 9.766 µs; projected ~2.5 ms/token | pending, default-off | CMake `PLOW_XR_TAGGED`, `PLOW_XR_TRACE_PHASES=1`, `scripts/k3_xr_decode_report.py` |
...
| Router + align parallel (09-03) | Per-token router slices (cap `n_cu`) + four align packets | TTFT −8.639 ms (−0.309%); router+align 82.334 → 70.956 ms, exact | pending, default-off | `PLOW_MOE_ALIGN_PAR=1` |
...
| KDA carry register state (09-04) | Carry state in MFMA accumulators, prefetched key factors | 1.916 → 0.726 ms (2.64×), hwcvt 0.572 ms; 0 mismatches | pending opt-in (TP8 A/B open) | `PLOW_KDA_CARRY_REGSTATE=1` |
...
| GemmWide c8 tile (09-04) | Tagged 128×384×64 at `8192×1536×7168` | 164.398 vs 223.929 µs (26.6%); TTFT −11.462 ms, TPOT +0.04, E2E −1.327 ms, exact | pending, default-off | `--emit-gemm-wide-c8-shape`; a42b21b, fb017cd |
...
| AttnRes f32-mix phase object (09-04) | Standalone `plow_attn_res_f32mix_gfx950`, 768-WG grid | 0.260 vs 0.589 ms (−56%); relL2 ≤ 2.1e-7; 0 spills | pending (TP8 A/B open) | `PLOW_ATTNRES_F32MIX=1`, `PLOW_HSACO_ATTN_RES_F32MIX=ON` |
