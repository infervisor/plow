...
| MoE combine folded into the tagged publish (09-04, L4) | The 7 publishing XReduce workgroups sum the 16 routed slot partials in the combine's fixed order and write the tagged words; the combine packet and its gate are not emitted | isolated 9.78 µs fused vs 17.25 µs combine+publish; 5.7M output words identical; TP8 gate 8192→256, 3 alternating folds: TPOT 24.00/24.02/24.07 vs 24.23/24.24/24.30 (−0.23 ms/token), TTFT neutral, checksum identical; decode object SGPR spill 108 → 170, VGPR/occupancy unchanged | promoted, default on | `PLOW_XR_COMBINE_FOLD` (`=0` opts out); branch codex/decode-l4-combine-publish |
| KDA lean Wu + key-fed register-state carry (09-04) | Four-wave lean Wu object (553 → 56 µs/layer isolated; 78 with key emission) whose scaled-key hi/lo pair feeds the regstate carry (726 → 507 µs); chain 1279 → 585 µs/layer | TP8 gate on the full stack, 8192→256, 3 alternating folds: TTFT 903.5/902.5/901.9 vs 955.9/956.1/955.0 ms (−53 ms), TPOT neutral, checksum identical; bit-exact on structured, random and adversarial inputs incl. 63-row tails | promoted, default on | `PLOW_KDA_WU_LEAN`, `PLOW_KDA_CARRY_KEYFEED` (`=0` rolls back); branch codex/prefill-kda-chain |
| Long-context probe (09-04) | Same defaults re-emitted at `--max-ctx 32768`; C1 benches on the 32K bundle | 16384→1024: TTFT 2045.7 ms, TPOT 24.48 ms; 8192→1024: 956.6 / 24.32 (identical to the 16K-context packet); 300→32: TTFT 203.4 ms, output checksum identical to the served bundle (`fnv1a64:1ba6b366432d2165`) | evidence: prefill ~2.14x for 2x tokens, decode flat at 16K context, ragged prompt exact | `docs/k3-mi355x-20260904/scripts` (`longctx_probe.sh` pattern) |
...
| KDA lean Wu + key-fed carry (09-04, `codex/prefill-kda-chain`) | Four-wave lean Wu (operands staged once in LDS, swapped-role MFMA, q pre-scale fused) emitting the scaled-key hi/lo pair; register-state carry consumes the pair | One GPU, T8192 H12: Wu 0.553 -> 0.056 ms (9.9x; with keys 0.078), carry 0.726 -> 0.507 ms (1.43x); chain 1.279 -> 0.585 ms/layer, bit-exact (W/U/q/out/state, structured/uniform/adversarial inputs, tail chunk) | bench + objects + routes ready; TP8 gate pending (`docs/k3-mi355x-20260904/prefill-kda-chain-status.md`) | `PLOW_KDA_CARRY_KEYFEED=1` (emit; implies lean Wu), `PLOW_KDA_WU_LEAN=1` (lean Wu only) |
