...
| decode | sampling and rank agreement | Per-row argmax through B128, every-token drain/counter audit, configurable full-rank token-read cadence, and a four-token device capture ring. | cadence **O/M**; multi-step **I/U** | The default remains every-token agreement. Deferred readback preserves the per-token counter drain and bounds stop/cancel overshoot to four tokens. A short real-weight TP8 gate is token-identical; a matched long-output A/B remains required. |
...
| Decode ladder | Independent-sequence rungs B1 through B128. Runtime chooses the narrowest rung covering the highest occupied slot; tiered B1/B32/B128 object inventories avoid wide-object dead lanes. | **I/U** gfx950 | Real-weight TP8 B1 and B128 smoke gates pass. B1/B32/B128 objects meet the 256-register/occupancy-2 ceiling; B32/B128 use the mandatory GEMV walk. Matched long-output B32/B64/B128 measurements remain. |
