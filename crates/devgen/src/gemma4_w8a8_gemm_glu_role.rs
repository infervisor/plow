...
fn eligible(model: &Model, rows: u32, op: &packet::dev::DevInst) -> bool {
    if op.op != DevOp::GemmGluFp8 as u16
        ...
        || [
            op.t[0], op.t[1], op.t[2], op.t[3], op.t[4], op.t[5], op.t[6],
        ]
        .into_iter()
    ...
    [
        (op.t[0], matrix_bytes(op.i[0], op.i[1], 2)),
        (op.t[1], matrix_bytes(op.i[0], op.i[2], 1)),
        (op.t[2], matrix_bytes(op.i[1], op.i[2], 1)),
        (op.t[3], Some(u64::from(op.i[0]) * 4)),
        (op.t[4], Some(u64::from(op.i[1]) * 4)),
        (op.t[5], matrix_bytes(op.i[1], op.i[2], 1)),
        (op.t[6], Some(u64::from(op.i[1]) * 4)),
    ]
    .into_iter()
    .all(|(handle, required)| {
...
pub(crate) fn apply_output_object(
    ...
    if profile != "sm90a" {
        return Err("Gemma-4 W8A8 GemmGlu role requires sm90a".into());
    }
    let path = output
        .parent()
    ...
    {
        return Err(format!(
            "{} has incompatible Gemma-4 W8A8 GemmGlu capabilities",
            path.display()
        ));
    }
    ...
    if metadata.objects.contains_key(&W8A8_PREFILL_GEMM_GLU_GEMMA4) {
        return Err("Gemma-4 W8A8 GemmGlu role already declared".into());
    }
    ...
    for index in 0..prefill_count {
        ...
        if !eligible.iter().any(|&yes| yes) {
            return Err(format!(
                "prefill rung M{rows} has no compatible Gemma-4 W8A8 GemmGlu instruction"
            ));
        }
        if program.l2_domains != 0 || program.hier_base != 0 {
            return Err("Gemma-4 W8A8 GemmGlu role requires a plain prefill program".into());
        }
        ...
        for (queue_index, entry) in program.gq_stream.iter().enumerate() {
            ...
            let segment = u16::try_from(roles.len() - 1)
                .map_err(|_| "too many Gemma-4 W8A8 GemmGlu role segments")?;
            if inst_segment[inst].is_some_and(|prior| prior != segment) {
                return Err("Gemma-4 W8A8 GemmGlu instruction is not contiguous".into());
            }
        ...
        selected += roles
            .iter()
            .filter(|&&role| role == W8A8_PREFILL_GEMM_GLU_GEMMA4)
            .count();
        updates.push(Update {
            index,
            roles,
    ...
    if selected == 0 {
        return Err("packet has no M4096/M8192 Gemma-4 W8A8 GemmGlu rungs".into());
    }

    metadata.objects.insert(
        W8A8_PREFILL_GEMM_GLU_GEMMA4,
        SegmentObject {
            abi: W8A8_PREFILL_GEMM_GLU_GEMMA4_ABI.into(),
...
mod tests {
    ...
    fn program(rows: u32, map_padding: usize) -> (packet::devbuild::Program, Vec<GenTensor>) {
        ...
        let input = builder.tensor("act.xqh", 8192 * 3840);
        let gate = builder.tensor("model.layers.0.mlp.gate_proj.weight.fp8", 15360 * 3840);
        let up = builder.tensor("model.layers.0.mlp.up_proj.weight.fp8", 15360 * 3840);
        let input_scale = builder.tensor("act.ash", 8192 * 4);
        let gate_scale = builder.tensor("model.layers.0.mlp.gate_proj.weight.scale", 15360 * 4);
        let up_scale = builder.tensor("model.layers.0.mlp.up_proj.weight.scale", 15360 * 4);
        ...
        let glu = builder.emit(DevOp::GemmGluFp8, builder.all(), &[before], |op| {
            op.t = [
                out,
                input,
                gate,
        ...
        let mut gen = [
            GenTensor::tmap_e4m3(input, rows, 3840, 128),
            GenTensor::tmap_e4m3(gate, 15360, 3840, 128),
            GenTensor::tmap_e4m3(up, 15360, 3840, 128),
        ];
    ...
    fn output_dir(label: &str) -> std::path::PathBuf {
        ...
        let path = std::env::temp_dir().join(format!(
            "plow-gemma4-w8a8-gemm-glu-role-{label}-{}-{}",
            std::process::id(),
    ...
    fn qualified_object_routes_only_exact_rungs_and_preserves_scheduling() {
        ...
        for index in [1, 2] {
            let record = roles
                ...
                .unwrap();
            assert_eq!(record.roles, [0, W8A8_PREFILL_GEMM_GLU_GEMMA4, 0]);
