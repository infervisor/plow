...
fn emit_v4_attn(
    ...
    if let Some(r) = c.compress_ratio(l) {
        ...
        let step = b.emit(DevOp::V4KvCompressStep, vec![0], &[g1, g2], |d| {
            ...
            d.t[5] = ape;
            d.t[6] = cnw;
            // `start_pos` advances every step, so it is a TENSOR, not an
            // immediate — the host re-uploads `in.pos` per step already.
            d.t[7] = tn.pos;
            d.i[0] = hd;
            d.i[1] = r;
            d.i[2] = coff - 1;
...
fn emit_v4_shared_expert(
    ...
    //
    // The size is what selects the E8M0 widen. `asset/shard.rs` converts a
    // checkpoint E8M0 scale to the f32 grid only when the declared tensor is
    // EXACTLY 4x the file's bytes; V4 stores one E8M0 byte per block, so with
    // the count-only size the two matched at 512 and the widen never fired.
    // Raw exponent bytes were bound where the kernel reads f32, which is why
    // the failure landed on a workgroup boundary — the first 1024 of 4096
    // outputs (scale rows 0..127, the only ones inside the 512-byte tensor)
    // came back NaN and the rest read whatever followed the allocation.
    // `emit_fp8_gemv` above had the `* F32` all along; this helper did not.
    let blk = |n: u32, kk: u32| (n as u64).div_ceil(128) * (kk as u64).div_ceil(128) * F32;
    let (w1, s1) = (
...
pub(crate) fn v4_emit_full(dir: &Path, ctx: u32, out: &str, n_cu: u32, tp: u32, target: u32) {
    ...
    // L2-DOMAIN PLACEMENT, the precondition for PLOW_GATE_HIER. The gate's
    // per-XCD leader needs a per-(packet, domain) slice count, and without
    // placement that count is a run-time property the emitter leaves zero —
    // which interp.hip reads as "no hierarchy" and silently does nothing.
    //
    // Safe for V4 specifically. `Builder::deny_uniseg` documents placement as
    // DESTRUCTIVE on a program with more than one wave class, because placement
    // repurposes the `seg` field that carries the class. V4 emits ONE segment —
    // a segment is class-4 iff it holds flash packets and V4 has none, its
    // attention being the custom sparse arm — so `seg` is free here.
    //
    // RoundRobin because the CDNA dispatcher assigns workgroups to XCDs
    // round-robin (see L2Map::RoundRobin), 8 XCDs on this part.
    // ALWAYS ON. Measured A/B, same object, same card, same prompt:
    //   placement off   38.330 ms/token   26.1 tok/s
    //   placement on    33.577            29.8      -12.4%
    // Less than the 3.8x ctr_convergence.hip measures for an EMPTY packet, and
    // that is expected: there the maintenance IS the whole cost, here real work
    // dilutes it.
    let domains = 8u32;
    b.set_l2_placement(Some(packet::devbuild::L2Layout {
        sms: n_cu.div_ceil(domains),
        domains,
    ...
    // TWICE, and the optimum MOVED between them, which is the point:
    //
    //   uniform width, every packet     with per-op widths (norm_cus et al)
    //      4  396.8 ms/token                 64  44.6
    //      8  205.9                          96  40.2
    //     16  112.9                         128  38.2   <- min
    //     32   69.3                         192  44.3
    //     64   52.1  <- min                 256  47.8
    //    128   53.5                         304  51.6
    //    304   81.0
    //
    // U-shaped in both, because two effects pull opposite ways: below the knee
    // the wide ops lose bandwidth they could have used, above it the
    // per-workgroup maintenance round costs more than the extra CUs return.
    // Pinning the ops that CANNOT use width — a norm reduces its row inside one
    // workgroup, an expand saturates at ceil(D/512) — removes their share of the
    // second effect, and the knee moves out from 64 to 128: the wide ops get to
    // keep the CUs the narrow ones were wasting.
    //
    // So this dial is NOT independent of the per-op rules. Re-sweep it whenever
    // they change; a per-op narrowing that is measured against a stale global
    // width will read as a smaller win than it is.
    //
...
mod tests {
    ...
    /// No packet may take more workgroups than its kernel can put to work.
    ///
    /// The packet floor is per-WORKGROUP cache maintenance (interp.hip's
    /// GATE_HIER note), so a workgroup that arrives, finds nothing to do and
    /// signals is not free — it is most of what a small packet costs. Measured
    /// on this program: RmsNorm at 128 CUs was 32.6 us and at 1 CU is 8.85.
    ///
    /// Each bound below is the KERNEL's grid mapping, read from the source, not
    /// a guess. Getting one wrong is invisible — the program stays correct and
    /// just runs slower — which is why it is asserted rather than commented.
    #[test]
    fn no_packet_takes_more_workgroups_than_its_kernel_can_use() {
        ...
        // d_hc_expand / d_v4_hc_mix are one element per thread over D — NOT over
        ...
        for (op, max, why) in [
            ...
            for inst in p.insts.iter().filter(|i| i.op == op as u16) {
                assert!(
                    u32::from(inst.blocks) <= max,
                    "{op:?}: emitted on {} workgroups, at most {max} can do work — {why}",
                    inst.blocks
                );
            }
        }
