...
/// The invariant `--cpu-twin` rests on: two INDEPENDENT emits of the same model
/// at the same context agree on how a sequence slot is cut, so a head's rows
/// can be copied from one into the other as a byte range.
///
/// Emitted for two different targets on purpose. If a target ever changed the
/// KV declarations, this is where it surfaces — and it must, because the
/// alternative is a slot whose rows `[0, n)` hold another geometry's bytes,
/// which faults nothing and reads as fluent wrong output.
#[test]
fn two_targets_at_one_context_share_a_kv_contract() {
    let _g = emit_guard();

    let dev = tempdir("kvc_device");
    write_qwen3_config(&dev);
    emit_arch(&dev, "gfx942", "MI300X", None);
    let device = devgen::kv_contract::last().expect("device emit records a contract");

    let twin = tempdir("kvc_twin");
    write_qwen3_config(&twin);
    emit_arch(&twin, "sm_120a", "RTX 6000 Ada", None);
    let cpu = devgen::kv_contract::last().expect("twin emit records a contract");

    assert_eq!(device, cpu, "same model, same context, same slot layout");
    assert!(!device.is_empty());
}

/// And the check is not vacuous: a twin compiled at another `--max-ctx` is a
/// different contract, because the caches are cut into slots of that context.
/// This is the mistake `--cpu-twin` exists to refuse.
#[test]
fn a_different_context_is_a_different_kv_contract() {
    let _g = emit_guard();

    let a = tempdir("kvc_ctx_a");
    write_qwen3_config(&a);
    emit(&a, 512, 128, 1);
    let at = devgen::kv_contract::last().expect("contract recorded");

    let b = tempdir("kvc_ctx_b");
    write_qwen3_config(&b);
    emit(&b, 1024, 128, 1);
    let bt = devgen::kv_contract::last().expect("contract recorded");

    assert_ne!(at, bt, "a slot of 1024 rows is not a slot of 512");
}

/// The contract covers the append-only caches and nothing else, so a model with
/// no KV at all still produces a stable (empty-set) digest rather than a panic.
#[test]
fn the_contract_names_the_caches_a_head_can_hand_over() {
    let _g = emit_guard();
    let td = tempdir("kvc_names");
    write_qwen3_config(&td);
    emit(&td, 512, 128, 1);

    let blob = std::fs::read(td.join("model.pkt")).unwrap();
    assert!(!blob.is_empty());
    let digest = devgen::kv_contract::last().unwrap();
    // Stable across a re-emit of the same inputs: the digest is over the
    // declarations, not over anything the emitter varies run to run.
    emit(&td, 512, 128, 1);
    assert_eq!(digest, devgen::kv_contract::last().unwrap());
}
