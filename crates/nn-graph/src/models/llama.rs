//! Llama / Mistral text decoder → symbolic operator graph.
//!
//! The standard pre-norm decoder: RMSNorm, GQA with RoPE, SwiGLU MLP. No
//! post-norms, no embedding scale, no qk-norm. Covers Llama 2/3, Mistral 7B,
//! CodeLlama, Vicuna, and all Llama-architecture derivatives.

use super::config::{parse_dtype, LlamaConfig};
use crate::op::ActKind;
use crate::Nn;
use crate::{DType, Dim, Graph, TensorId};

pub fn build(cfg: &LlamaConfig) -> Graph {
...
/// order, before the final hidden states) — FLUX.2-style multi-layer feature
/// extraction. Pass `&[]` for last-hidden-state only.
pub fn build_encoder(cfg: &LlamaConfig, taps: &[u32]) -> Graph {
    build_inner(cfg, Some(taps))
}

fn build_inner(cfg: &LlamaConfig, encoder_taps: Option<&[u32]>) -> Graph {
