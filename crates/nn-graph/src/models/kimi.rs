//! Kimi K2 (Moonshot) decoder → symbolic operator graph.
//!
...
//! * **DeepSeek-style MoE.** The first `first_k_dense_replace` layers use a
//!   dense MLP; the rest use a router + shared experts + routed experts.

use super::config::{parse_dtype, KimiConfig};
...
pub fn build(cfg: &KimiConfig) -> Graph {
    ...
    for layer in 0..cfg.num_hidden_layers {
        ...
        // FFN sub-block (pre-norm residual): dense for first K layers, MoE after.
        let residual = x;
...
fn mla_attention(
    ...
    nn.param_dtype(
        &format!("{p}.self_attn.rotary_emb.inv_freq"),
        [Dim::stat(qk_rope / 2)],
        DType::F32,
    );
    k_pe = nn.broadcast(
...
fn swiglu_mlp(nn: &mut Nn, cfg: &KimiConfig, p: &str, x: TensorId, h: i64, inter: i64) -> TensorId {
...
/// MoE: router logits + shared expert(s) + one representative routed expert.
fn moe(nn: &mut Nn, cfg: &KimiConfig, p: &str, x: TensorId, h: i64) -> TensorId {
