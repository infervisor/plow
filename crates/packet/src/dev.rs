...
impl DevOp {
    ...
    /// context parallelism).
    /// 194 -> 196 for `CompressPool = 194` / `RopeInverseO = 195` (DeepSeek-V4 CSA2). Both
    ...
    /// 196 -> 198 for `EngramGate = 196` / `EngramEmbed = 197` (DeepSeek-V4.1 Engram). The
    /// hash is host work and the `wkv` projection is an ordinary fp8 GEMM, so the family costs
    /// two opcodes rather than four.
    /// 198 -> 200 for `GemmFp8Mx = 198` (DeepSeek-V4.1's `[32, 32]` ue8m0 block-fp8 GEMM) and
    /// `CompressRopeQuant = 199` (its compressed-row rope + quant tail).
    ...
    pub fn gemv_case(self, i: &[u32; 8]) -> Option<(&'static str, u32, u32, u32, &'static str)> {
        let (fam, n, q) = match self {
            ...
            DevOp::GemvF32 => ("gemvf32", i[1], "None"),
