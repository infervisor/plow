...
pub fn inspect(image: &[u8]) -> Option<CubinInfo> {
    ...
    let shdr = |i: usize| -> Option<&[u8]> {
        ...
        image.get(o..o.checked_add(e_shentsize)?)
    };
...
/// Read one initialized scalar in an ELF64-LE cubin. NVIDIA device globals
/// have LOCAL ELF binding even when exported through cuModuleGetGlobal. Undefined, duplicate,
/// relocated, non-object, and zero-fill symbols are not capability evidence.
pub fn global_u32(image: &[u8], want: &str) -> Option<u32> {
    if !is_elf64_le(image) {
        return None;
    }
    let start = usize::try_from(u64_at(image, 0x28)?).ok()?;
    let stride = u16_at(image, 0x3a)? as usize;
    let count = u16_at(image, 0x3c)? as usize;
    if stride < 64 {
        return None;
    }
    let header = |i: usize| -> Option<&[u8]> {
        if i >= count {
            return None;
        }
        let at = start.checked_add(i.checked_mul(stride)?)?;
        image.get(at..at.checked_add(stride)?)
    };
    let data = |h: &[u8]| -> Option<&[u8]> {
        let at = usize::try_from(u64_at(h, 24)?).ok()?;
        let n = usize::try_from(u64_at(h, 32)?).ok()?;
        image.get(at..at.checked_add(n)?)
    };
    let mut value = None;
    for i in 0..count {
        let sh = header(i)?;
        ...
        if u64_at(sh, 56)? != SYM_SZ as u64 {
            return None;
        }
        let names = data(header(u32_at(sh, 40)? as usize)?)?;
        let symbols = data(sh)?;
        if symbols.len() % SYM_SZ != 0 {
            return None;
        }
        for sym in symbols.chunks_exact(SYM_SZ) {
            let name = names.get(u32_at(sym, 0)? as usize..)?;
            let end = name.iter().position(|&c| c == 0)?;
            if &name[..end] != want.as_bytes() {
                continue;
            }
            if value.is_some() || !matches!(sym[4], 0x01 | 0x11) || u64_at(sym, 16)? != 4 {
                return None;
            }
            let section = u16_at(sym, 6)? as usize;
            if section == 0 {
                return None;
            }
            let target = header(section)?;
            if u32_at(target, 4)? != 1 {
                return None;
            }
            let offset = u64_at(sym, 8)?.checked_sub(u64_at(target, 16)?)?;
            // Relocations can replace an initializer at module load.
            for r in 0..count {
                let rel = header(r)?;
                if matches!(u32_at(rel, 4)?, 4 | 9) && u32_at(rel, 44)? as usize == section {
                    let stride = if u32_at(rel, 4)? == 4 { 24 } else { 16 };
                    for entry in data(rel)?.chunks_exact(stride) {
                        let pos = u64_at(entry, 0)?;
                        if pos < offset.checked_add(4)? && pos.checked_add(8)? > offset {
                            return None;
                        }
                    }
                }
            }
            value = Some(u32_at(data(target)?, usize::try_from(offset).ok()?)?);
        }
    }
    value
}
...
#[doc(hidden)]
pub fn synthetic_elf(entry: &str, globals: &[(&str, u32)], sm: u32) -> Vec<u8> {
    let mut strings = vec![0u8];
    let mut symbols = Vec::new();
    for (index, name) in std::iter::once(entry)
        .chain(globals.iter().map(|(name, _)| *name))
        .enumerate()
    {
        let mut symbol = [0u8; SYM_SZ];
    ...
    for (index, kind, data) in [(1usize, 2u32, symbols), (2, 3, strings), (3, 1, constants)] {
    ...
    image[184..192].copy_from_slice(&(SYM_SZ as u64).to_le_bytes());
    image
}
