...
impl T3Engine {
    ...
    pub fn run(&mut self, jobs: &[T3Job], mut on_done: impl FnMut(usize, T3Output)) -> Result<()> {
        let mut outputs = vec![T3Output::default(); jobs.len()];
        let mut next = 0usize;
        let mut active: Vec<Option<Active>> = (0..self.pairs()).map(|_| None).collect();
        let mut toks = Vec::new();
        let mut feeds = Vec::new();
        let mut started: Vec<Option<std::time::Instant>> = vec![None; jobs.len()];
        loop {
            for pair in 0..active.len() {
                if active[pair].is_none() && next < jobs.len() {
                    let a = self.admit(jobs, next, pair, &mut outputs)?;
                    started[next] = Some(std::time::Instant::now());
                    active[pair] = Some(a);
                    next += 1;
                }
            }
            // Retire rows whose last draw ended them before stepping.
            for pair in 0..active.len() {
                ...
                if finished {
                    ...
                    o.decode_us = started[a.job_index].map_or(0, |t| t.elapsed().as_micros() as u64);
            ...
            for pair in 0..active.len() {
                let Some(a) = active[pair].as_mut() else { continue };
                if a.out.len() >= a.max {
                    continue;
                }
