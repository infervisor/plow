...
pub struct NvidiaRuntimeConfig {
    ...
    /// Name the first instruction that makes a decode rung incompatible with rung 0.
    /// The fallback to the widest rung is SILENT and reads exactly like a large kernel
    /// regression, so this is the only cheap way to tell those two apart.
    #[arg(long = "ladder-debug", env = "PLOW_LADDER_DEBUG", default_value_t = false, value_parser = clap::builder::BoolishValueParser::new(), action = clap::ArgAction::Set, require_equals = true, num_args = 0..=1, default_missing_value = "true", global = true)]
...
pub struct AmdRuntimeConfig {
    ...
    /// table is unread. Default on (checkpoint P, perf-certs/rt.union_skip.json); `=0` is the
    /// rollback.
