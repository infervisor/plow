...
const UNION_SKIP_QUALIFIED: Status = Status::Qualified {
    evidence: &[
        "perf-certs/rt.union_skip.json: checkpoint P accepts P8192-S 667.0 -> 648.3 ms (floor 3.7), P4096-S 391.0 -> 379.9 (floor 1.2), P8192-0 neutral, 16 serving cells not worse",
        "review log #92: glue2-t4 served A/B (off/on/off2/on2), C16 TTFT -39.0 / -23.1 ms and TPOT -2.66 / -1.92 ms beyond the P floor at ISL 16384 / 12288; retrieval 18/18 + 21/21 every arm",
        "docs/flags-reference.md: `=0` is the rollback",
    ],
};
...
/// Unset = on for an sm90a packet whose `attn_softmax_sm90a.cubin` is present; only one-KV-head
/// full-attention segments are routed.
const PF_ATTN_GEMM_QUALIFIED: Status = Status::Qualified {
    evidence: &[
        "agent/pf-attn-batched-gemm e8227eed (Gemma-4-12B H100): served C1 TTFT 4096 176.7 -> 170.6 ms, 8192 386 -> 360, 15000 832 -> 750; C4/C16 8192-15000 TTFT -7..-12%, TPOT -6..-12%",
        "exec/gpu/attention_gemm.rs sites: a packet with no one-KV-head full-attention segment (26B-A4B: 2 KV heads) routes nothing",
        "docs/flags-reference.md: `=0` is the rollback",
    ],
};
...
const DECODE_DENSE_EXACT_SCOPE: &[Allow] = &[Allow {
    kinds: &["decode"],
    rows: (8, 20),
    fields: &[ScopeField::Route],
    ..Allow::ANY
...
pub const RUNTIME: &[KnobSpec] = &[
    ...
    KnobSpec::new("rt.qkv_dot4", Some("PLOW_METAL_QKV_DOT4"), Layer::Runtime, Domain::Bool, UNSET, OPT_IN),
    KnobSpec::new("rt.decode_heads", Some("PLOW_METAL_DECODE_HEADS"), Layer::Runtime, Domain::Bool, UNSET, OPT_IN),
    KnobSpec::new("rt.glu_pair", Some("PLOW_METAL_GLU_PAIR"), Layer::Runtime, Domain::Bool, OFF, OPT_IN),
    KnobSpec::new("rt.mx4_dedicated", Some("PLOW_METAL_MX4_DEDICATED"), Layer::Runtime, Domain::Bool, OFF, OPT_IN),
    ...
    KnobSpec::new("rt.pf_attn_gemm", Some("PLOW_PF_ATTN_GEMM"), Layer::Runtime, Domain::Bool, UNSET, PF_ATTN_GEMM_QUALIFIED),
...
pub const RAW_ENV: &[KnobSpec] = &[
    ...
    KnobSpec::new("env.PLOW_TEST_ALIGN_CAPTURE", Some("PLOW_TEST_ALIGN_CAPTURE"), Layer::RawEnv, Domain::Str, UNSET, DIAG),
    KnobSpec::new("env.PLOW_TEST_ALIGN_NPART", Some("PLOW_TEST_ALIGN_NPART"), Layer::RawEnv, Domain::Str, UNSET, DIAG),
    KnobSpec::new("env.PLOW_TEST_ALIGN_OBJ", Some("PLOW_TEST_ALIGN_OBJ"), Layer::RawEnv, Domain::Str, UNSET, DIAG),
