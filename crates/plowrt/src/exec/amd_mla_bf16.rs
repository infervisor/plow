...
impl SplitMla {
    fn load(be: &HsaBackend, dir: &Path, modules: &mut Vec<Module>) -> Result<Self> {
        ...
        let reduce = kernel(be, m, "_fwd_kernel_stage2_asm", 136, 0)?;
...
impl PersistentMla {
    pub fn load(be: &HsaBackend, dir: &Path, rows: u32, modules: &mut Vec<Module>) -> Result<Self> {
        ...
        let metadata = kernel(
            be,
            &m,
            "_Z33kn_get_mla_metadata_v1_2_parallelI20MlaMetadataV12TraitsILi128ELb0ELi1ELb1ELb0EEEv28MlaMetadataV1KernelParameter",
            392,
            0,
        )?;
        let m = load(
        ...
        let reduce = kernel(
            be,
            &m,
            "_Z16kn_mla_reduce_v1I23MlaReduceKernelV1TraitsILi512ELi16ELi1EEfDF16bEv23MlaReduceKernelV1Params24MlaReduceKernelV1Configs",
            84,
            0,
        )?;
        let r = u64::from(rows);
    ...
    pub fn enqueue(&self, be: &HsaBackend, route: Route, table: &[u8]) -> Result<()> {
        if route.rows > self.rows_cap
            || route.max_live == 0
            || !supported_rows(route.rows)
            || (route.rows >= 32 && self.split.is_none())
        {
...
mod tests {
    ...
    fn bf16_route_guards_geometry_counters_capacity_and_live_lengths() {
        for rows in [1, 8, 16, 31, 32, 64] {
            ...
            assert_eq!(r.dispatch_count(), if rows >= 32 { 4 } else { 5 });
    ...
    fn bf16_split_hsa_reference_replay() {
        ...
        for case in cases {
            ...
            route.arm(&lengths);
            assert_eq!(route.dispatch_count(), 4);
            for poison in [0x55, 0xaa] {
                ...
                EngineDevice::upload(&be, &devices[0], 0, &vec![poison; expected.len() + 128])
                    .unwrap();
                be.begin_dispatch_chain(route.dispatch_count()).unwrap();
                kernel
