...
const SPLIT_STAGE: &str = "mla_dec_stage1_bf16_a16w16_subQ16_mqa16.co";
const SPLIT_REDUCE: &str =
    "attention-reduce-3ca36e1de81b9fe111c2e1681fb8339af1639d37383e3ba868b3f79877bb7c1d.elf";

fn supported_rows(rows: u32) -> bool {
    (1..=32).contains(&rows) || rows == 64
}
...
impl Route {
    ...
    pub fn plan_metadata(&mut self, previous: &mut Option<MetadataKey>, enabled: bool) {
        self.reuse_metadata = enabled && self.rows < 32 && self.max_live != 0 && *previous == Some(self.metadata_key);
        *previous = (self.rows < 32 && self.max_live != 0).then_some(self.metadata_key);
    }

    pub fn launches(self) -> usize {
        if self.rows >= 32 { return 4; }
...
pub(super) fn route(prog: &DevProg, tensors: &[DevTensor], seg: usize) -> Result<Option<Route>> {
    ...
    if !supported_rows(prog.t)
        ...
        || merge.t[1..3] != f.t[..2]
        || (prog.t >= 32 && merge.i[4] != 0)
        || !matches!(merge.i[4], 0 | 1024)
        || merge.i[5..].iter().any(|&v| v != 0)
    {
        return Err(invalid(
            "requires QH8 latent512/rope64, BF16, top2048 or short dense, M1..32 or M64",
        ));
    }
...
fn load<'a>(
    ...
    if file == SPLIT_STAGE {
        if image.get(0xf48..0xf4c) != Some(&[0, 0, 0, 0]) {
            return Err(invalid("split stage descriptor"));
        }
        image[0xf48..0xf4c].copy_from_slice(&208u32.to_le_bytes());
    }
...
impl SplitMla {
    fn load(be: &HsaBackend, dir: &Path, modules: &mut Vec<Module>) -> Result<Self> {
        let m = load(
            be,
            dir,
            SPLIT_STAGE,
            "5bfd90e2f981f5ff9828258d8843a122c28ce790c98ee89ae44e0a6e19c9f09a",
            modules,
        )?;
        let stage = kernel_raw(
            be,
            m,
        ...
        let m = load(
            ...
            dir,
            SPLIT_REDUCE,
            "3ca36e1de81b9fe111c2e1681fb8339af1639d37383e3ba868b3f79877bb7c1d",
            modules,
        )?;
        let reduce = kernel_raw(be, m, "_fwd_kernel_stage2_asm", 136, 0)?;
        let mut values = Vec::<u32>::new();
        for rows in [32, 64] {
            values.extend((0..=rows).map(|b| b * (256 / rows)));
            values.extend(std::iter::repeat_n(256 / rows, rows as usize));
        }
        let metadata = EngineDevice::alloc(be, (values.len() * 4) as u64)?;
        EngineDevice::upload(be, &metadata, 0, bytemuck::cast_slice(&values))?;
        Ok(Self {
            stage,
            reduce,
            metadata,
    ...
    fn enqueue(&self, be: &HsaBackend, rows: u32, ptr: &[u64; 14], lse: u64) -> Result<()> {
        let splits = 256 / rows;
        let splitptr = self.metadata.base + if rows == 64 { 65 * 4 } else { 0 };
        let valid = splitptr + u64::from(rows + 1) * 4;
        let [q, kv, kvi, qp, kp, last, ..] = *ptr;
        let (part, out) = (ptr[12], ptr[13]);
        let mut args = [0u64; 26];
        for (i, v) in [
            (0, part),
            (2, lse),
            (4, q),
...
impl PersistentMla {
    ...
    pub fn load(be: &HsaBackend, dir: &Path, rows: u32, modules: &mut Vec<Module>) -> Result<Self> {
        ...
        {
            return Err(invalid(
                "requires gfx950/256CU/160KiB LDS and M1..32 or M64",
            ));
        }
        let m = load(
            be,
        ...
        let split = if rows >= 32 {
            Some(SplitMla::load(be, dir, modules)?)
        } else {
            None
        };
        Ok(Self {
    ...
    pub fn enqueue(&self, be: &HsaBackend, route: Route, table: &[u8]) -> Result<()> {
        if route.rows > self.rows_cap || route.max_live == 0
            || !supported_rows(route.rows) || (route.rows >= 32 && self.split.is_none()) {
...
mod tests {
    ...
    #[test]
    fn split_routes_do_not_reuse_persistent_metadata_or_claim_its_evidence() {
        let (p, tensors) = fixture(16, 8192);
        let mut persistent = route(&p, &tensors, 0).unwrap().unwrap();
        persistent.arm(&[512; 16]);
        for rows in [32, 64] {
            let mut previous = None;
            persistent.plan_metadata(&mut previous, true);
            assert!(previous.is_some());
            let (mut p, tensors) = fixture(rows, 8192);
            let mut split = route(&p, &tensors, 0).unwrap().unwrap();
            split.arm(&vec![512; rows as usize]);
            split.plan_metadata(&mut previous, true);
            assert_eq!(split.launches(), 4);
            assert!(!split.reuse_metadata);
            assert_eq!(previous, None);
            assert!(split.evidence(rows, true, |_| panic!("split evidence is unsupported")).is_err());
            persistent.plan_metadata(&mut previous, true);
            assert_eq!(persistent.launches(), 5);
            p.insts[1].i[4] = 1024;
            assert!(route(&p, &tensors, 0).is_err());
        }
    }
    ...
    fn bf16_route_guards_geometry_counters_capacity_and_live_lengths() {
        for rows in [1, 8, 16, 31, 32, 64] {
            ...
            assert_eq!(r.launches(), if rows >= 32 { 4 } else { 5 });
    ...
    #[test]
    #[ignore = "requires queued gfx950 GPU and PLOW_TEST_AITER_DIR with split fixtures/objects"]
    fn bf16_split_hsa_reference_replay() {
        let dir = std::env::var("PLOW_TEST_AITER_DIR").unwrap();
        let dir = Path::new(&dir);
        let be = HsaBackend::new(0).unwrap();
        let mut modules = Vec::new();
        let kernel = PersistentMla::load(&be, dir, 64, &mut modules).unwrap();
        ...
        for case in cases {
            ...
            assert_eq!(word(0), 0x41535031);
            let (rows, ctx, splits, nnz) = (word(1), word(2), word(3), word(4));
            assert!(rows == 32 || rows == 64);
            assert_eq!(splits, 256 / rows);
            let sizes = [
                rows * 16 * 576 * 2,
                rows * ctx * 576 * 2,
                (rows + 1) * 4,
                ...
                rows * 16 * 512 * 2,
                rows * 4,
                rows * splits * 16 * 512 * 4,
                rows * splits * 16 * 4,
            ];
            let mut at = 20;
            let data = sizes.map(|size| {
                let value = &bytes[at..at + size];
                at += size;
                value
            });
            assert_eq!(at, bytes.len());
            let ptr: Vec<usize> = data[3]
                .chunks_exact(4)
                .map(|v| u32::from_le_bytes(v.try_into().unwrap()) as usize)
                .collect();
            let lengths: Vec<u32> = ptr
                .windows(2)
                .map(|v| {
                    if v[1] - v[0] == 2048 {
                        ctx as u32
            ...
            for row in 0..rows * 8 {
                let src = row * 2304;
                assert_eq!(&data[0][src..src + 1152], &data[0][src + 1152..src + 2304]);
                inputs[1][row * 1024..(row + 1) * 1024].copy_from_slice(&data[0][src..src + 1024]);
                inputs[2][row * 128..(row + 1) * 128]
                    .copy_from_slice(&data[0][src + 1024..src + 1152]);
            }
            for (row, src) in data[1].chunks_exact(1152).enumerate() {
                inputs[3][row * 1024..(row + 1) * 1024].copy_from_slice(&src[..1024]);
                inputs[4][row * 128..(row + 1) * 128].copy_from_slice(&src[1024..]);
            }
            for b in 0..rows {
                for j in ptr[b]..ptr[b + 1] {
                    let index = u32::from_le_bytes(data[4][j * 4..j * 4 + 4].try_into().unwrap())
                        - (b * ctx) as u32;
                    assert!(index < lengths[b]);
                    let offset = (b * 2048 + j - ptr[b]) * 4;
                    inputs[6][offset..offset + 4].copy_from_slice(&index.to_le_bytes());
                }
            }
            let padded = if rows == 64 && ctx == 71680 {
                let corrected: serde_json::Value = serde_json::from_slice(
                    &std::fs::read(dir.join("compact/comparison.json")).unwrap(),
                )
                .unwrap();
                let reference = corrected["cases"]
                    .as_array()
                    .unwrap()
                    .iter()
                    .find(|v| v["case"].as_str().unwrap() == name.replace(".bin", ".json"))
                    .unwrap();
                let output = std::fs::read(
                    dir.join("compact")
                        .join(reference["output"]["file"].as_str().unwrap()),
                )
                .unwrap();
                assert_eq!(
                    plow_asset::decode_objects::image_sha256(&output),
                    reference["output"]["sha256"].as_str().unwrap()
                );
                output
            } else {
                data[7].to_vec()
            ...
            let devices: Vec<_> = inputs
                ...
                .map(|(i, v)| {
                    let mem =
                        EngineDevice::alloc(&be, (v.len() + if i == 0 { 128 } else { 0 }) as u64)
                            .unwrap();
                    EngineDevice::upload(&be, &mem, 0, v).unwrap();
                    mem
                })
                .collect();
            let mut table: Vec<u64> = devices.iter().map(|v| v.base).collect();
            table[0] += 64;
            let (prog, tensors) = fixture(rows as u32, ctx as u32);
            let mut route = route(&prog, &tensors, 0).unwrap().unwrap();
            route.arm(&lengths);
            assert_eq!(route.launches(), 4);
            for poison in [0x55, 0xaa] {
                EngineDevice::upload(
                    &be,
                    &kernel._scratch,
                    0,
                    &vec![poison; kernel._scratch.len as usize],
                )
                .unwrap();
                EngineDevice::upload(&be, &devices[0], 0, &vec![poison; expected.len() + 128])
                    .unwrap();
                ...
                let mut output = vec![0; expected.len() + 128];
                EngineDevice::download(&be, &devices[0], 0, &mut output).unwrap();
                assert_eq!(&output[..64], &[poison; 64]);
                assert_eq!(&output[64 + expected.len()..], &[poison; 64]);
                assert_eq!(
                    &output[64..64 + expected.len()],
                    expected,
                    "{name} poison={poison}"
                );
            }
            eprintln!("BF16 split Rust replay {name}: two poisons/guards PASS");
        }
    }
