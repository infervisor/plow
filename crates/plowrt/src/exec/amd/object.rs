...
pub(super) const PREFILL_ARM_MARKERS: &[(&str, &[&str])] = &[
    ...
    // DeepSeek-V4 CSA2 (ops 194/195). CONDITIONALLY compiled, like PLOW_MOE_PF_ATOMIC below: an
    ...
    // Engram (op 196). Same standing as CSA2 above, and the silent failure is worse: the
...
pub(super) fn check_dsa_select_local(
    ...
    for p in progs {
            ...
            .filter(|d| d.op == DevOp::IndexSelect as u16 && d.i[4] != 0)
        {
            ...
            // per-row strips, on decode rungs only.
            let split = d.i[4] == 2;
            let g = if split { d.i[5] } else { 1 };
            let phase = if split { d.i[6] } else { 0 };
            ...
            let per_row = if split && phase != 3 { g } else { 1 };
            let geometry = if split {
                [2048, 0, 0, 2, g, phase, 0]
            } else {
                [2048, 0, 0, 1, 0, 0, 0]
            };
            let unbound: &[usize] = if split { &[7] } else { &[2, 3, 5, 6, 7] };
            if arch != "gfx942"
                || !tp8
