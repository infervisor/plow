...
impl AmdEngine {
    ...
    pub(crate) fn preflight_segment_batch(&self, p: usize, seg: usize) -> Result<crate::device::kernarg_retirement::Admission> {
        ...
        // These routes can synchronize/restage CSR inside enqueue. That needs
        // an all-rank preparation boundary, not a reserved immediate batch.
        let (timed_native, restages_csr) = match self.progs[p].prefill_routes.get(seg) {
            Some(PrefillSegmentRoute::SparseMla(route)) => {
                let native = self.packed_prefill.is_some_and(|b| b.prog == p)
                    || route.active || route.split_row0 != 0;
                (native, native && (route.native_lo || route.is_rowsplit()))
            }
            Some(PrefillSegmentRoute::IndexTp(_)) => (true, false),
            _ => (false, false),
        };
        if prefill_segment_specialization_allowed(self.prog_dispatch(p))
            && (restages_csr || timed_native && crate::config::RuntimeConfig::get().amd.native_launch_timing) {
            return Err(RuntimeError::Rejected(
                "TP segment admission does not cover internal native timing/CSR drains".into()
            ));
        }
