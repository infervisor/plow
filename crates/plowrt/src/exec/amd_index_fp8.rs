...
mod tests {
    ...
    fn index_fp8_native_chain_reference_replay() {
        ...
        let model_capture = report["model_capture"].as_bool().unwrap_or(false);
        assert!(!model_capture || (!sweep && !headroom));
        ...
        let expected_coverage: std::collections::BTreeSet<_> = if model_capture {
            ["model-8193", "model-8194", "model-8195", "model-8196"]
                .into_iter()
                .map(|profile| (1, 131072, profile))
                .collect()
        } else if sweep {
            [1, 8, 16, 32, 64]
                .into_iter()
        ...
        for case in cases {
            ...
            let lengths: Vec<_> = inputs[6]
                ...
                .collect();
            if model_capture {
                let length: u32 = case["profile"].as_str().unwrap()[6..].parse().unwrap();
                assert_eq!(lengths, [length]);
