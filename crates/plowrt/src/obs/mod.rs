...
#[derive(Default)]
pub struct Metrics {
    pub serving: serving::ServingMetrics,
...
impl Metrics {
    pub fn accumulate_counters(&self, other: &Self) {
        self.requests
            .fetch_add(other.requests.load(Ordering::Relaxed), Ordering::Relaxed);
        self.tokens
            .fetch_add(other.tokens.load(Ordering::Relaxed), Ordering::Relaxed);
        self.rejected
            .fetch_add(other.rejected.load(Ordering::Relaxed), Ordering::Relaxed);
        self.batch_size_sum.fetch_add(
            other.batch_size_sum.load(Ordering::Relaxed),
            Ordering::Relaxed,
        );
        self.batch_count
            .fetch_add(other.batch_count.load(Ordering::Relaxed), Ordering::Relaxed);
        self.hold_ms_sum
            .fetch_add(other.hold_ms_sum.load(Ordering::Relaxed), Ordering::Relaxed);
        self.hold_count
            .fetch_add(other.hold_count.load(Ordering::Relaxed), Ordering::Relaxed);
        self.admit_shed
    ...
    pub fn accumulate(&self, other: &Self) {
        self.accumulate_counters(other);
        self.queued_requests.fetch_add(
            other.queued_requests.load(Ordering::Relaxed),
            Ordering::Relaxed,
        );
        self.lambda_milli.fetch_add(
            other.lambda_milli.load(Ordering::Relaxed),
            Ordering::Relaxed,
        );
        self.util_milli
            .fetch_add(other.util_milli.load(Ordering::Relaxed), Ordering::Relaxed);
        self.decode_rung_actual.fetch_max(
            other.decode_rung_actual.load(Ordering::Relaxed),
            Ordering::Relaxed,
        );
        self.decode_rung_admission.fetch_max(
            other.decode_rung_admission.load(Ordering::Relaxed),
            Ordering::Relaxed,
        );
        self.decode_occupied_extent.fetch_max(
    ...
    pub fn to_prometheus(&self) -> String {
        ...
        format!(
            "# HELP plowrt_requests_total Requests submitted to dispatchers or the direct reference API.\n\
             # TYPE plowrt_requests_total counter\n\
             plowrt_requests_total {}\n\
             # HELP plowrt_tokens_total Tokens generated by model dispatchers.\n\
