...
fn main() {
    ...
    let path = args
        .next()
        .expect("usage: tilegraph_stat <asset-dir|model.pkt> [T]");
    ...
    let tname = |h: u16| -> &str {
        if h == TENSOR_NONE16 {
            ...
            names.get(h as usize).copied().unwrap_or("?")
        }
    };
    ...
    for (cu, (&ofs, &len)) in prog
        .stream_ofs
        .iter()
        .zip(prog.stream_len.iter())
        .enumerate()
    {
        let _ = ofs;
    ...
    let (lo, hi) = (
        per_cu.iter().copied().min().unwrap_or(0),
        per_cu.iter().copied().max().unwrap_or(0),
    );
    let fine = prog
        .stream
        .iter()
    ...
    for (id, inst) in prog.insts.iter().enumerate() {
        for slot in 1..8 {
            ...
            if is_tiled_gemm(inst.op) && slot == 1 {
                ...
                let elem = if op_name(inst.op).ends_with("Fp8") {
                    1
                } else {
                    2
                };
                fanin = if is_tiled_gemm(p.op) {
                    pn.div_ceil(BN)
                } else {
                    p.blocks as u64
                };
            ...
            println!(
                "{:<32} {fanin:>9} {kib:>12.1} {over:>11.1}x {busy:>11}  {kind}",
    ...
    println!(
        "tightest max_busy_CU   {worst_busy} of {n_cu} ({:.1}% of the machine)",
    ...
    for (id, inst) in prog.insts.iter().enumerate() {
        ...
        let staging: u64 = if op_name(inst.op).contains("Glu") {
            2 * 30720
        } else {
            30720
        };
        ...
        println!(
            "{:<24} {k:>7} {elem:>7} {bm_cap:>8} {bm_free:>8} {mult:>11.1}x {:>12}",
            format!("{id} {}", op_name(inst.op)),
            m.div_ceil(bm_free)
        );
    }
}
