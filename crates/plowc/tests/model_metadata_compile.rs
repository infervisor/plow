...
// deepseek-ai/DeepSeek-V4-Flash-0731's config.json, VERBATIM. Not scaled: the
// point of this fixture is that the released numbers parse and validate
// (compress_ratios reconciling against 43 layers + 3 DSpark blocks, o_groups
// dividing 64 heads, the YaRN factor reproducing max_position_embeddings), so
// the refusal below is an emit-time gap list rather than a config-parse dead end.
...
fn packet_routes_missing_from_model_source_fail_closed_instead_of_generic_lowering() {
    for (model_id, config, reason) in [
        ...
        (
            ...
            "deepseek_v4 graph emit is not implemented",
        ),
        (
