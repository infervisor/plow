...
fn simple_candidate() -> TileCandidate {
    ...
    TileCandidate {
        gemm: GemmShapeJ { m: 128, n: 128, k: 64 },
        tile: TileShapeJ { bm: 64, bn: 64, bk: 32 },
...
fn rejects_zero_bm() {
    ...
    c.tile.bm = 0;
    let req = TilePartitionRequest { candidates: vec![c] };
    let cert = check_tile_partition(&req).expect("verifier call");
...
fn accepts_non_divisor_partition_with_slack_bound() {
    ...
    let c = TileCandidate {
        gemm: GemmShapeJ { m: 100, n: 100, k: 64 },
        tile: TileShapeJ { bm: 32, bn: 32, bk: 32 },
        cost_bound: 1_048_576,
    };
    let req = TilePartitionRequest { candidates: vec![c] };
    let cert = check_tile_partition(&req).expect("verifier call");
