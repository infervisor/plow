...
Two claims the emit makes by OMISSION, priced here before either is changed.
  ...
  2. `Attention.forward` ends with `apply_rotary_emb(o[..., -rd:], freqs_cis, True)`
     ...
     `FlashMerge`. Op 181 exists for exactly this and has no emit site.
...
def main():
    ...
    # ---- the inverse -----------------------------------------------------------------
    # op 181 is the conjugate, and `sin -> -sin` is the whole of it. Round-tripping is the
    ...
    print(f"[4] irope(rope(q)) == q (op 181 is the conjugate): max|err| = {d_rt:.3e}")
    ok4 = d_rt < 1e-4
    ...
    print(f"[5] attention output with vs without op 181    : max|err| = {d_o:.3e}")
    ok5 = d_o > 1e-3
    ...
    checks = {
        ...
        "op 181's sin -> -sin is exactly the inverse of the forward rope": ok4,
        "omitting op 181 leaves the query's rotation in the attention output": ok5,
    }
