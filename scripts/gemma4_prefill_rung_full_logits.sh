#!/usr/bin/env bash
# One-process Gemma-4 prefill-rung full-logit sweep. The wrapper takes one GPU
...
if [[ -z "${PLOW_GEMMA4_FULL_LOGITS_LEASED:-}" ]]; then
  ...
  exec "$LEASE" -n 1 gemma4-prefill-full-logits \
...
export TEST_PREFILL_RUNG_BASELINE="$REFERENCE"
export TEST_RUNG_PACKET_SHA256="$PACKET_SHA256"
export TEST_PREFILL_RUNG_LOGITS_OUT="$OUTPUT"
exec "$BIN" gpu_isolated_prefill_rungs_match_widest_logits \
