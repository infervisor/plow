#!/usr/bin/env python3
"""Attribute a plow GPU drain to ops, splitting STALL from BODY, from PLOW_TRACE_RAW.
...
Per PACKET, over the workgroups that ran it:
    stall = max(t_ready) - min(t_arrive)   waiting on its wait-counters
    body  = max(t_end)   - max(t_ready)    the op itself
    span  = max(t_end)   - min(t_arrive)

Summing spans gives the serialized chain. The STALL column is what a packet spent
waiting for its producers, i.e. the cost of the decomposition rather than of the
arithmetic — which is the number that decides whether to tune a kernel or to fuse
...
print()
print(f"{'op':<24}{'n':>5}{'wg/pkt':>8}{'stall ms':>10}{'body ms':>10}{'span ms':>10}{'%span':>8}")
...
print()
print(f"{'TOTAL (serialized)':<24}{len(packets):>5}{'':>8}"
      f"{tot_stall/1e3:>10.3f}{tot_body/1e3:>10.3f}{tot/1e3:>10.3f}{100:>8.1f}")
print(f"    stall share of the chain : {100*tot_stall/tot:.1f}%")
