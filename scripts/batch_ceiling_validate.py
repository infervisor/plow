...
def main():
    ...
    output_len = args.warmup + args.measured + 1
    ...
    if report.get("num_gpus") != 1 or report.get("parallel") != "tp":
        fail("batch ceiling requires the single-GPU production engine")
    if len(prompts) != args.batch or any(len(row) != args.ctx for row in prompts):
        fail("prompt rows do not preserve the requested batch and context")
    ...
    if report.get("warmup_requests") != 0:
        fail("request warmups would change the dispatch window")
    if report.get("prompt_tokens") != args.batch * args.ctx:
        fail("prompt token count changed")
    if report.get("output_tokens") != args.batch * output_len:
    ...
    if not isinstance(outputs, list) or len(outputs) != args.batch or any(
        not isinstance(row, list) or len(row) != output_len for row in outputs
    ):
    ...
    if any(row != outputs[0] for row in outputs[1:]):
        fail("same-prompt slots produced different streams")
    ...
    print(f"  {args.measured} dispatches x batch {args.batch} at ctx={args.ctx} after {args.warmup} warmups:")
