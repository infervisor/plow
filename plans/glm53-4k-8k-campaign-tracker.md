...
With step 4 at ≈ −4 and step 6 at ≈ −8, steps 1–7 predict ≈ 562–566 ms: the goal needs step 8 or new levers. A lever hunt over attention upper half (136 ms), MoE experts (106), collective transfer (93) and GEMM picks is looking for another −25…−40 ms. Rank-3 skew is the card: with 65:00.0 rotated to rank 7 (confirmed on NUMA node 1), its native time stays +5.9 ms over the other ranks' median (+5.86 as rank 3, +5.92 as rank 7) and its late arrivals move with it (109 → 122 of 405), while rank 3 on another card drops to +2.7 ms; per-device probe +2.0…+3.2% on the pinned attention kernel. Reseating or swapping it is worth ≈ −3…−5 ms served.
