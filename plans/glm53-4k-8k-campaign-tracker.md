...
Detailed log: `docs/bringup/tp-bringup-upstream-review-log.md` (rows #81–#92); live rung board `plans/rung-board.md` (local)
...
| G4 MoE shared seed | P8192-S | `PLOW_GLM_MOE_SHARED_SEED` | opt-in (#86), T4 pass on P floor (with G1) | T3 −7.3 ms (floor 2.3); P4096-S −3.3; P8192-0 −8.6 | last P rung P2048-S2048 measured (`glue2-t3-g4-serve2k`, rc 0); certificate and flip-2 patch in progress; emit default lands with the next production serving-set regeneration |
| begin_slot copy-out | P8192-0 admission | `PLOW_VMM_RELEASE_RETIRE` | opt-in, v3 T4 FAIL | baseline clear 102.8 ms; v1 smoke FAIL (0.4–119 ms); spare probe: copy into mapped spares 11.15 ms serial / 2.13 ms parallel (8 GPUs × 78 blocks); v2 smoke2 FAIL (117 ms max: with no spare, 78 maps per rank serialize to 115–153 ms and lose the race to admission) ; v3 (spare pool mapped at load, 354 MiB/rank from the existing KV pool cap; copies outside the process lock; 5 ms bounded wait) smoke3 PASS: every clear 0.38–0.90 ms, 0 inline unmaps, all copies from mapped spares; **v3 T4 FAIL**: 8192 C1 clear 100 → 0.4 ms and TTFT −90 ms, but the copy-out moved into the prefill tick's deferred publish (38–47 → 60–118 ms on reused slots), so ITL[0] 71 → 107–130 ms at 4096 C1 and 67 → 115 ms at 8192 C16; E2E median −40…−50 ms (8192 C1), +50…+64 ms (4096 C1), +250…+300 ms (8192 C16); TPOT beyond the P floor in every cell; decode ticks identical across arms | v4: zero-copy handoff, or unawaited parallel copies; T4 scored on ITL[0] and E2E too |
