...
| Admission path | served TTFT | — | building (admission agent) | 8192 C1 host split (PLOW_TTFT_LOG): handler before submit 11.9 ms, tokenize 12.9, queue 9.2, HTTP 1.7; handler cost is `vocab_size()` → `get_vocab_size(true)` rebuilding the 154,856-entry map per request; queue is the formation hold (`max_hold_ms` 8) at λ=0; `encode_fast` 9.6 → 7.2 ms (CPU) | vocab cache at load (bug fix), no hold when the mux is idle (opt-in), `encode_fast` + parallel encode; served C1 host split |
...
|---|---|---:|---|
| 1–3 | vocab-size cache, no formation hold when idle, `encode_fast` + parallel encode | −12, −8, −2.4…−6 | admission agent |
...
| 5 | serving-set regeneration A: G4 + fusepost | −10.8 | regeneration agent; includes the object build-script fix (`glm_fold_lt` compared to "true" only) |
| 6 | glue G3/G5/G6-G7 | −15…−23 | glue agent |
| 7 | banded all-gather K=4 | −5 | seams agent |
| 8 | W8A8 shared expert + FP8 MoE-input gather | −15…−20 | needs a numerics decision (FFN rel-L2 1.7% → 4.8%) |
| 9 | Band64 band chain in a sibling program selected only with band rows > 0 | 0 at C1 | after token equality (v29 FAIL) |

With step 4 at ≈ −4, steps 1–7 predict ≈ 550–560 ms, so step 8 or a hardware fix on rank 3 is likely needed for margin.
