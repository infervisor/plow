...
__device__ void d_flash_prefill(float* __restrict__ Opart, float* __restrict__ mlpart,
     ...
     * BKV > 32: each lane owns BKV/32 consecutive columns, reduced locally before the warp op. */
...
#pragma unroll
                for (int sc = 0; sc < SOFT_COLS; sc++) {
                    ...
                    if (col < rmax) {
                        ...
                        if (!masked) { sv[sc] = Ss[row * BKV + col]; act[sc] = true; }
                    }
                }
...
#pragma unroll
                for (int sc = 0; sc < SOFT_COLS; sc++) {
                    const unsigned col = lane + sc * 32;
                    const float p = (act[sc] && m_new != FA_NEG_INF) ? FA_EXP(sv[sc] - m_new) : 0.0f;
                    if (col < (unsigned)BKV) Ps[row * (BKV + PAD) + col] = __float2bfloat16(p);
                    psum += p;
                }
