...
| `gemma4_ws384_fused_glu.cu` | Can an exact-shape Hopper WS384 role fuse Gemma-4 gate/up+GeGLU while preserving the split packet's BF16 boundaries? | **YES.** One TMA producer warpgroup and two register-donated consumer warpgroups, BN128, NS4, 160 registers and zero spills. Five-seed H100 runs are bit-exact against the production BN256/NS3 two-GEMM + `d_glu` chain and improve min time **1.19x at M4096** and **1.17x at M8192**. NS3 reaches 1.13x/1.14x. Packet-role integration remains required to measure the launch and segment-count win. |
