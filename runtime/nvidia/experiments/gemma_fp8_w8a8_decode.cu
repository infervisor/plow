...
__global__ void w8a16_control(bf16* y, const bf16* x, const uint8_t* w, const uint8_t* u,
    const float* ws, const float* us, unsigned M, unsigned N, unsigned K, unsigned first, unsigned blocks) {
    ...
    if constexpr (Glu) d_gemv_glu_fp8(y,x,w,u,ws,us,M,N,K,PLOW_ACT_GELU_TANH_,slice,blocks);
...
static void run(unsigned M,unsigned N,unsigned K,unsigned blocks,bool glu,bool exact,unsigned* flush,bool qkv=false) {
    ...
    std::vector<bf16> x((size_t)M*K),control((size_t)M*N),out(control.size());
...
#undef LAUNCH
    ...
    for(unsigned mode=1;mode<3;++mode){
        ...
        for(unsigned slice:slices){CHECK(cudaMemset(storage,0x5a,(out.size()+128)*2));launch(mode,slice,exact?1:blocks);CHECK(cudaMemcpy(out.data(),dy,out.size()*2,cudaMemcpyDeviceToHost));
            ...
            const unsigned per=(N+blocks-1)/blocks;double diff2=0,norm2=0,maxabs=0,oracle2=0,oracle_norm2=0;unsigned changed=0,samples=0;
            ...
            if((exact&&changed)||oracle>(mode==1?.02:.004)){fprintf(stderr,"oracle failure mode%u exact%u changed%u rel%g\n",mode,exact,changed,oracle);exit(2);}
            printf("check M=%u N=%u K=%u blocks=%u glu=%u qkv=%u mode=%u slice=%u exact=%u quant_bytes_exact=1 quant_scales_exact=1 changed=%u bf16_relL2=%.9g max_abs=%.9g quantized_oracle_relL2=%.9g samples=%u PASS\n",M,N,K,blocks,glu,qkv,mode,slice,exact,changed,relative,maxabs,oracle,samples);
        }
    }
    if(!exact)for(unsigned mode=0;mode<3;++mode)for(unsigned include_quant=0;include_quant<2;++include_quant){if(!mode&&include_quant)continue;
        for(unsigned rep=0;rep<4;++rep){if(include_quant)quant();launch(mode,0,blocks);}CHECK(cudaDeviceSynchronize());std::vector<float> times;
        for(unsigned rep=0;rep<15;++rep){flush_w8a8<<<528,256>>>(flush);CHECK(cudaEventRecord(begin));if(include_quant)quant();launch(mode,0,blocks);CHECK(cudaEventRecord(end));CHECK(cudaEventSynchronize(end));float ms;CHECK(cudaEventElapsedTime(&ms,begin,end));times.push_back(ms*1000);}
