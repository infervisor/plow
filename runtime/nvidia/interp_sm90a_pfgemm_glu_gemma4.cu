...
extern "C" __device__ unsigned plow_pfgemm_glu_gemma4_abi = 1;
extern "C" __device__ unsigned plow_pfgemm_glu_gemma4_min_rows = 4096;
extern "C" __device__ unsigned plow_pfgemm_glu_gemma4_max_rows = 8192;
extern "C" __device__ unsigned plow_pfgemm_glu_gemma4_n = 15360;
extern "C" __device__ unsigned plow_pfgemm_glu_gemma4_k = 3840;
extern "C" __device__ unsigned plow_pfgemm_glu_gemma4_stages = PGM90_GEMMA4_GLU_STAGES;
extern "C" __device__ unsigned plow_pfgemm_glu_gemma4_bm = PGM90_GEMMA4_GLU_BM;
extern "C" __device__ unsigned plow_pfgemm_glu_gemma4_bn = PGM90_GEMMA4_GLU_BN;
extern "C" __device__ unsigned plow_pfgemm_glu_gemma4_bk = PGM90_GEMMA4_GLU_BK;
extern "C" __device__ unsigned plow_block_pfgemm_glu_gemma4 = 384;
extern "C" __device__ unsigned plow_arena_bytes_pfgemm_glu_gemma4 =
...
__device__ __forceinline__ unsigned gemma4_glu_ctr_poll(const unsigned* address) {
    unsigned value;
    asm volatile("ld.acquire.gpu.u32 %0, [%1];" : "=r"(value) : "l"(address) : "memory");
    return value;
}

__device__ __forceinline__ void gemma4_glu_ctr_signal(unsigned* address) {
    asm volatile("red.release.gpu.global.add.u32 [%0], 1;" :: "l"(address) : "memory");
}

__device__ __forceinline__ PlowStreamEnt gemma4_glu_stream_ent(const PlowStreamEnt* address) {
    PlowStreamEnt entry;
...
template <bool PROD>
__device__ __forceinline__ void gemma4_glu_role_loop(
    ...
    for (;;) {
        ...
        const PlowStreamEnt entry = gemma4_glu_stream_ent(prog.gq_stream + index);
        ...
        for (unsigned w = threadIdx.x; w < entry.wait_len; w += blockDim.x) {
            ...
            while (gemma4_glu_ctr_poll(PLOW_CTR(prog.counters, wait.id)) < wait.threshold) {}
        }
        __syncthreads();
        ...
        const bool tensors_valid = in->t[0] != PLOW_TENSOR_NONE &&
            ...
            in->t[5] != PLOW_TENSOR_NONE && in->t[3] == PLOW_TENSOR_NONE &&
            in->t[4] == PLOW_TENSOR_NONE && in->t[6] == PLOW_TENSOR_NONE &&
            in->t[7] == PLOW_TENSOR_NONE;
        if (in->op != PLOW_DOP_GEMM_GLU || !exact_rows || in->i[1] != 15360 ||
        ...
        d_gemm_glu_sm90_tma_ws384_gemma4_role<PROD>(
            static_cast<__nv_bfloat16*>(prog.tensors[in->t[0]]), prog.tensors[in->i[6]],
            prog.tensors[in->i[7]], prog.tensors[in->i[3]], in->i[0], entry.slice,
            in->blocks, arena);

        __syncthreads();
        for (unsigned s = threadIdx.x; s < entry.succ_len; s += blockDim.x)
            gemma4_glu_ctr_signal(PLOW_CTR(prog.counters, prog.succs[entry.succ_ofs + s]));
    }
}

extern "C" __global__ __maxnreg__(160)
void plow_sm90a_pfgemm_glu_gemma4(PlowProgram prog) {
    ...
    if (threadIdx.x < 128) {
        sm90_reg_dec(32);
        gemma4_glu_role_loop<true>(prog, cursor, lo, hi, &claim, arena);
    } else {
        sm90_reg_inc(224);
        gemma4_glu_role_loop<false>(prog, cursor, lo, hi, &claim, arena);
    }
}
