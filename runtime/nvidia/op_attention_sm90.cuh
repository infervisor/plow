...
#define FA_SM90_WG_ELIGIBLE(HD, BQ, BKV) ((BQ) == 64 && (BKV) == 32 && (HD) % 128 == 0)
...
__device__ void d_flash_prefill_sm90(float* __restrict__ Opart, float* __restrict__ mlpart,
    ...
    static_assert(BKV == 32, "n32 score tile + swizzle-0 Ps of width BKV");
...
#pragma unroll 1
            for (int ks = 0; ks < KS0; ks++) {
                ...
                fa90_wgmma_m64n32k16(S, sm90_desc(Qs + sub * QT + ko),
...
#pragma unroll 1
            ...
            sm90_wg_commit();
            sm90_wg_wait<0>();
            sm90_cp_wait<0>(); /* tile t+1 has landed for this thread */
            sb ^= 1;
        }
