...
enum {
     ...
     * positions (lowest-index tie-break) into idx. t4=kv_len is the LIVE occupancy (TOKEN-granular,
     ...
     * unused slots are -1. i3=pool_size (GLM-5.3-Flash pooled indexer, DEFAULT 1, unset/zero on
     * every existing emission, treated as 1 at dispatch): each row's per-row causal token
     * boundary (q_pos0+t+1, unchanged, still token-granular) divides down to a pool count
     * before it bounds the scan.
     *   t0=idx(i32[n_tok][top_k]) t1=Score t2=kv_len; i0=n_tok i1=top_k i2=kv_stride i3=pool_size */
     ...
     * split n3=2n+n^2 logits into pre/post/comb groups, RMS-scale by residual's own sumsq,
     * sigmoid the pre/post gates, softmax+Sinkhorn-normalize the nxn comb matrix, weighted-sum
     * residual by the pre-gate into the (UNNORMED) single-stream layer input.
     * hc_post_mult_value is a COMPILE-TIME constant (2.0 for GLM-5.3-Flash — its config.json
     * omits the field, taking vLLM's Glm5NextConfig default; NOT the more obvious 1.0), because
     * the descriptor has only two f slots and both are needed for rms_eps/hc_eps. The emitter
     * MUST assert the config matches what this kernel was built for.
     *   t0=post_mix(out,f32[T][n]) t1=comb_mix(out,f32[T][n][n])
     ...
     * combine plus a broadcast multiply-add. No epsilon needed — every input is an already-
     * computed gate from the paired pre call.
     ...
     * oracle generated by running that kernel on real hardware. Per-dim softmax over
     * pool_size=4 tokens (score+ape, independent per one of head_dim=128 dims) weights a sum
     * of raw K; bf16 round-trip; fixed Hadamard-128 rotation (1/sqrt(128) normalized); bf16
     * round-trip; per-vector fp8 e4m3 quant. Scale is 2^ceil(log2(absmax/448)) — POWER-OF-2
     * ROUNDED, not plain absmax.
     *   t0=compressed_k(out,fp8_e4m3[n_pools][head_dim]) t1=compressed_scale(out,f32[n_pools])
     *   t2=slot_k(in,bf16[n_pools][pool_size][head_dim]) t3=slot_score(in,bf16[n_pools]
     *   [pool_size][head_dim]) t4=ape(in,f32[pool_size][head_dim])
     *   i0=n_pools i1=pool_size i2=head_dim
     * DECODE MODE (t5 present): n_pools MUST be 1. Gates the whole launch on the live
     ...
     * the trailing incomplete pool's raw token ids. One thread per output element.
     * t2 is a SCALAR chunk-end kv_len, not a materialized per-row array — mirrors
     * PLOW_DOP_INDEX_SELECT_PF's own derivation: row rows-1 has seq_len==kv_len[0], each
     * earlier row one fewer token (q_pos0 = kv_len[0]-rows, seq_len = q_pos0+row+1).
     * Decode (rows==1) collapses to seq_len==kv_len[0], byte-identical to the old
     * materialized-array contract.
     ...
     * a small per-layer tail ring of pool_size slots at pos % pool_size, so once every
     * pool_size steps the ring holds a complete pool in slot order for
     * PLOW_DOP_DSA_POOL_COMPRESS's decode-mode gate to read. No floats — a masked,
     * dynamically-indexed copy.
     ...
     * generated by running that real function on real hardware. One index head's
     * head_dim-wide query vector per "row" (caller flattens [token][index_head]):
     * Hadamard-128 rotate (same 1/sqrt(128) constant), ONE bf16 round-trip (no
     * softmax-pooling step on the query side, so no pre-Hadamard round-trip), same
     * power-of-2 rounded fp8 scale as the k side. Strict subset of
     * PLOW_DOP_DSA_POOL_COMPRESS's math, no pooling loop, no ape.
     ...
     * PLOW_DOP_DSA_POOL_COMPRESS) instead of plain bf16. INDEX_SCORE/INDEX_SCORE_PF are
     * UNCHANGED, this is an addition. Ground truth: fp8_mqa_logits_torch
     * (vllm.v1.attention.ops.rocm_aiter_mla_sparse), verified against a numerical oracle
     * generated by running that real function on real hardware:
     *   score[h,m,pool] = (q_fp8.to(bf16) . k_fp8.to(bf16)) * k_scale[pool]
     *   logit[m,pool] = sum_h relu(score) * weights_scaled[m,h]
     *   weights_scaled = weights * q_scale * (index_dim**-0.5 * index_heads**-0.5)
     * f0 carries that same constant (multiplies w[h]*q_scale[h] instead of a bare w[h]).
     * IndexSelect/IndexSelectPf need no changes.
     *
     * t6 IS TOKEN-GRANULAR, NOT POOL-GRANULAR -- a real bug in an earlier version of this
     * op took pool-granular occupancy directly. Fixed: the kernel takes the same n.kvlen
     * every other GLM decode op reads (token count, host-patched every step) and divides
     * by i4=pool_size internally, FLOOR: len = kv_len/pool_size. A partial trailing pool
     * has no compressed cache entry yet (PLOW_DOP_DSA_POOL_COMPRESS only writes a pool
     * once complete) and is handled entirely by PLOW_DOP_DSA_POOL_EXPAND's separate
     * always-select-tail path -- confirmed by precedent: d_dsa_pool_expand already
     * computes this exact "how many complete pools" quantity the same way (floor).
     *   t0=Score(out,f32[n_batch][pool_stride])
     *   t1=Qfp8(in,fp8_e4m3[n_batch][index_heads][index_head_dim])
     *   t2=Qscale(in,f32[n_batch][index_heads])
     ...
     * budget, `check decode` fails above 256 VGPR). One caller: GLM-5.3-Flash's DSA
     * indexer weights_proj head-gate projection (PLOW_DOP_INDEX_SCORE_KPOOL's t5=W
     * input -- NOT index_kpool_compress_gate, an earlier pass of this comment named the
     * wrong tensor; that one feeds PLOW_DOP_DSA_POOL_COMPRESS's pooling softmax and needs
     * no fp32 treatment), [index_heads]=32 wide, once per indexer token. WHY fp32: the reference
     * computes this weight in fp32 throughout (glm5next_ref/nvidia/attention.py:327-336)
     * with an explicit comment that bf16 "introduce[s] ~1e-2 logit error that flips
     * near-tie pool rankings on hard long-context tasks" -- an accuracy requirement of the
    ...
    PLOW_DOP_QUANT_FP8_BLOCK128 = 184,
    PLOW_DOP_GEMM_FP8_BLOCK128 = 185,
    PLOW_DOP_GEMM_FP8_BLOCK128_SPLIT4 = 186,
    PLOW_DOP_SUM4_BF16 = 187,
    PLOW_DOP_MOE_GLU_FP8_BLOCK128 = 188,
    PLOW_DOP_MOE_QUANT_FP8_BLOCK128 = 189,
    PLOW_DOP_MOE_DOWN_FP8_BLOCK128 = 190,
    ...
    PLOW_DOP_GEMM_FP8_MX = 198,
    PLOW_DOP_COMPRESS_ROPE_QUANT = 199,
