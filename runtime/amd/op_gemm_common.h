...
/* op_gemm_common.h — bf16 MFMA GEMM family, the SHARED bodies (CDNA3 + CDNA4).
 *
 * Do not include this file directly: op_gemm.h selects op_gemm_gfx942.h or op_gemm_gfx950.h
 * by PLOW_CDNA4, and that arch file sets every arch-defaulted knob and arch policy macro
 * (GM_BM, GM_DBUF, GM_SM_BK, GV_USCALAR, GV_RS_WIDE, GV_UNROLL_M4/M8, GM8_FIX8,
 * PLOW_GM_DIRECT_STAGE, PLOW_GM_FP8_PACK2, PLOW_GV_UN_FP8_KDIV) before including this one.
 * Everything below is keyed on THOSE, never on PLOW_CDNA4 itself.
 *
 *   C[M,N] = A[M,K] . B[N,K]^T
 *
 ...
 */
#ifndef PLOW_OP_GEMM_COMMON_H
#define PLOW_OP_GEMM_COMMON_H
#ifndef PLOW_OP_GEMM_ARCH
#error "op_gemm_common.h is the shared body: include op_gemm.h, which selects op_gemm_gfx942.h or op_gemm_gfx950.h and sets the arch defaults this file needs"
#endif
 ...
 * ------------------------------------------------------------------------- */
/* GM_BM -- the DEFAULT rung's BM -- is ARCH-DEFAULTED in op_gemm_gfx950.h (256) and
 * op_gemm_gfx942.h (192, the single-buffered stage that fits 64 KiB). Both are `#ifndef`
 * guards, so -DGM_BM still wins for plowc's per-shape bucketing and every A/B sweep. */
#ifndef GM_BN
#define GM_BN 256
#endif
...
/* GM_DBUF is ARCH-DEFAULTED: op_gemm_gfx950.h (2) / op_gemm_gfx942.h (1). See the note there. */
...
template <int BM, int BN, int BK, int WM, int WN, bool NORM, int SWZ = GM_SWZ, int WGM = GM_WGM,
          ...
          bool WFP8BLK = false, bool CF32 = false, bool WFP8MX = false
...
#define GM_SM_BN 128
#endif
/* GM_SM_BK is ARCH-DEFAULTED: op_gemm_gfx942.h (128, the re-cut measured above) /
 * op_gemm_gfx950.h (64, the rung it was tuned at). */
#ifndef GM_MD_BM
#define GM_MD_BM 128
#endif
#ifndef GM_MD_BN
...
__device__ void d_gemm_med_rope(bf16* C, const bf16* A, const bf16* B, unsigned M, unsigned N,
    ...
    d_gemm_t<GM_MD_BM, GM_MD_BN, GM_MD_BK, GM_WM, GM_WN, false, GM_SWZ, GM_WGM, (GM_PP != 0), true,
             false, false, false, false, false, true>(C, A, B, nullptr, nullptr, M, N, K, slice,
                                               nblk, lds, nullptr, 0, nullptr, nullptr, nullptr, rcos,
                                               rsin, rpos, roff);
}
#endif
...
/* d_gemm_c8, the 128x384 GEMM_WIDE rung, is gfx950-only (144 KiB of LDS) and is defined in
 * op_gemm_gfx950.h after this file. */
...
__device__ void d_gemm_fp8_mx(bf16* C, const bf16* A, const unsigned char* W,
    ...
    d_gemm_t<GM_MX_BM, GM_MX_BN, GM_MX_BK, GM_WM, GM_WN, false, GM_SWZ, GM_WGM, (GM_PP != 0), true,
             false, false, false, false, true>(C, A, (const bf16*)W, nullptr, nullptr, M, N, K, slice,
...
/* GV_USCALAR is ARCH-DEFAULTED: op_gemm_gfx942.h (1) / op_gemm_gfx950.h (0). */
#if GV_USCALAR
 ...
 * the arm at all. */
/* GV_RS_WIDE is ARCH-DEFAULTED: op_gemm_gfx942.h (1) / op_gemm_gfx950.h (0). */
#ifndef GV_RS_WIDE_R
...
#if !PLOW_GV_UN_FP8_KDIV /* op_gemm_gfx950.h: constant, the switches below fold */
    (void)nchunk;
...
/* GV_UNROLL_M4 / GV_UNROLL_M8 -- the unroll at the M=4 and M>=8 buckets -- are ARCH-DEFAULTED:
 * op_gemm_gfx942.h carries the measured table (6 / 3); op_gemm_gfx950.h pins both to GV_UNROLL
 * and refuses a -D for either. */
