...
static bool run_oracle(hipFunction_t bf16, hipFunction_t w8a16, hipFunction_t w8a16_row) {
    ...
    uint16_t *d_bf16, *d_x, *d_out_bf16, *d_out_fp8, *d_out_row_fp8;
    uint8_t *d_fp8, *d_row_fp8;
    ...
    CK(hipModuleLaunchKernel(w8a16_row, BLOCKS, 1, 1, THREADS, 1, 1, 0, nullptr, row_fp_args,
                             nullptr));
    CK(hipDeviceSynchronize());
    std::vector<uint16_t> out_bf16(N), out_fp8(N), out_row_fp8(N);
    ...
    double ref2 = 0.0, bf_err2 = 0.0, fp_err2 = 0.0, row_err2 = 0.0, cross_err2 = 0.0;
    double dot = 0.0, row_dot = 0.0, bf2 = 0.0, fp2 = 0.0, row2 = 0.0;
...
int main(int argc, char** argv) {
    ...
    CK(hipInit(0));
    hipModule_t module;
    hipFunction_t bf16, bf16_aa, w8a16, w8a16_row;
    ...
    if (!run_oracle(bf16, w8a16, w8a16_row)) return 3;
    ...
    std::printf("%-18s %7s %6s %5s | %9s %6s | %9s %7s | %9s %7s\n", "shape", "N", "K",
                "grid", "bf16_us", "A/A", "block_us", "speedup", "row_us", "speedup");
    ...
    std::printf("weighted body projection: bf16 %.3f ms, block %.3f (save %.3f), "
                "row %.3f (save %.3f), %s\n",
                bf_total, block_total, block_saving, row_total, row_saving,
                row_saving >= 5.0 && stable ? "PROMOTE ROW" : "STOP");
    return stable ? 0 : 4;
}
