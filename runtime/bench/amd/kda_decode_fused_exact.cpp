...
int main(int argc, char** argv) {
    ...
    std::vector<uint16_t> qr(bp),kr(bp),vr(bp),gate(bp),beta((size_t)B*H);
    std::vector<float> weight(3ull*P*W),conv0(cs),alog(H),dt(P),state0(ss),nw(D);
    for(auto* x:{&qr,&kr,&vr,&gate,&beta}) for(auto& v:*x) v=bf16(rnd(rng));
    ...
    auto dqr=upload(qr),dkr=upload(kr),dvr=upload(vr),dg=upload(gate),db=upload(beta);
    auto dw=upload(weight),dc0=upload(conv0),da=upload(alog),dd=upload(dt),ds0=upload(state0),dn=upload(nw);
    auto mix=alloc<uint16_t>(3*bp),oc=alloc<uint16_t>(bp),ox=alloc<uint16_t>(bp),yc=alloc<uint16_t>(bp),yx=alloc<uint16_t>(bp),yf=alloc<uint16_t>(bp),yz=alloc<uint16_t>(bp),yw=alloc<uint16_t>(bp);
    auto cc=alloc<float>(cs),cf=alloc<float>(cs),cz=alloc<float>(cs),cw=alloc<float>(cs),sc=alloc<float>(ss),sx=alloc<float>(ss),sf=alloc<float>(ss),sz=alloc<float>(ss),sw=alloc<float>(ss);
    auto reset=[&]{ CK(hipMemcpy(cc,dc0,cs*4,hipMemcpyDeviceToDevice)); CK(hipMemcpy(cf,dc0,cs*4,hipMemcpyDeviceToDevice));
        ...
        CK(hipMemcpy(sw,ds0,ss*4,hipMemcpyDeviceToDevice)); };
    ...
    auto step=[&]{ void* as[]={&oc,&mix,&dg,&db,&da,&dd,&sc,&B}; launch(fs,step_blocks,Shared,as); };
    auto step_bv8x16=[&]{ void* as[]={&ox,&mix,&dg,&db,&da,&dd,&sx,&B}; launch128(fx,B*H*D/BV,as); };
    ...
    auto fused=[&]{ void* af[]={&yf,&dqr,&dkr,&dvr,&dw,&dc0,&cf,&dg,&db,&da,&dd,&sf,&dn,&B};
        launch(ff,B*H,0,af); };
    auto fused_bv8x16=[&]{ void* af[]={&yz,&dqr,&dkr,&dvr,&dw,&dc0,&cz,&dg,&db,&da,&dd,&sz,&dn,&B};
        launch128(fz,B*H,af); };
    auto fused_256x16=[&]{ void* af[]={&yw,&dqr,&dkr,&dvr,&dw,&dc0,&cw,&dg,&db,&da,&dd,&sw,&dn,&B};
        launch256(fw,B*H,af); };
    reset(); control(); fused(); step_bv8x16(); norm_bv8x16(); fused_bv8x16(); fused_256x16(); CK(hipDeviceSynchronize());
    ...
    if(!ok){ std::fprintf(stderr,"FAIL conv %.3e state %.3e y %.3e bv8_state %.3e bv8_output %.3e bv8_y %.3e fused_bv8_conv %.3e fused_bv8_state %.3e fused_bv8_y %.3e fused_256_conv %.3e fused_256_state %.3e fused_256_y %.3e\n",ce.rel,se.rel,ye.rel,xse.rel,xoe.rel,xye.rel,zce.rel,zse.rel,zye.rel,wce.rel,wse.rel,wye.rel); return 2; }
    ...
    double c=measure(control),cx=measure(control_bv8x16),f=measure(fused),fzv=measure(fused_bv8x16),fwv=measure(fused_256x16),cv=measure(conv),st=measure(step),xs=measure(step_bv8x16),nm=measure(norm);
    std::printf("{\"schema\":\"plow.kda-decode-fused-exact.v1\",\"batch\":%u,\"heads\":12,\"dim\":128,\"conv_width\":4,\"step_blocks\":%u,\"control_us\":%.3f,\"control_bv8x16_us\":%.3f,\"fused_us\":%.3f,\"fused_bv8x16_us\":%.3f,\"fused_256x16_us\":%.3f,\"speedup\":%.4f,\"conv_us\":%.3f,\"step_us\":%.3f,\"step_bv8x16_us\":%.3f,\"norm_us\":%.3f,\"conv_rel_l2\":%.9g,\"state_rel_l2\":%.9g,\"output_rel_l2\":%.9g,\"output_max_abs\":%.9g,\"bv8x16_state_rel_l2\":%.9g,\"bv8x16_output_rel_l2\":%.9g,\"bv8x16_output_max_abs\":%.9g,\"bv8x16_final_rel_l2\":%.9g,\"bv8x16_final_max_abs\":%.9g,\"fused_bv8x16_conv_rel_l2\":%.9g,\"fused_bv8x16_state_rel_l2\":%.9g,\"fused_bv8x16_final_rel_l2\":%.9g,\"fused_bv8x16_final_max_abs\":%.9g,\"fused_256x16_conv_rel_l2\":%.9g,\"fused_256x16_state_rel_l2\":%.9g,\"fused_256x16_final_rel_l2\":%.9g,\"fused_256x16_final_max_abs\":%.9g}\n",
        B,step_blocks,c*1000.0,cx*1000.0,f*1000.0,fzv*1000.0,fwv*1000.0,c/f,cv*1000.0,st*1000.0,xs*1000.0,nm*1000.0,ce.rel,se.rel,ye.rel,ye.max,xse.rel,xoe.rel,xoe.max,xye.rel,xye.max,zce.rel,zse.rel,zye.rel,zye.max,wce.rel,wse.rel,wye.rel,wye.max);
    CK(hipModuleUnload(mod)); return 0;
}
