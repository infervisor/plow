...
Packet generation routes pure compatible segments by default;
`PLOW_MOE_COMBINE_LEAN=0` or `false` restores the interpreter route. The
structural contract is model-independent: nonzero runtime H/T, topk 16,
materialized f32 parts, and no banded, part16, or deterministic-accumulator
encoding. A missing object falls back to the interpreter; a present object must
pass the ABI/resource markers.
