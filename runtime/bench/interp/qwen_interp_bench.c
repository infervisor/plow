...
int main(int argc, char** argv) {
    const char* elf = argc > 1 ? argv[1] : "interp_prefill.elf";
    unsigned Mov = argc > 2 ? (unsigned)atoi(argv[2]) : 8192;
    plow_hsa* h = plow_hsa_init();
    if (!h) { printf("hsa init failed\n"); return 1; }
    char gfx[64]; unsigned cus = 0, lds = 0;
    plow_hsa_device_info(h, 0, gfx, &cus, &lds);
    printf("dev0: %s  CUs=%u  LDS=%u B  elf=%s  M=%u\n", gfx, cus, lds, elf, Mov);
    ...
    struct Shape qwen[] = {
        {"q_proj  ", Mov, 4096, 2560},
        {"kv_proj ", Mov, 2048, 2560},
        {"o_proj  ", Mov, 2560, 4096},
        {"gate/up ", Mov, 9728, 2560},
        {"down    ", Mov, 2560, 9728},
    };
    struct Shape gem[] = {
        {"q_proj  ", Mov, 8192, 5376},
        {"kv_proj ", Mov, 4096, 5376},
        {"o_proj  ", Mov, 5376, 8192},
        {"gate/up ", Mov, 21504, 5376},
        {"down    ", Mov, 5376, 21504},
    };
    struct Shape* shapes = gemma ? gem : qwen;
    const int NS = 5;
