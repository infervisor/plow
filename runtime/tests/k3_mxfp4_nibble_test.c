...
#define NEXT(cnt, elt) ({ void* _p = base + off; off += (size_t)(cnt) * (elt); _p; })
    ...
    plow_hsa_upload(h, 0, d_x, P_x, (size_t)K * 2);
    plow_hsa_upload(h, 0, d_w, P_w, (size_t)N * (K / 2));
    plow_hsa_upload(h, 0, d_s, P_s, (size_t)N * (K / 32));
    void* tens[4] = { d_c, d_x, d_w, d_s };
    void* d_tens = plow_hsa_alloc(h, 0, sizeof(tens));
    plow_hsa_upload(h, 0, d_tens, tens, sizeof(tens));
    ...
    plow_hsa_upload(h, 0, d_inst, &in, sizeof(in));
    ...
    plow_hsa_upload(h, 0, d_stream, stream, (size_t)cus * sizeof(PlowStreamEnt));
    plow_hsa_upload(h, 0, d_sofs, sofs, 4u * cus);
    plow_hsa_upload(h, 0, d_slen, slen, 4u * cus);
    plow_hsa_upload(h, 0, d_succ, &succ0, 4);
    uint32_t* zc = calloc(PLOW_CTR_STRIDE, 4);
    plow_hsa_upload(h, 0, d_ctr, zc, (size_t)PLOW_CTR_STRIDE * 4);
