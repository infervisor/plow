...
#define NR 8
#define NWG 256
#define XCTR_OFF (32u * 1024u * 1024u)
#define REGION_BYTES (40u * 1024u * 1024u)
...
int main(int argc, char** argv) {
    ...
    const uint32_t shape[] = {512u * 7168u, 1024u * 7168u};
    ...
    for (int r = 0; r < NR; r++) {
        if (plow_hsa_load_code_object(h, dev[r], elf, elf_len) ||
            ...
            plow_hsa_get_kernel(h, dev[r], "tp_allreduce_twoshot", &xr2[r])) {
            fprintf(stderr, "load: %s\n", plow_hsa_last_error()); return 2;
        }
        scratch[r] = plow_hsa_alloc_peer(h, dev[r], REGION_BYTES);
        out[r] = plow_hsa_alloc(h, dev[r], (size_t)shape[1] * 2u);
    ...
    bf16* host = (bf16*)malloc((size_t)shape[1] * 2u);
    const uint64_t deadline = freq ? freq : 1000000000ull;

    for (unsigned si = 0; si < sizeof shape / sizeof shape[0]; si++) {
        ...
        for (int r = NR - 1; r >= 0; r--)
            plow_hsa_launch(h, dev[r], &xr2[r], NWG * 512, 1, 1, 512, 1, 1, 0,
                            &a[r], sizeof a[r]);
        for (int r = 0; r < NR; r++) plow_hsa_wait(h, dev[r]);
        plow_hsa_download(h, dev[0], host, out[0], (size_t)n * 2u);
        size_t bad = 0;
        for (uint32_t e = 0; e < n; e++) {
            float sum = 0; for (uint32_t r = 0; r < NR; r++) sum += bf2f(partial_val(r, e));
            bad += host[e] != f2bf(sum);
        }
        const double us = (double)ticks * (1e9 / (double)freq) / (double)iters / 1e3;
        printf("rows=%u n=%u nblk=%d %.3f us/collective parity=%s timeout=%s bad=%zu\n",
               n / 7168u, n, NWG, us, bad ? "FAIL" : "PASS", timeout ? "YES" : "no", bad);
