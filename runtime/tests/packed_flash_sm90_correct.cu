...
__global__ void run_attention(const bf16* q, const bf16* k, const bf16* v,
    ...
    d_flash_prefill_mux<HD,64,BKV>(req, partial, stats, q, k, v, out,
        ...
        1.0f / sqrtf(float(HD)), blockIdx.x, blocks, arena);
}
...
template<int HD, int BKV> static bool check(unsigned kv_heads, unsigned stride,
    ...
    run_attention<HD,BKV><<<blocks,256,smem>>>(dq,dk,dv,out,partial,stats,dr,kv_heads,stride,mask,window);
    CK(cudaGetLastError());
    ...
    std::printf("HD=%d KV=%u window=%u checked=%u worst_relL2=%.6g max_abs=%.6g %s\n",
                HD,kv_heads,window,checked,worst,max_error,ok?"PASS":"FAIL");
...
int main() {
    const bool local=check<256,32>(8,2048,2047,1024);
    const bool global=check<512,16>(1,16384,0xffffffffu,0);
