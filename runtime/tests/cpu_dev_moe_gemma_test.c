...
int main(void) {
    ...
    for (uint32_t nblk = 1; nblk <= 16; nblk *= 4) {
        /* ---- decode chain over B rows ---- */
        float score[B * E]; plow_moe_route tab[B * K_]; plow_bf16 fu[B * K_ * I]; float part[B * K_ * H]; plow_bf16 out[B * H];
        PlowDevInst in = inst();
        void* T1[8] = {score, resid, w.proj, w.scale, NULL, NULL, NULL, NULL};
        in.t[0] = 0; in.t[1] = 1; in.t[2] = 2; in.t[3] = 3; in.i[0] = H; in.i[1] = E; in.i[2] = B; in.fj[0].f = root; in.fj[1].f = eps;
        run_op(PLOW_DOP_MOE_ROUTER_GEMMA_SCORE_FAST, &in, T1, nblk, &ctx);
        in = inst(); void* T2[8] = {tab, score, w.pes, NULL, NULL, NULL, NULL, NULL};
        in.t[0] = 0; in.t[1] = 1; in.t[2] = 2; in.i[1] = E; in.i[2] = K_; in.i[3] = B;
        run_op(PLOW_DOP_MOE_ROUTER_GEMMA_TOPK, &in, T2, nblk, &ctx);
        for (int b = 0; b < B; b++) for (int j = 0; j < K_; j++) {
        ...
        in.t[0] = 0; in.t[1] = 1; in.t[2] = 2; in.t[3] = 3; in.t[4] = 4; in.i[0] = K_; in.i[1] = I; in.i[2] = H; in.i[3] = E; in.i[5] = B; in.fj[0].f = eps;
        run_op(PLOW_DOP_MOE_EXPERT_GLU_NORM_GEMMA, &in, T3, nblk, &ctx);
        in = inst(); void* T4[8] = {part, fu, tab, w.ewt, NULL, NULL, NULL, NULL};
        in.t[0] = 0; in.t[1] = 1; in.t[2] = 2; in.t[3] = 3; in.i[0] = K_; in.i[1] = H; in.i[2] = I; in.i[3] = E; in.i[5] = B;
        run_op(PLOW_DOP_MOE_EXPERT_DOWN_GEMMA, &in, T4, nblk, &ctx);
        in = inst(); void* T5[8] = {out, part, h1, w.g_pf2, NULL, NULL, NULL, NULL};
        in.t[0] = 0; in.t[1] = 1; in.t[2] = 2; in.t[3] = 3; in.i[0] = H; in.i[1] = K_; in.i[2] = B; in.fj[0].f = eps;
        run_op(PLOW_DOP_MOE_COMBINE_NORM_GEMMA, &in, T5, nblk, &ctx);
        for (int b = 0; b < B; b++) { char what[64]; snprintf(what, sizeof what, "decode out row %d nblk %u", b, nblk); cmp_out(what, out + b * H, ref_out[b], H, 2e-2); }
        ...
        in = inst(); void* P1[8] = {ptab, resid, w.proj, w.scale, w.pes, NULL, NULL, NULL};
