 ...
 * the NEON tier vs golden (generated from cpu_dev_fp8_test.c), over all slices at Gemma shapes.
 *
...
static void test_gemv(uint32_t M, uint32_t N, uint32_t K, uint32_t a_row0, int have_v) {
    ...
    for (size_t bi = 0; bi < sizeof(NBLKS) / sizeof(*NBLKS); bi++) {
        ...
        if (have_v) {
            ...
            T[0] = Cg;
            snprintf(what, sizeof what, "GEMV_FP8 neon vs golden nblk=%u", nblk);
...
static void test_glu(uint32_t M, uint32_t N, uint32_t K, uint32_t act, int have_v) {
    ...
    for (size_t bi = 0; bi < sizeof(NBLKS) / sizeof(*NBLKS); bi++) {
        ...
        if (have_v) {
            ...
            T[0] = Cg;
            snprintf(what, sizeof what, "GEMV_GLU_FP8 neon vs golden nblk=%u", nblk);
...
static void test_gemm(uint16_t op, kfn golden, uint32_t M, uint32_t N, uint32_t K, int have_x) {
    ...
    for (size_t bi = 0; bi < sizeof(NBLKS) / sizeof(*NBLKS); bi++) {
        ...
        cmp(what, Cref, Cg, (size_t)M * N, 2e-2f);
        if (have_x && plow_cpu_tier_of(op) == PLOW_CPU_ISA_NEON) {
            T[0] = Cx; memset(Cx, 0, (size_t)M * N * 2);
            run_all(plow_cpu_kernel(op), &in, nblk, T, &ctx);
            T[0] = Cg;
            snprintf(what, sizeof what, "op%u neon vs golden nblk=%u", op, nblk);
            /* BFDOT sums each bf16 pair with one unfused rounding, so a K=3840 chain sits slightly
             * further from golden's sequential f32 FMA than AVX-512's vdpbf16ps does. */
...
static void test_gemm_glu(uint32_t M, uint32_t N, uint32_t K, uint32_t act, int have_x) {
    ...
    for (size_t bi = 0; bi < sizeof(NBLKS) / sizeof(*NBLKS); bi++) {
        ...
        cmp(what, Cref, Cg, (size_t)M * N, 3e-2f);
        if (have_x && plow_cpu_tier_of(PLOW_DOP_GEMM_GLU_FP8) == PLOW_CPU_ISA_NEON) {
            T[0] = Cx; memset(Cx, 0, (size_t)M * N * 2);
            run_all(plow_cpu_kernel(PLOW_DOP_GEMM_GLU_FP8), &in, nblk, T, &ctx);
            T[0] = Cg;
            snprintf(what, sizeof what, "GEMM_GLU_FP8 neon vs golden nblk=%u", nblk);
            cmp(what, Cg, Cx, (size_t)M * N, 2e-2f);
        }
    }
    g_floor = 1e-2f;
...
int main(int argc, char** argv) {
    ...
    const int have_v = tier >= PLOW_CPU_ISA_NEON;
