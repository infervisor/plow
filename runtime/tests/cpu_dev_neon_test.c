/* cpu_dev_neon_test.c — the NEON (aarch64 bf16) tier against the golden tier on random inputs.
 * Generated from cpu_dev_avx512_test.c; the two must keep the same shape list.
 *
 * Every registered op runs over ALL slices for nblk in {1, 3, 16} at Gemma-like shapes, both
 * through the live table (NEON after plow_cpu_init(NEON)) and through the golden entry
 * points called directly. bf16 outputs: 1e-2 relative (+1e-2 abs); integer outputs exact.
 * `--bench`: single-thread GEMV bandwidth, N=15360 K=3840 M=1, vs a plain NEON read, then
...
void n_gemv(const PlowDevInst* in, uint32_t slice, uint32_t nblk, void* const* T, PlowCpuCtx* ctx);
...
#include <arm_neon.h>
static uint64_t stream_read(const void* p, size_t bytes) {
    const char* c = p;
    uint64x2_t a = vdupq_n_u64(0), b = vdupq_n_u64(0), d = vdupq_n_u64(0), e = vdupq_n_u64(0);
    size_t i = 0;
    for (; i + 64 <= bytes; i += 64) {
        a = veorq_u64(a, vld1q_u64((const uint64_t*)(c + i)));
        b = veorq_u64(b, vld1q_u64((const uint64_t*)(c + i + 16)));
        d = veorq_u64(d, vld1q_u64((const uint64_t*)(c + i + 32)));
        e = veorq_u64(e, vld1q_u64((const uint64_t*)(c + i + 48)));
    }
    a = veorq_u64(veorq_u64(a, b), veorq_u64(d, e));
...
static void bench(void) {
    ...
    printf("read  same buffer, NEON loads only: %.1f GB/s (gemv = %.0f%% of it)\n",
...
static void* mt_run(void* p) {
    ...
    while (atomic_load(&b->arrive) < b->nthr) __asm__ volatile("yield");
    b->t0[t->slice] = now_s();
...
plow_cpu_kernel_fn volatile g_force_avx512_member = n_gemv;

int main(int argc, char** argv) {
    const int tier = plow_cpu_init(PLOW_CPU_ISA_NEON);
    if (tier < PLOW_CPU_ISA_NEON) {
        printf("SKIP: host tier %d has no NEON BF16\n", tier);
        return 0;
    }
    ...
    if (fails == 0) printf("cpu_dev_neon_test: all passed (tier %d)\n", tier);
    else printf("cpu_dev_neon_test: %d failures\n", fails);
    return fails ? 1 : 0;
}
