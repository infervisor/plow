...
static void replay_indexer_score(plow_hsa* h, unsigned m, unsigned ctx, const char* dir, const char* output,
                                 const char* key_object) {
    if (m != 16 || (ctx != 8192 && ctx != 71680)) exit(1);
    char arch[64]; uint32_t cus, lds;
    check_hsa(plow_hsa_device_info(h, 0, arch, &cus, &lds));
    if (strcmp(arch, "gfx950") || cus != 256) exit(1);
    const unsigned blocks = m * (ctx / 16);
    const size_t sizes[] = {(size_t)m * 32 * 128, (size_t)blocks * 2112,
        (size_t)m * 32 * 4, (size_t)m * (ctx / 16) * 4, (size_t)m * ctx * 4};
    const char* tags[] = {"query", "cache", "weights", "block-table", "logits"};
    void* host[5]; void* gpu[5];
    for (unsigned i = 0; i < 5; i++) {
        host[i] = plow_hsa_alloc_host(h, sizes[i]); gpu[i] = plow_hsa_alloc(h, 0, sizes[i]);
        char path[4096];
        int n = snprintf(path, sizeof(path), "%s/ctx%u.%s.bin", dir, ctx, tags[i]);
        if (!host[i] || !gpu[i] || n < 0 || (size_t)n >= sizeof(path)) exit(1);
        FILE* f = fopen(path, "rb");
        if (!f || fread(host[i], 1, sizes[i], f) != sizes[i] || fgetc(f) != EOF || ferror(f) || fclose(f)) exit(1);
        if (i != 4 && !(key_object && i == 1)) check_hsa(plow_hsa_copy_h2d(h, 0, gpu[i], host[i], sizes[i]));
    }
    for (size_t i = 0; i < sizes[3] / 4; i++) if (((uint32_t*)host[3])[i] >= blocks) exit(1);
    for (size_t i = 0; i < sizes[4] / 4; i++) if (!isfinite(((float*)host[4])[i])) exit(1);
    uint32_t* lengths = plow_hsa_alloc_host(h, m * 4);
    void* dlengths = plow_hsa_alloc(h, 0, m * 4);
    float* actual = plow_hsa_alloc_host(h, sizes[4]);
    if (!lengths || !dlengths || !actual) exit(1);
    for (unsigned i = 0; i < m; i++) lengths[i] = ctx;
    check_hsa(plow_hsa_copy_h2d(h, 0, dlengths, lengths, m * 4));
    plow_hsa_kernel kernel;
    check_hsa(plow_hsa_get_kernel(h, 0, "_gluon_deepgemm_fp8_paged_mqa_logits_preshuffle", &kernel));
    if (kernel.kernarg_size != 152 || kernel.private_segment_size || kernel.group_segment_size) exit(1);
    struct {
        uint32_t batch, next, heads; void* q;
        uint32_t qs0, qs1, qs2; void* kv; uint32_t kvs; void* scale; uint32_t ss;
        void *lengths, *table, *weights; uint32_t ws; void* out; uint64_t os;
        uint32_t maxctx, maxblocks, blocks, splits; void *scratch, *profile;
    } args = {m, 1, 32, gpu[0], 4096, 4096, 128, gpu[1], 2112,
        (uint8_t*)gpu[1] + 2048, 528, dlengths, gpu[3], gpu[2], 32, gpu[4], ctx,
        ctx, ctx / 16, blocks, ((cus / m + 4) / 5) * 5 * 2, NULL, NULL};
    _Static_assert(sizeof(args) == 152, "indexer score kernargs");
    if ((char*)&args.q - (char*)&args != 16 || (char*)&args.scale - (char*)&args != 56
            || (char*)&args.profile - (char*)&args != 144) exit(1);
    plow_hsa_kernel key_kernel;
    const unsigned key_rows = m * ctx;
    const size_t history_bytes = (size_t)key_rows * 256, slot_bytes = (size_t)key_rows * 8;
    bf16* history = NULL;
    int64_t* slots = NULL;
    uint8_t* actual_cache = NULL;
    struct { void *cache, *x, *slots; unsigned rows; } key_args = {gpu[1], NULL, NULL, key_rows};
    if (key_object) {
        load_pipeline_object(h, key_object);
        check_hsa(plow_hsa_get_kernel(h, 0, "glm_indexer_key_cache", &key_kernel));
        if (key_kernel.kernarg_size != 288 || key_kernel.private_segment_size || key_kernel.group_segment_size) exit(1);
        history = plow_hsa_alloc_host(h, history_bytes);
        slots = plow_hsa_alloc_host(h, slot_bytes);
        actual_cache = plow_hsa_alloc_host(h, sizes[1]);
        key_args.x = plow_hsa_alloc(h, 0, history_bytes);
        key_args.slots = plow_hsa_alloc(h, 0, slot_bytes);
        if (!history || !slots || !actual_cache || !key_args.x || !key_args.slots) exit(1);
        char path[4096];
        int n = snprintf(path, sizeof(path), "%s/ctx%u.history.bin", dir, ctx);
        if (n < 0 || (size_t)n >= sizeof(path)) exit(1);
        FILE* input = fopen(path, "rb");
        if (!input || fread(history, 1, history_bytes, input) != history_bytes
                || fgetc(input) != EOF || ferror(input) || fclose(input)) exit(1);
        for (size_t i = 0; i < history_bytes / 2; i++) if (!isfinite(bf2f(history[i]))) exit(1);
        uint8_t* seen = calloc(blocks, 1);
        if (!seen) exit(1);
        for (unsigned i = 0; i < blocks; i++) {
            unsigned physical = ((uint32_t*)host[3])[i];
            if (seen[physical]++) exit(1);
            for (unsigned lane = 0; lane < 16; lane++) slots[(size_t)i * 16 + lane] = (int64_t)physical * 16 + lane;
        }
        free(seen);
        check_hsa(plow_hsa_copy_h2d(h, 0, key_args.x, history, history_bytes));
        check_hsa(plow_hsa_copy_h2d(h, 0, key_args.slots, slots, slot_bytes));
    }
    for (unsigned run = 0; run < 2; run++) {
        memset(actual, 0xff, sizes[4]);
        check_hsa(plow_hsa_copy_h2d(h, 0, gpu[4], actual, sizes[4]));
        if (key_object) {
            memset(actual_cache, run ? 0xaa : 0x55, sizes[1]);
            check_hsa(plow_hsa_copy_h2d(h, 0, gpu[1], actual_cache, sizes[1]));
            check_hsa(plow_hsa_launch(h, 0, &key_kernel, ((key_rows + 3) / 4) * 256, 1, 1,
                256, 1, 1, 0, &key_args, sizeof(key_args)));
        }
        check_hsa(plow_hsa_launch(h, 0, &kernel, m * args.splits * 256, 1, 1, 256, 1, 1,
            4096, &args, sizeof(args)));
        check_hsa(plow_hsa_wait(h, 0));
        if (key_object) {
            check_hsa(plow_hsa_copy_d2h(h, 0, actual_cache, gpu[1], sizes[1]));
            size_t mismatch = 0;
            for (size_t i = 0; i < sizes[1]; i++) mismatch += actual_cache[i] != ((uint8_t*)host[1])[i];
            printf("indexer native cache->score M=%u ctx=%u run=%u cache_mismatch_bytes=%zu/%zu\n",
                m, ctx, run, mismatch, sizes[1]);
            fails += mismatch != 0;
        }
        check_hsa(plow_hsa_copy_d2h(h, 0, actual, gpu[4], sizes[4]));
        size_t mismatch = 0;
        for (size_t i = 0; i < sizes[4] / 4; i++) {
            float want = ((float*)host[4])[i];
            if (!isfinite(actual[i]) || memcmp(actual + i, &want, 4)) {
                if (mismatch < 4) printf("indexer score mismatch row=%zu col=%zu actual=%a expected=%a\n",
                    i / ctx, i % ctx, actual[i], want);
                mismatch++;
            }
        }
        printf("indexer AOT score M=%u ctx=%u run=%u mismatches=%zu/%zu\n", m, ctx, run, mismatch, sizes[4] / 4);
        fails += mismatch != 0;
    }
    FILE* f = fopen(output, "wbx");
    if (!f || fwrite(actual, 1, sizes[4], f) != sizes[4] || fclose(f)) exit(1);
    if (key_object) {
        char path[4096];
        int n = snprintf(path, sizeof(path), "%s.cache", output);
        if (n < 0 || (size_t)n >= sizeof(path)) exit(1);
        f = fopen(path, "wbx");
        if (!f || fwrite(actual_cache, 1, sizes[1], f) != sizes[1] || fclose(f)) exit(1);
        plow_hsa_free(h, history); plow_hsa_free(h, slots); plow_hsa_free(h, actual_cache);
        plow_hsa_free(h, key_args.x); plow_hsa_free(h, key_args.slots);
    }
    for (unsigned i = 0; i < 5; i++) { plow_hsa_free(h, host[i]); plow_hsa_free(h, gpu[i]); }
    plow_hsa_free(h, lengths); plow_hsa_free(h, dlengths); plow_hsa_free(h, actual);
}

static void replay_indexer_decode(plow_hsa* h, const char* path, const char* output) {
    FILE* f = fopen(path, "rb");
    uint32_t hdr[4];
    if (!f || fread(hdr, sizeof(hdr), 1, f) != 1 || hdr[0] != 0x49445031 || hdr[3]
            || !(hdr[1] == 1 || hdr[1] == 8 || hdr[1] == 16 || hdr[1] == 32 || hdr[1] == 64)
            || hdr[2] < 16 || hdr[2] > 131072 || hdr[2] % 16) exit(1);
    unsigned m = hdr[1], ctx = hdr[2];
    size_t sizes[] = {(size_t)m * 8192, (size_t)m * 64, (size_t)m * 256, (size_t)m * 4, (size_t)m * 4};
    size_t qb = (size_t)m * 4096, sb = (size_t)m * 128;
    size_t ob = qb + 2 * sb + (size_t)m * ctx / 16 * 2112;
    void* host[5]; void* gpu[5];
    for (unsigned i = 0; i < 5; i++) {
        host[i] = plow_hsa_alloc_host(h, sizes[i]); gpu[i] = plow_hsa_alloc(h, 0, sizes[i]);
        if (!host[i] || !gpu[i] || fread(host[i], 1, sizes[i], f) != sizes[i]) exit(1);
    }
    for (unsigned row = 0; row < m; row++) {
        uint32_t length = ((uint32_t*)host[4])[row];
        int32_t pos = ((int32_t*)host[3])[row];
        if (length > ctx || (length && (pos < 0 || (uint32_t)pos + 1 != length))) exit(1);
        if (length) for (unsigned part = 0; part < 3; part++) {
            size_t width = sizes[part] / m / 2;
            for (size_t j = 0; j < width; j++) if (!isfinite(bf2f(((bf16*)host[part])[row * width + j]))) exit(1);
        }
    }
    uint8_t* expected = malloc(ob);
    uint8_t* actual = plow_hsa_alloc_host(h, ob);
    uint8_t* dout = plow_hsa_alloc(h, 0, ob);
    if (!expected || !actual || !dout || fread(expected, 1, ob, f) != ob
            || fgetc(f) != EOF || ferror(f) || fclose(f)) exit(1);
    for (unsigned i = 0; i < 5; i++) check_hsa(plow_hsa_copy_h2d(h, 0, gpu[i], host[i], sizes[i]));
    plow_hsa_kernel kernel;
    check_hsa(plow_hsa_get_kernel(h, 0, "glm_indexer_decode_prepare", &kernel));
    if (kernel.kernarg_size != 352 || kernel.private_segment_size || kernel.group_segment_size) exit(1);
    uint32_t* mask = plow_hsa_alloc_host(h, m * 4);
    void* dmask = plow_hsa_alloc(h, 0, m * 4);
    void* deffective = plow_hsa_alloc(h, 0, m * 4);
    if (!mask || !dmask || !deffective) exit(1);
    memset(mask, 0, m * 4);
    check_hsa(plow_hsa_copy_h2d(h, 0, dmask, mask, m * 4));
    struct { void *q, *scales, *weights, *cache, *raw_q, *raw_w, *key, *positions, *lengths, *parked, *effective; unsigned rows, ctx; }
        args = {dout, dout + qb, dout + qb + sb, dout + qb + 2 * sb,
            gpu[0], gpu[1], gpu[2], gpu[3], gpu[4], dmask, deffective, m, ctx};
    _Static_assert(sizeof(args) == 96, "decode preparation kernargs");
    for (unsigned run = 0; run < 2; run++) {
        memset(actual, 0xa5, ob);
        check_hsa(plow_hsa_copy_h2d(h, 0, dout, actual, ob));
        check_hsa(plow_hsa_launch(h, 0, &kernel, ((m * 33 + 3) / 4) * 256, 1, 1, 256, 1, 1,
            0, &args, sizeof(args)));
        check_hsa(plow_hsa_wait(h, 0));
        check_hsa(plow_hsa_copy_d2h(h, 0, actual, dout, ob));
        size_t mismatch = 0;
        for (size_t i = 0; i < ob; i++) if (actual[i] != expected[i]) {
            if (mismatch < 4) printf("decode preparation mismatch byte=%zu actual=%02x expected=%02x\n", i, actual[i], expected[i]);
            mismatch++;
        }
        printf("indexer decode prepare M=%u ctx=%u run=%u mismatch_bytes=%zu/%zu\n", m, ctx, run, mismatch, ob);
        fails += mismatch != 0;
    }
    f = fopen(output, "wbx");
    if (!f || fwrite(actual, 1, ob, f) != ob || fclose(f)) exit(1);
    for (unsigned i = 0; i < 5; i++) { plow_hsa_free(h, host[i]); plow_hsa_free(h, gpu[i]); }
    free(expected); plow_hsa_free(h, actual); plow_hsa_free(h, dout);
    plow_hsa_free(h, mask); plow_hsa_free(h, dmask); plow_hsa_free(h, deffective);
}

static void replay_indexer_prefill(plow_hsa* h, const char* path, const char* output,
                                    const char* score_object, const char* append_object) {
    FILE* f = fopen(path, "rb");
    uint32_t hdr[8];
    if (!f || fread(hdr, sizeof(hdr), 1, f) != 1 || hdr[0] != 0x49504631) exit(1);
    const unsigned m = hdr[1], ctx = hdr[2], base = hdr[3], live = hdr[4], slots = hdr[5], slot = hdr[6];
    if (!(m == 128 || m == 512 || m == 1024 || m == 2048 || m == 4096 || m == 8192)
            || ctx != 131072 || live > m || base > ctx || live > ctx - base || (!base && !live)
            || !slots || slots > 64 || slot >= slots) exit(1);
    const unsigned n = base + live, fixture_stride = (n + 255) / 256 * 256;
    const unsigned stride = append_object ? ctx : fixture_stride;
    if (hdr[7] != fixture_stride) exit(1);
    char arch[64]; uint32_t cus, lds;
    check_hsa(plow_hsa_device_info(h, 0, arch, &cus, &lds));
    if (strcmp(arch, "gfx950") || cus != 256) exit(1);
    size_t ins[] = {(size_t)m * 8192, (size_t)m * 64, (size_t)m * 256, (size_t)slots * ctx * 132};
    size_t outs[] = {(size_t)m * 4096, (size_t)m * 128, (size_t)m * 128, ins[3],
        (size_t)n * 128, (size_t)n * 4, (size_t)m * 4, (size_t)m * 4, (size_t)m * stride * 4};
    size_t total = sizeof(hdr);
    for (unsigned i = 0; i < 4; i++) total += ins[i];
    for (unsigned i = 0; i < 8; i++) total += outs[i];
    total += (size_t)m * fixture_stride * 4;
    if (fseek(f, 0, SEEK_END) || ftell(f) != (long)total || fseek(f, sizeof(hdr), SEEK_SET)) exit(1);
    uint8_t *in[4], *din[3], *actual[9], *expected[9], *dout[9];
    for (unsigned i = 0; i < 4; i++) {
        in[i] = plow_hsa_alloc_host(h, ins[i] + 512);
        if (!in[i]) exit(1);
        memset(in[i], 0x7b, ins[i] + 512);
        if (fread(in[i] + 256, 1, ins[i], f) != ins[i]) exit(1);
        if (i < 3) {
            for (size_t j = 0; j < ins[i] / m / 2 * live; j++)
                if (!isfinite(bf2f(((bf16*)(in[i] + 256))[j]))) exit(1);
            din[i] = plow_hsa_alloc(h, 0, ins[i] + 512);
            if (!din[i]) exit(1);
            check_hsa(plow_hsa_copy_h2d(h, 0, din[i], in[i], ins[i] + 512));
        }
    }
    for (unsigned i = 0; i < 9; i++) {
        expected[i] = malloc(outs[i]);
        actual[i] = plow_hsa_alloc_host(h, outs[i] + 512);
        dout[i] = plow_hsa_alloc(h, 0, outs[i] + 512);
        if (!expected[i] || !actual[i] || !dout[i]) exit(1);
        if (i == 8) {
            memset(expected[i], 0, outs[i]);
            for (unsigned row = 0; row < m; row++)
                if (fread(expected[i] + (size_t)row * stride * 4, 4, fixture_stride, f) != fixture_stride) exit(1);
        } else if (fread(expected[i], 1, outs[i], f) != outs[i]) exit(1);
    }
    if (fgetc(f) != EOF || ferror(f) || fclose(f)) exit(1);
    plow_hsa_kernel prepare, score;
    check_hsa(plow_hsa_get_kernel(h, 0, "glm_indexer_prefill_prepare", &prepare));
    load_pipeline_object(h, score_object);
    check_hsa(plow_hsa_get_kernel(h, 0, "_gluon_fp8_mqa_logits_kernel", &score));
    if (prepare.kernarg_size != 360 || prepare.private_segment_size || prepare.group_segment_size
            || score.kernarg_size != 360 || score.private_segment_size || score.group_segment_size) exit(1);
    struct {
        void *q, *scales, *weights, *cache, *keys, *key_scales, *starts, *ends, *raw_q, *raw_w, *raw_k;
        unsigned rows, base, live;
    } prep_args = {dout[0] + 256, dout[1] + 256, dout[2] + 256,
        dout[3] + 256 + (size_t)slot * ctx * 132, dout[4] + 256, dout[5] + 256,
        dout[6] + 256, dout[7] + 256, din[0] + 256, din[1] + 256, din[2] + 256, m, base, live};
    struct {
        void *q, *keys, *key_scales, *weights, *starts, *ends, *out;
        unsigned rows, keys_count, qs, ks, ws, os, od;
        void *scratch, *profile;
    } score_args = {dout[0] + 256, dout[4] + 256, dout[5] + 256, dout[2] + 256,
        dout[6] + 256, dout[7] + 256, dout[8] + 256, m, n, 4096, 128, 32, stride, 1, NULL, NULL};
    _Static_assert(sizeof(prep_args) == 104, "prefill preparation kernargs");
    _Static_assert(sizeof(score_args) == 104, "prefill score kernargs");
    for (unsigned run = 0; run < 2; run++) {
        uint8_t poison = run ? 0xaa : 0x55;
        for (unsigned i = 0; i < 9; i++) {
            memset(actual[i], poison, outs[i] + 512);
            if (i == 3) memcpy(actual[i] + 256, in[3] + 256, ins[3]);
            check_hsa(plow_hsa_copy_h2d(h, 0, dout[i], actual[i], outs[i] + 512));
        }
        check_hsa(plow_hsa_launch(h, 0, &prepare, ((m * 33 + base + 3) / 4) * 256, 1, 1,
            256, 1, 1, 0, &prep_args, sizeof(prep_args)));
        unsigned chunk_rows = (size_t)live * n * 4 < ((size_t)1 << 31)
            ? ((size_t)1 << 31) / ((size_t)stride * 4) : m;
        for (unsigned row = 0; row < live; row += chunk_rows) {
            unsigned count = live - row < chunk_rows ? live - row : chunk_rows;
            score_args.q = dout[0] + 256 + (size_t)row * 4096;
            score_args.weights = dout[2] + 256 + (size_t)row * 128;
            score_args.starts = dout[6] + 256 + (size_t)row * 4;
            score_args.ends = dout[7] + 256 + (size_t)row * 4;
            score_args.out = dout[8] + 256 + (size_t)row * stride * 4;
            score_args.rows = count;
            check_hsa(plow_hsa_launch(h, 0, &score, count * 64, 1, 1,
                64, 1, 1, 12416, &score_args, sizeof(score_args)));
        }
        check_hsa(plow_hsa_wait(h, 0));
        size_t bad_guards = 0, mismatches = 0;
        for (unsigned i = 0; i < 9; i++) {
            check_hsa(plow_hsa_copy_d2h(h, 0, actual[i], dout[i], outs[i] + 512));
            for (size_t j = 0; j < 256; j++) {
                bad_guards += actual[i][j] != poison;
                bad_guards += actual[i][outs[i] + 256 + j] != poison;
            }
            if (i == 8) {
                for (unsigned row = 0; row < m; row++) {
                    unsigned end = row < live ? base + row + 1 : 0;
                    for (unsigned col = 0; col < stride; col++) {
                        size_t offset = ((size_t)row * stride + col) * 4;
                        if (col >= end) {
                            for (unsigned b = 0; b < 4; b++)
                                bad_guards += actual[i][256 + offset + b] != poison;
                            memset(actual[i] + 256 + offset, 0, 4);
                        }
                    }
                }
            }
            size_t changed = 0;
            if (memcmp(actual[i] + 256, expected[i], outs[i]))
                for (size_t j = 0; j < outs[i]; j++) if (actual[i][256 + j] != expected[i][j]) {
                    if (changed < 3) printf("prefill mismatch part=%u byte=%zu got=%02x want=%02x\n",
                        i, j, actual[i][256 + j], expected[i][j]);
                    changed++;
                }
            mismatches += changed;
        }
        for (unsigned i = 0; i < 3; i++) {
            uint8_t* check = plow_hsa_alloc_host(h, ins[i] + 512);
            if (!check) exit(1);
            check_hsa(plow_hsa_copy_d2h(h, 0, check, din[i], ins[i] + 512));
            bad_guards += memcmp(check, in[i], ins[i] + 512) != 0;
            plow_hsa_free(h, check);
        }
        printf("indexer prefill T=%u base=%u live=%u slot=%u/%u run=%u mismatched_bytes=%zu guard_failures=%zu\n",
            m, base, live, slot, slots, run, mismatches, bad_guards);
        fails += mismatches != 0 || bad_guards != 0;
    }
    f = fopen(output, "wbx");
    if (!f) exit(1);
    for (unsigned i = 0; i < 9; i++)
        if (fwrite(actual[i] + 256, 1, outs[i], f) != outs[i]) exit(1);
    if (fclose(f)) exit(1);
    if (append_object) {
        load_pipeline_object(h, append_object);
        plow_hsa_kernel append;
        check_hsa(plow_hsa_get_kernel(h, 0, "glm_indexer_prefill_append", &append));
        if (append.kernarg_size != 288 || append.private_segment_size || append.group_segment_size) exit(1);
        struct { void *cache, *key; unsigned rows, base, live; } args = {
            dout[3] + 256 + (size_t)slot * ctx * 132, din[2] + 256, m, base, live};
        _Static_assert(sizeof(args) == 32, "prefill append kernargs");
        for (unsigned run = 0; run < 2; run++) {
            memset(actual[3], 0x73, outs[3] + 512);
            memcpy(actual[3] + 256, in[3] + 256, ins[3]);
            check_hsa(plow_hsa_copy_h2d(h, 0, dout[3], actual[3], outs[3] + 512));
            check_hsa(plow_hsa_launch(h, 0, &append, ((m + 3) / 4) * 256, 1, 1,
                256, 1, 1, 0, &args, sizeof(args)));
            check_hsa(plow_hsa_wait(h, 0));
            check_hsa(plow_hsa_copy_d2h(h, 0, actual[3], dout[3], outs[3] + 512));
            size_t bad = memcmp(actual[3] + 256, expected[3], outs[3]) != 0;
            for (size_t j = 0; j < 256; j++) {
                bad += actual[3][j] != 0x73;
                bad += actual[3][outs[3] + 256 + j] != 0x73;
            }
            printf("indexer prefill append T=%u base=%u live=%u slot=%u/%u run=%u failures=%zu\n",
                m, base, live, slot, slots, run, bad);
            fails += bad != 0;
        }
    }
    for (unsigned i = 0; i < 4; i++) plow_hsa_free(h, in[i]);
    for (unsigned i = 0; i < 3; i++) plow_hsa_free(h, din[i]);
    for (unsigned i = 0; i < 9; i++) {
        free(expected[i]); plow_hsa_free(h, actual[i]); plow_hsa_free(h, dout[i]);
    }
}

static void replay_indexer_quant(plow_hsa* h, const char* path, const char* output) {
    FILE* f = fopen(path, "rb");
    uint32_t hdr[4];
    if (!f || fread(hdr, sizeof(hdr), 1, f) != 1 || hdr[0] != 0x49515131
            || hdr[1] > 1 || !hdr[2] || hdr[2] > 2097152 || hdr[3] > 131072
            || (hdr[1] == 0 ? hdr[3] != 0 : hdr[3] == 0)) exit(1);
    unsigned mode = hdr[1], rows = hdr[2], blocks = hdr[3];
    size_t xb = (size_t)rows * 256, ab = (size_t)rows * (mode ? 8 : 2);
    size_t ob = mode ? (size_t)blocks * 2112 : (size_t)rows * 136;
    void* x = plow_hsa_alloc_host(h, xb);
    void* aux = plow_hsa_alloc_host(h, ab);
    uint8_t* expected = malloc(ob);
    uint8_t* actual = plow_hsa_alloc_host(h, ob);
    if (!x || !aux || !expected || !actual || fread(x, 1, xb, f) != xb
            || fread(aux, 1, ab, f) != ab || fread(expected, 1, ob, f) != ob
            || fgetc(f) != EOF || ferror(f) || fclose(f)) exit(1);
    for (size_t i = 0; i < xb / 2; i++) if (!isfinite(bf2f(((bf16*)x)[i]))) exit(1);
    uint8_t* seen = mode ? calloc((size_t)blocks * 16, 1) : NULL;
    if (mode && !seen) exit(1);
    for (unsigned i = 0; i < rows; i++) {
        if (mode) {
            int64_t slot = ((int64_t*)aux)[i];
            if (slot >= (int64_t)blocks * 16) exit(1);
            if (slot >= 0 && seen[slot]++) exit(1);
        } else if (!isfinite(bf2f(((bf16*)aux)[i]))) exit(1);
    }
    free(seen);
    void* dx = plow_hsa_alloc(h, 0, xb);
    void* da = plow_hsa_alloc(h, 0, ab);
    uint8_t* dout = plow_hsa_alloc(h, 0, ob);
    if (!dx || !da || !dout) exit(1);
    check_hsa(plow_hsa_copy_h2d(h, 0, dx, x, xb));
    check_hsa(plow_hsa_copy_h2d(h, 0, da, aux, ab));
    plow_hsa_kernel kernel;
    check_hsa(plow_hsa_get_kernel(h, 0, mode ? "glm_indexer_key_cache" : "glm_indexer_query_quant", &kernel));
    if (kernel.private_segment_size || kernel.group_segment_size
            || kernel.kernarg_size != (mode ? 288 : 304)) exit(1);
    struct { void *q, *s, *w, *x, *weights; unsigned rows; } query_args = {
        dout, dout + (size_t)rows * 128, dout + (size_t)rows * 132, dx, da, rows};
    struct { void *cache, *x, *slots; unsigned rows; } key_args = {dout, dx, da, rows};
    for (unsigned run = 0; run < 2; run++) {
        memset(actual, 0xa5, ob);
        check_hsa(plow_hsa_copy_h2d(h, 0, dout, actual, ob));
        check_hsa(plow_hsa_launch(h, 0, &kernel, ((rows + 3) / 4) * 256, 1, 1, 256, 1, 1, 0,
            mode ? (void*)&key_args : (void*)&query_args,
            mode ? sizeof(key_args) : sizeof(query_args)));
        check_hsa(plow_hsa_wait(h, 0));
        check_hsa(plow_hsa_copy_d2h(h, 0, actual, dout, ob));
        size_t mismatches = 0;
        for (size_t i = 0; i < ob; i++) if (actual[i] != expected[i]) {
            if (mismatches < 8) printf("indexer mode=%u byte=%zu actual=%02x expected=%02x\n",
                mode, i, actual[i], expected[i]);
            mismatches++;
        }
        printf("indexer quant mode=%u rows=%u blocks=%u run=%u mismatch_bytes=%zu/%zu\n",
            mode, rows, blocks, run, mismatches, ob);
        fails += mismatches != 0;
    }
    f = fopen(output, "wbx");
    if (!f || fwrite(actual, 1, ob, f) != ob || fclose(f)) exit(1);
    plow_hsa_free(h, dx); plow_hsa_free(h, da); plow_hsa_free(h, dout);
    plow_hsa_free(h, x); plow_hsa_free(h, aux); plow_hsa_free(h, actual); free(expected);
}

static void test_rope_cache(plow_hsa* h, const char* path) {
    FILE* f = fopen(path, "rb");
    if (!f || fseek(f, 0, SEEK_END)) exit(1);
    const long bytes = ftell(f);
    if (bytes <= 0 || bytes % 128 || bytes > 1048576L * 128 || fseek(f, 0, SEEK_SET)) exit(1);
    const unsigned ctx = bytes / 128;
    const size_t count = (size_t)ctx * 32, output_bytes = count * sizeof(float);
    bf16* reference = malloc(bytes);
    if (!reference || fread(reference, 1, bytes, f) != (size_t)bytes || fclose(f)) exit(1);
    float* actual = plow_hsa_alloc_host(h, output_bytes);
    void* dc = plow_hsa_alloc(h, 0, output_bytes);
    void* ds = plow_hsa_alloc(h, 0, output_bytes);
    if (!actual || !dc || !ds) exit(1);
    plow_hsa_kernel kernel;
    check_hsa(plow_hsa_get_kernel(h, 0, "glm_rope_cache_bf16", &kernel));
    struct { void *cosb, *sinb; unsigned ctx; float theta; } args = {dc, ds, ctx, 8000000.0f};
    for (unsigned run = 0; run < 2; run++) {
        memset(actual, 0xff, output_bytes);
        check_hsa(plow_hsa_copy_h2d(h, 0, dc, actual, output_bytes));
        check_hsa(plow_hsa_copy_h2d(h, 0, ds, actual, output_bytes));
        check_hsa(plow_hsa_launch(h, 0, &kernel, 256 * 256, 1, 1, 256, 1, 1, 0, &args, sizeof(args)));
        check_hsa(plow_hsa_wait(h, 0));
        for (unsigned part = 0; part < 2; part++) {
            check_hsa(plow_hsa_copy_d2h(h, 0, actual, part ? ds : dc, output_bytes));
            size_t mismatches = 0;
            for (size_t i = 0; i < count; i++) {
                float expected = bf2f(reference[(i / 32) * 64 + part * 32 + i % 32]);
                if (!isfinite(actual[i]) || memcmp(actual + i, &expected, sizeof(float))) {
                    if (mismatches < 4) printf("rope mismatch run=%u part=%u pos=%zu dim=%zu actual=%a expected=%a\n",
                        run, part, i / 32, i % 32, actual[i], expected);
                    mismatches++;
                }
            }
            printf("rope cache run=%u part=%u ctx=%u mismatches=%zu/%zu\n", run, part, ctx, mismatches, count);
            fails += mismatches != 0;
        }
    }
    plow_hsa_free(h, dc); plow_hsa_free(h, ds);
...
static void replay_attention_split(plow_hsa* h, const char* input, const char* reducer,
                                   const char* adapter, const char* compact_reference) {
    const int wrap32 = adapter && strcmp(adapter, "wrap32") == 0;
    if (wrap32) adapter = NULL;
    FILE* f = fopen(input, "rb");
    uint32_t hdr[5];
    if (!f || fread(hdr, sizeof(hdr), 1, f) != 1) exit(1);
    unsigned m = hdr[1], ctx = hdr[2], splits = hdr[3], nnz = hdr[4];
    if (hdr[0] != 0x41535031 || (m != 32 && m != 64) || splits != 256 / m
            || (ctx != 512 && ctx != 8192 && ctx != 71680) || !nnz || nnz > m * 2048) exit(1);
    char arch[64]; uint32_t cus, lds;
    check_hsa(plow_hsa_device_info(h, 0, arch, &cus, &lds));
    if (strcmp(arch, "gfx950") || cus != 256) exit(1);
    size_t sizes[11] = {(size_t)m * 16 * 576 * 2, (size_t)m * ctx * 576 * 2,
        (m + 1) * 4, (m + 1) * 4, nnz * 4, m * 4, (m + 1) * 4,
        (size_t)m * 16 * 512 * 2, m * 4, (size_t)m * splits * 16 * 512 * 4,
        (size_t)m * splits * 16 * 4};
    void* host[11]; void* gpu[9] = {0};
    ...
    if (fgetc(f) != EOF || fclose(f)) exit(1);
    if (compact_reference) {
        if (!adapter || m != 64 || ctx != 71680) exit(1);
        const char* name = strrchr(input, '/');
        name = name ? name + 1 : input;
        size_t len = strlen(name);
        if (len < 4 || strcmp(name + len - 4, ".bin")) exit(1);
        const unsigned slots[3] = {7, 9, 10};
        const char* suffixes[3] = {"output", "partial", "lse"};
        for (unsigned i = 0; i < 3; i++) {
            char path[4096];
            int n = snprintf(path, sizeof(path), "%s/%.*s.compact.%s.bin",
                compact_reference, (int)(len - 4), name, suffixes[i]);
            if (n < 0 || (size_t)n >= sizeof(path)) exit(1);
            FILE* replacement = fopen(path, "rb");
            if (!replacement || fread(host[slots[i]], 1, sizes[slots[i]], replacement) != sizes[slots[i]]
                    || fgetc(replacement) != EOF || fclose(replacement)) exit(1);
        }
    }
    const int32_t* qo = host[2]; const int32_t* kvptr = host[3];
    const int32_t* indices = host[4]; const int32_t* sp = host[6];
    if (kvptr[0] || kvptr[m] <= 0 || kvptr[m] > (int32_t)nnz) exit(1);
    for (unsigned b = 0; b <= m; b++) {
        if (qo[b] != (int32_t)b || sp[b] != (int32_t)(b * splits)) exit(1);
        if (b == m) break;
        if (kvptr[b + 1] <= kvptr[b] || kvptr[b + 1] - kvptr[b] > 2048
                || ((int32_t*)host[5])[b] != 1 || ((int32_t*)host[8])[b] != (int32_t)splits) exit(1);
        for (int32_t j = kvptr[b]; j < kvptr[b + 1]; j++)
            if (indices[j] < (int32_t)(b * ctx) || indices[j] >= (int32_t)((b + 1) * ctx)) exit(1);
    }
    if (wrap32) {
        if (m != 64 || ctx != 71680 || sizes[1] <= UINT64_C(0x100000000)) exit(1);
        size_t bytes = (size_t)kvptr[m] * 576 * 2;
        unsigned char* compact = plow_hsa_alloc_host(h, bytes);
        int32_t* identity = plow_hsa_alloc_host(h, sizes[4]);
        if (!compact || !identity) exit(1);
        memset(identity, 0, sizes[4]);
        unsigned wrapped = 0;
        for (int32_t j = 0; j < kvptr[m]; j++) {
            // Diagnostic only: reproduce the vendor's 32-bit byte offsets, not intended attention.
            uint64_t offset = (uint64_t)indices[j] * 1152;
            uint32_t low = (uint32_t)offset;
            size_t first_bytes = UINT64_C(0x100000000) - low;
            if (first_bytes > 1152) first_bytes = 1152;
            memcpy(compact + (size_t)j * 1152, (char*)host[1] + low, first_bytes);
            if (first_bytes < 1152)
                memcpy(compact + (size_t)j * 1152 + first_bytes, host[1], 1152 - first_bytes);
            identity[j] = j;
            wrapped += offset != low || first_bytes < 1152;
        }
        if (!wrapped) exit(1);
        check_hsa(plow_hsa_copy_h2d(h, 0, gpu[1], compact, bytes));
        check_hsa(plow_hsa_copy_h2d(h, 0, gpu[4], identity, sizes[4]));
        printf("attention-split-wrap32 selected=%d wrapped=%u\n", kvptr[m], wrapped);
        plow_hsa_free(h, compact); plow_hsa_free(h, identity);
    }
    plow_hsa_kernel first, reduce;
    check_hsa(plow_hsa_get_kernel(h, 0, "_ZN5aiter39mla_dec_stage1_bf16_a16w16_subQ16_mqa16E", &first));
    load_pipeline_object(h, reducer);
    check_hsa(plow_hsa_get_kernel(h, 0, "_fwd_kernel_stage2_asm", &reduce));
    if (first.kernarg_size != 208 || first.group_segment_size != 65536 || first.private_segment_size
            || reduce.kernarg_size != 136 || reduce.group_segment_size || reduce.private_segment_size) exit(1);
    plow_hsa_kernel pack, unpad;
    void* source[6] = {0};
    uint64_t pa[14] = {0}, ua[3] = {0};
    void *unpad_allocation = NULL, *unpad_host = NULL;
    size_t unpad_bytes = (size_t)m * 8 * 512 * 2;
    if (adapter) {
        load_pipeline_object(h, adapter);
        check_hsa(plow_hsa_get_kernel(h, 0, "plow_mla_bf16_pack", &pack));
        check_hsa(plow_hsa_get_kernel(h, 0, "plow_mla_bf16_unpad", &unpad));
        if (pack.kernarg_size != 368 || pack.group_segment_size != 260 || pack.private_segment_size
                || unpad.kernarg_size != 280 || unpad.group_segment_size || unpad.private_segment_size) exit(1);
        const size_t source_sizes[6] = {(size_t)m * 8 * 512 * 2, (size_t)m * 8 * 64 * 2,
            (size_t)m * ctx * 512 * 2, (size_t)m * ctx * 64 * 2, (size_t)m * 2048 * 4, m * 4};
        for (unsigned i = 0; i < 6; i++) {
            void* tmp = plow_hsa_alloc_host(h, source_sizes[i]);
            source[i] = plow_hsa_alloc(h, 0, source_sizes[i]);
            if (!tmp || !source[i]) exit(1);
            memset(tmp, 0, source_sizes[i]);
            if (i < 4) {
                size_t rows = i < 2 ? (size_t)m * 8 : (size_t)m * ctx;
                unsigned width = (i & 1) ? 64 : 512, offset = (i & 1) ? 512 : 0;
                const bf16* original = host[i < 2 ? 0 : 1];
                for (size_t row = 0; row < rows; row++) {
                    size_t src = i < 2 ? row * 2 : row;
                    if (i < 2 && memcmp(original + src * 576, original + (src + 1) * 576, 576 * 2)) exit(1);
                    memcpy((bf16*)tmp + row * width, original + src * 576 + offset, width * 2);
                }
            } else {
                for (unsigned b = 0; b < m; b++) {
                    unsigned count = kvptr[b + 1] - kvptr[b];
                    unsigned length = count == 2048 ? ctx : count;
                    if (i == 5) ((int32_t*)tmp)[b] = length;
                    else for (unsigned j = 0; j < count; j++) {
                        unsigned index = indices[kvptr[b] + j] - b * ctx;
                        if (index >= length) exit(1);
                        ((int32_t*)tmp)[b * 2048 + j] = index;
                    }
                }
            }
            check_hsa(plow_hsa_copy_h2d(h, 0, source[i], tmp, source_sizes[i]));
            plow_hsa_free(h, tmp);
        }
        uint64_t pack_args[14] = {(uintptr_t)gpu[0], (uintptr_t)gpu[1], (uintptr_t)gpu[4],
            (uintptr_t)source[0], (uintptr_t)source[1], (uintptr_t)source[2], (uintptr_t)source[3],
            (uintptr_t)source[4], (uintptr_t)source[5], (uintptr_t)gpu[2], (uintptr_t)gpu[3],
            (uintptr_t)gpu[5], (uint64_t)m | ((uint64_t)ctx << 32), 2048};
        memcpy(pa, pack_args, sizeof(pa));
        unpad_allocation = plow_hsa_alloc(h, 0, unpad_bytes + 128);
        unpad_host = plow_hsa_alloc_host(h, unpad_bytes + 128);
        if (!unpad_allocation || !unpad_host) exit(1);
        ua[0] = (uintptr_t)((char*)unpad_allocation + 64); ua[2] = m;
    }
    size_t out_sizes[3] = {sizes[9], sizes[10], sizes[7]};
    void* allocation[3]; void* outputs[3]; void* staging[3];
    for (unsigned i = 0; i < 3; i++) {
        allocation[i] = plow_hsa_alloc(h, 0, out_sizes[i] + 128);
        staging[i] = plow_hsa_alloc_host(h, out_sizes[i] + 128);
        if (!allocation[i] || !staging[i]) exit(1);
        outputs[i] = (char*)allocation[i] + 64;
    }
    uint64_t args[26] = {0};
    args[0] = (uintptr_t)outputs[0]; args[2] = (uintptr_t)outputs[1];
    args[4] = (uintptr_t)gpu[0]; args[6] = (uintptr_t)gpu[1];
    args[8] = (uintptr_t)gpu[3]; args[10] = (uintptr_t)gpu[4]; args[12] = (uintptr_t)gpu[5];
    float scale = 0.0625f; memcpy(&args[14], &scale, 4);
    args[16] = 16; args[18] = splits; args[20] = 16 * 576 * 2; args[22] = 576 * 2;
    uint64_t ra[17] = {(uintptr_t)outputs[0], (uintptr_t)outputs[1], (uintptr_t)outputs[2],
        (uintptr_t)outputs[2], (uintptr_t)gpu[2], (uintptr_t)gpu[3], (uintptr_t)gpu[5],
        (uintptr_t)gpu[6], (uintptr_t)gpu[8], splits * 16, 1, 16, 16 * 512, 512, 0, 0, 0};
    size_t mismatches = 0, partial_mismatches = 0; int guards = 1;
    for (unsigned run = 0; run < 2; run++) {
        unsigned char poison = run ? 0xaa : 0x55;
        for (unsigned i = 0; i < 3; i++) {
            memset(staging[i], poison, out_sizes[i] + 128);
            check_hsa(plow_hsa_copy_h2d(h, 0, allocation[i], staging[i], out_sizes[i] + 128));
        }
        if (adapter) {
            memset(unpad_host, poison, unpad_bytes + 128);
            check_hsa(plow_hsa_copy_h2d(h, 0, unpad_allocation, unpad_host, unpad_bytes + 128));
            // Destroy captured inputs so a missing pack write cannot inherit the reference.
            for (unsigned i = 0; i < 6; i++) {
                void* tmp = plow_hsa_alloc_host(h, sizes[i]);
                if (!tmp) exit(1);
                memset(tmp, poison, sizes[i]);
                check_hsa(plow_hsa_copy_h2d(h, 0, gpu[i], tmp, sizes[i]));
                plow_hsa_free(h, tmp);
            }
            check_hsa(plow_hsa_launch(h, 0, &pack, 256 * 256, 1, 1, 256, 1, 1, 0, pa, sizeof(pa)));
        }
        check_hsa(plow_hsa_launch(h, 0, &first, 256, m, splits, 256, 1, 1, 0, args, sizeof(args)));
        check_hsa(plow_hsa_launch(h, 0, &reduce, m * 64, 16, 1, 64, 1, 1, 0, ra, sizeof(ra)));
        if (adapter) {
            ua[1] = (uintptr_t)outputs[2];
            check_hsa(plow_hsa_launch(h, 0, &unpad, 16 * 256, 1, 1, 256, 1, 1, 0, ua, sizeof(ua)));
        }
        check_hsa(plow_hsa_wait(h, 0));
        if (adapter) {
            size_t pack_mismatches = 0;
            for (unsigned i = 0; i < 6; i++) {
                size_t bytes = i == 1 ? (size_t)kvptr[m] * 576 * 2 : sizes[i];
                void* tmp = plow_hsa_alloc_host(h, bytes);
                if (!tmp) exit(1);
                check_hsa(plow_hsa_copy_d2h(h, 0, tmp, gpu[i], bytes));
                if (i == 1) {
                    for (int32_t j = 0; j < kvptr[m]; j++)
                        pack_mismatches += memcmp((bf16*)tmp + (size_t)j * 576,
                            (bf16*)host[1] + (size_t)indices[j] * 576, 576 * 2) != 0;
                } else if (i == 4) {
                    for (int32_t j = 0; j < kvptr[m]; j++)
                        pack_mismatches += ((int32_t*)tmp)[j] != j;
                } else pack_mismatches += memcmp(tmp, host[i], bytes) != 0;
                plow_hsa_free(h, tmp);
            }
            printf("attention-split-pack M=%u ctx=%u run=%u mismatch=%zu\n", m, ctx, run, pack_mismatches);
            if (pack_mismatches) guards = 0;
            check_hsa(plow_hsa_copy_d2h(h, 0, unpad_host, unpad_allocation, unpad_bytes + 128));
            for (unsigned j = 0; j < 64; j++)
                guards &= ((unsigned char*)unpad_host)[j] == poison
                    && ((unsigned char*)unpad_host)[64 + unpad_bytes + j] == poison;
            const bf16* actual = (bf16*)((char*)unpad_host + 64);
            for (size_t j = 0; j < unpad_bytes / 2; j++)
                mismatches += actual[j] != ((bf16*)host[7])[(j / 512) * 1024 + j % 512];
        }
        for (unsigned i = 0; i < 3; i++) {
            check_hsa(plow_hsa_copy_d2h(h, 0, staging[i], allocation[i], out_sizes[i] + 128));
            for (unsigned j = 0; j < 64; j++)
                guards &= ((unsigned char*)staging[i])[j] == poison
                    && ((unsigned char*)staging[i])[64 + out_sizes[i] + j] == poison;
        }
        const bf16* actual = (bf16*)((char*)staging[2] + 64);
        for (size_t j = 0; j < sizes[7] / 2; j++) {
            mismatches += actual[j] != ((bf16*)host[7])[j];
            if (!isfinite(bf2f(actual[j]))) guards = 0;
        }
        const float* lse = (float*)((char*)staging[1] + 64);
        const float* part = (float*)((char*)staging[0] + 64);
        for (size_t j = 0; j < sizes[10] / 4; j++) {
            const unsigned row = j / (splits * 16), split = (j / 16) % splits;
            const unsigned valid = ((unsigned)(kvptr[row + 1] - kvptr[row]) + 63) / 64;
            // The installed reducer reads min(splits, ceil(length / 64)); unused slots are uninitialized.
            if (split >= valid) continue;
            if (!isfinite(lse[j]) || !isfinite(((float*)host[10])[j])) guards = 0;
            partial_mismatches += memcmp(lse + j, (float*)host[10] + j, 4) != 0;
            partial_mismatches += memcmp(part + j * 512, (float*)host[9] + j * 512, 512 * 4) != 0;
        }
    }
    printf("attention-split-replay M=%u ctx=%u splits=%u pipeline=%d wrap32=%d compact-reference=%d output-mismatch=%zu partial-mismatch=%zu guards=%d\n",
        m, ctx, splits, adapter != NULL, wrap32, compact_reference != NULL, mismatches, partial_mismatches, guards);
    if (mismatches || partial_mismatches || !guards) fails++;
    for (unsigned i = 0; i < 11; i++) plow_hsa_free(h, host[i]);
    for (unsigned i = 0; i < 9; i++) if (gpu[i]) plow_hsa_free(h, gpu[i]);
    for (unsigned i = 0; i < 3; i++) { plow_hsa_free(h, allocation[i]); plow_hsa_free(h, staging[i]); }
    for (unsigned i = 0; i < 6; i++) if (source[i]) plow_hsa_free(h, source[i]);
    if (unpad_allocation) plow_hsa_free(h, unpad_allocation);
    if (unpad_host) plow_hsa_free(h, unpad_host);
}
...
int main(int argc, char** argv) {
    ...
    if (argc > 2 && strcmp(argv[2], "attention-split-replay") == 0) {
        const uint32_t descriptor[16] = {65536, 0, 0, 0, 0x10c0, 0, 0, 0,
            0, 0, 0, 0x3f, 0xc033f, 0x384, 8, 0};
        if (n < 0xf80 || memcmp((char*)co + 0xf40, descriptor, sizeof(descriptor))) {
            fprintf(stderr, "split attention descriptor differs\n"); return 1;
        }
        uint32_t size = 208;
        memcpy((char*)co + 0xf48, &size, sizeof(size));
    }
    ...
    if (argc > 2 && strcmp(argv[2], "attention-split-replay") == 0) {
        if (argc < 5 || argc > 7) { fprintf(stderr, "split replay requires fixture and reducer [adapter|wrap32 diagnostic] [compact-reference]\n"); return 1; }
        replay_attention_split(h, argv[3], argv[4], argc >= 6 ? argv[5] : NULL, argc == 7 ? argv[6] : NULL);
        free(co); plow_hsa_shutdown(h); return fails ? 1 : 0;
    }
    if (argc > 2 && (strcmp(argv[2], "indexer-score-replay") == 0 || strcmp(argv[2], "indexer-cache-score-replay") == 0)) {
        int cache_score = strcmp(argv[2], "indexer-cache-score-replay") == 0;
        if (argc != (cache_score ? 8 : 7)) { fprintf(stderr, "indexer-score-replay requires M/ctx/reference-dir/output [key-object]\n"); return 1; }
        replay_indexer_score(h, (unsigned)strtoul(argv[3], NULL, 10),
            (unsigned)strtoul(argv[4], NULL, 10), argv[5], argv[6], cache_score ? argv[7] : NULL);
        plow_hsa_shutdown(h); free(co);
        return fails ? 1 : 0;
    }
    if (argc > 2 && strcmp(argv[2], "indexer-prefill-replay") == 0) {
        if (argc != 6 && argc != 7) { fprintf(stderr, "indexer-prefill-replay requires input/output/score-object [append-object with fixed score stride]\n"); return 1; }
        replay_indexer_prefill(h, argv[3], argv[4], argv[5], argc == 7 ? argv[6] : NULL);
        plow_hsa_shutdown(h); free(co);
        return fails ? 1 : 0;
    }
    if (argc > 2 && strcmp(argv[2], "indexer-decode-replay") == 0) {
        if (argc != 5) { fprintf(stderr, "indexer-decode-replay requires input/output\n"); return 1; }
        replay_indexer_decode(h, argv[3], argv[4]);
        plow_hsa_shutdown(h); free(co);
        return fails ? 1 : 0;
    }
    if (argc > 2 && strcmp(argv[2], "indexer-quant-replay") == 0) {
        if (argc != 5) { fprintf(stderr, "indexer-quant-replay requires input/output\n"); return 1; }
        replay_indexer_quant(h, argv[3], argv[4]);
        plow_hsa_shutdown(h);
        free(co);
        return fails ? 1 : 0;
    }
    if (argc > 2 && strcmp(argv[2], "rope-cache") == 0) {
        if (argc != 4) { fprintf(stderr, "rope-cache requires pinned BF16 cache\n"); return 1; }
        test_rope_cache(h, argv[3]);
        plow_hsa_shutdown(h);
        free(co);
        return fails ? 1 : 0;
    }
