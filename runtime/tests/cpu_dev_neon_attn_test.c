/* cpu_dev_neon_attn_test.c — NEON HEADNORM_ROPE / FLASH_* against the golden tier.
 *
...
static plow_cpu_kernel_fn avx(uint16_t op, plow_cpu_kernel_fn golden, const char* name) {
    ...
    CHECK(plow_cpu_tier_of(op) == PLOW_CPU_ISA_NEON, "%s: tier %d", name, plow_cpu_tier_of(op));
    return f ? f : golden;
}
...
static void bench_decode(uint32_t n_batch, uint32_t n_head, uint32_t n_kv_head, uint32_t hd, uint32_t kvlen,
    ...
    for (int which = with_golden ? 0 : 1; which < 2; which++) {
        ...
        printf("flash_decode B=%u kvlen=%u hd=%u heads=%u/%u %s: %.1f us/call, %.1f GB/s KV read\n", n_batch, kvlen,
               hd, n_head, n_kv_head, which ? "neon" : "golden", best * 1e6, bytes / best / 1e9);
    }
...
static void bench_prefill(uint32_t n_q, uint32_t n_head, uint32_t n_kv_head, uint32_t hd, uint32_t window,
    ...
    for (int which = with_golden ? 0 : 1; which < 2; which++) {
        ...
        printf("flash_prefill T=%u hd=%u heads=%u/%u window=%u %s: %.2f ms/call, %.2f ns per (q,k,head)\n", n_q, hd,
               n_head, n_kv_head, window, which ? "neon" : "golden", best * 1e3, best * 1e9 / pairs);
    }
...
int main(int argc, char** argv) {
    ...
    if (tier < PLOW_CPU_ISA_NEON) {
        printf("no NEON bf16 on this host; skipping\n");
        return 0;
    }
    ...
    if (fails == 0) printf("all neon attention tests passed\n");
    return fails ? 1 : 0;
}
