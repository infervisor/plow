...
static double run(PlowDevInst d, NSArray<id<MTLBuffer>>* bs, id<MTLComputePipelineState> pso, unsigned variant) {
    ...
    [enc dispatchThreadgroups:MTLSizeMake(dedicated?(d.i[1]+width-1)/width+1:d.blocks+1,1,1) threadsPerThreadgroup:MTLSizeMake(dedicated?64:1024,1,1)];
...
int main(int argc, char** argv) {
    @autoreleasepool {
        assert(argc == 3 || (argc == 4 && !strcmp(argv[3], "--dedicated")));
        dedicated = argc == 4;
        ...
        for (unsigned v=0;v<2;v++) {
            NSString* code=[NSString stringWithContentsOfFile:@(argv[v+1]) encoding:NSUTF8StringEncoding error:&error];
            code=[@"#define PLOW_MX4_FOUR_ROWS 1\n" stringByAppendingString:code];
            id<MTLLibrary> lib=[dev newLibraryWithSource:code options:opts error:&error];
            if (!lib) { fprintf(stderr,"%s\n",error.description.UTF8String); return 1; }
            psos[v]=[dev newComputePipelineStateWithFunction:[lib newFunctionWithName:dedicated?@"plow_mx4_dedicated":@"plow_single"] error:&error];
            assert(psos[v] && psos[v].maxTotalThreadsPerThreadgroup>=(dedicated?64:1024));
        }
        const unsigned shapes[][3]={{1,8,32},{2,16,64},{4,128,2048},{4,6144,2048},{4,32,1024},{4,2048,6144},{4,151936,2048},{4,132,48}};
        const unsigned partitions[]={1,8,16,20,64};
        unsigned checks=0;
        for(unsigned c=0;c<8;c++) for(unsigned part=0;part<5;part++)
        for(unsigned activation=0;activation<4;activation++)
        for(unsigned glu=0;glu<2;glu++) { @autoreleasepool {
            if((c==6 && glu) || (c==7 && dedicated)) continue;
            unsigned m=shapes[c][0],n=shapes[c][1],k=shapes[c][2];
            NSMutableArray<id<MTLBuffer>>* bs=[NSMutableArray array];
            size_t sizes[]={m*n*2+64,(m+1)*k*2,n*k/2,n*((k+31)/32),n*((k+31)/32),n*k/2,n*((k+31)/32),4};
            ...
            d.i[4]=1;d.i[0]=m;d.i[1]=n;d.i[2]=k;d.i[5]=activation;
            float f0=0.7f,f1=0.2f;memcpy(&d.fj[0],&f0,4);memcpy(&d.fj[1],&f1,4);
            NSData* expected=nil;
            for(unsigned v=0;v<2;v++) {
            ...
            checks++;
            if(activation==1 && c>=3 && c!=4 && partitions[part]==16) {
