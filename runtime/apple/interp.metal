...
// 64x64 sub-tiles, 16x16 threads each owning a 4x4 micro-tile, K staged 32 at a time in
// threadgroup memory as f32 (A: 64x32, B: 64x32). Weights are bf16 or e4m3 (per-column scale in
// the epilogue); the second weight stream (GLU) reuses the same machinery.
constant uint GK = 32;
struct GemmArgs {
...
// acc[4][4] += A[m0+tm*4 .. +4][k0..k0+32] . B[n0+tn*4 .. +4][k0..k0+32]^T via shared staging.
inline void gemm_sub(thread const GemmArgs& g, uint m0, uint n0, threadgroup float* As, threadgroup float* Bs,
                     uint lid, thread float (&acc)[4][4]) {
    uint tm = lid / 16u, tn = lid % 16u;
    for (uint k0 = 0; k0 < g.K; k0 += GK) {
        // stage: 64 rows x 32 k = 2048 values each, 8 per thread
        for (uint e = lid; e < 64u * GK; e += NT) {
            uint r = e / GK, kk = e % GK;
            uint m = m0 + r, k = k0 + kk;
            As[r * GK + kk] = (m < g.M && k < g.K) ? bf2f(g.A[m * g.K + k]) : 0.0f;
            uint n = n0 + r;
            float b = 0.0f;
            if (n < g.N && k < g.K) b = g.B8 ? e4m3(g.B8[n * g.K + k]) : bf2f(g.B16[n * g.K + k]);
            Bs[r * GK + kk] = b;
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        for (uint kk = 0; kk < GK; kk++) {
            float a[4], b[4];
            for (uint i = 0; i < 4; i++) a[i] = As[(tm * 4 + i) * GK + kk];
            for (uint j = 0; j < 4; j++) b[j] = Bs[(tn * 4 + j) * GK + kk];
            for (uint i = 0; i < 4; i++)
...
void op_gemm(const thread Inst& in, device const ulong* tab, uint slice, uint nblk, uint BM, uint BN, bool fp8,
    ...
    for (uint lin = slice; lin < tm * tn; lin += nblk) {
        ...
        gemm_tile(g, C, N, m0, m1, n0, n1, As, Bs, lid);
    }
}
...
void op_gemm_glu(const thread Inst& in, device const ulong* tab, uint slice, uint nblk, bool fp8,
    ...
    for (uint lin = slice; lin < tm * tn; lin += nblk) {
        ...
        for (uint mm = m0; mm < m1; mm += 64u)
            for (uint nn = n0; nn < n1; nn += 64u) {
                float ag[4][4], au[4][4];
                for (uint i = 0; i < 4; i++) for (uint j = 0; j < 4; j++) { ag[i][j] = 0.0f; au[i][j] = 0.0f; }
                if (!poison) {
                    gemm_sub(gg, mm, nn, As, Bs, lid, ag);
                    gemm_sub(gu, mm, nn, As, Bs, lid, au);
                }
                for (uint i = 0; i < 4; i++) {
                    uint m = mm + tmi * 4 + i;
                    if (m >= m1) continue;
...
bool exec_op(const thread Inst& in, device const ulong* tab, uint slice, uint nblk,
             threadgroup float* As, threadgroup float* Bs, threadgroup float* red, threadgroup ulong* keys,
             uint lid, uint sg, uint lane) {
    switch (in.op) {
        case 0: return true;
        ...
        case 8: op_gemm(in, tab, slice, nblk, 256, 256, false, As, Bs, lid); return true;
        case 14: op_gemm(in, tab, slice, nblk, 64, 128, false, As, Bs, lid); return true;
        case 15: op_gemm(in, tab, slice, nblk, 128, 128, false, As, Bs, lid); return true;
        case 94: op_gemm(in, tab, slice, nblk, 128, 256, false, As, Bs, lid); return true;
        case 95: op_gemm(in, tab, slice, nblk, 192, 256, false, As, Bs, lid); return true;
        case 33: op_gemm(in, tab, slice, nblk, 256, 256, true, As, Bs, lid); return true;
        case 34: op_gemm(in, tab, slice, nblk, 128, 128, true, As, Bs, lid); return true;
        case 35: op_gemm(in, tab, slice, nblk, 64, 128, true, As, Bs, lid); return true;
        case 100: op_gemm(in, tab, slice, nblk, 128, 256, true, As, Bs, lid); return true;
        case 101: op_gemm(in, tab, slice, nblk, 192, 256, true, As, Bs, lid); return true;
        case 20: op_gemm_glu(in, tab, slice, nblk, false, As, Bs, lid); return true;
        case 36: op_gemm_glu(in, tab, slice, nblk, true, As, Bs, lid); return true;
...
kernel void plow_interp(device const Inst* insts [[buffer(0)]],
    ...
    threadgroup float As[64 * GK];
    threadgroup float Bs[64 * GK];
    ...
    for (; head < end; head++) {
        ...
        if (!exec_op(in, tab, e.slice, in.blocks, As, Bs, red, keys, lid, sg, lane)) {
...
kernel void plow_single(device const Inst* insts [[buffer(0)]],
    ...
    if (!exec_op(in, tab, tg, in.blocks, As, Bs, red, keys, lid, sg, lane))
